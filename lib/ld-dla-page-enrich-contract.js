...
(function (root, factory) {
  ...
  var facilitatedDeliveryMod = null;
  if (typeof require === "function") {
    try {
      facilitatedDeliveryMod = require("./dla-facilitated-delivery.js");
    } catch (_) {}
  }
  ...
  /**
   * S82 — generic operand sufficiency when downstream production requires transform without invention.
   */
  var DLA_OPERAND_TRANSFORM_SUFFICIENCY_COMMISSION =
    "S82 operand transform sufficiency: when learner_task, expected_output, or a bound response_fulfilment surface requires the learner to rewrite, reframe, transform, or otherwise reconstruct supplied particulars without adding information, every task-input operand commissioned in required_materials (including evidence-provider rows) must contain enough recoverable particulars in purpose and specification for that transformation. Do not commission label-only judgements, interpretations, or conclusions where the required transformed response depends on underlying particulars that are not supplied.";
  ...
  function normalizeDlaAssembleCtx(ctx) {
    ...
    return {
      ...
      facilitatedDelivery: c.facilitatedDelivery === true,
  ...
  function buildDlaFacilitatedDeliveryContractBlock() {
    if (
      facilitatedDeliveryMod &&
      typeof facilitatedDeliveryMod.buildDlaFacilitatedDeliveryContractBlock === "function"
    ) {
      return facilitatedDeliveryMod.buildDlaFacilitatedDeliveryContractBlock();
    }
    return "";
  }
  ...
  function buildDlaSectionProduction(ctx) {
    ...
    if (ctx.facilitatedDelivery) {
      var facilitatedBlock = buildDlaFacilitatedDeliveryContractBlock();
      if (facilitatedBlock) parts.push("", facilitatedBlock);
    }
  ...
  function buildDlaSectionCommissioning(ctx) {
    ...
    var parts = [
      ...
      DLA_OPERAND_TRANSFORM_SUFFICIENCY_COMMISSION,
      ...
      "S78-T-041 (commissioning): on the culminating/final activity, when transfer/application is pedagogically appropriate, commission compact transfer/application production via transfer_prompt OR template with response_fulfilment (changed context, same core learning). purpose/specification must require learner production without a worked answer and without new teaching. If evidence_decision.required is true with separate_provider, the inspectable evidence provider must be a distinct scenario or task_card row—not transfer_prompt and not the response template; do not attach evidence_requirement to transfer_prompt. transfer_prompt is not consolidation_summary, not Study tips, and not a second worked example. Earlier non-culminating activities may omit transfer_prompt when transfer is not required by their episode beats.",
  ...
  function buildDlaSectionProviders() {
    return withHeading(
      ...
      [
        ...
        "- separate_provider closure: when evidence_layout is separate_provider, provider_material_ids must resolve to inspectable-particulars rows (prefer scenario, task_card, data_table, or reference_table carrying values/extracts/cases)—not transfer_prompt, consolidation_summary, checklist, or teaching text alone. transfer_prompt commissions learner transfer/application production only; do not attach evidence_requirement to transfer_prompt. When a culminating activity requires both inspectable evidence and transfer production, commission scenario (or task_card) as the evidence provider and bind transfer production via learner_task plus template (response_fulfilment) or a distinct transfer_prompt row without evidence_requirement. Transfer pedagogy belongs in learner_task and expected_output, not in the evidence provider material_type or teaching-framed purpose/specification.",
        "- Evidence-provider purpose/specification must supply inspectable particulars (objective, constraint, values, cases, extracts)—not teaching overview or transfer-instruction prose. Avoid specification language that introduces or explains theory rather than stating particulars to inspect.",
        "- When downstream production requires rewrite, reframe, transform, or reconstruction without adding information, apply operand transform sufficiency (§commissioning): provider purpose, specification, and learner_action must commission recoverable particulars for every supplied item subject to that transformation—not label-only judgements or conclusions.",
  ...
  function buildDlaSectionOutput(ctx) {
    ...
    if (ctx.facilitatedDelivery) {
      parts.push(
        "Facilitated session required activity-row fields (in addition to fields above):",
        "- each activities[] row MUST include grouping (existing schema vocabulary: individual, pair, pairs, small_group, whole_group, individual_then_small_group, individual_then_pair, mixed)",
        "- learner_task MUST express participant-facing orchestration consistent with grouping; facilitator_moves alone is insufficient for collaborative modes",
        ""
      );
    }
    parts.push(
  ...
  function buildDlaSectionExamples(ctx) {
    ...
    if (ctx.facilitatedDelivery) {
      lines.push(
        "",
        "Facilitated session activity miniature (grouping + participant-facing learner_task):",
        "{",
        '  "activity_id": "A1",',
        '  "title": "Compare inflation indicators",',
        '  "grouping": "small_group",',
        '  "learner_task": "In your small group, compare the CPI and GDP deflator using the task cards, then agree one defensible difference to share.",',
        '  "expected_output": "A group comparison note citing one indicator-specific difference supported by the cards.",',
        '  "intellectual_coherence_bridge": "The overview established inflation as a measurement problem. This activity applies that orientation to compare measures.",',
        '  "activity_preamble": "Different inflation measures capture different baskets and purposes. Before you judge which difference matters, compare what each measure includes.",',
        "  ...remaining required DLA fields per §10..."
      );
    }
    lines.push(
        "",
        "General obligation-population miniature:",
        "{",
        '  "activity_id": "A1",',
    ...
    return withHeading("examples", lines.join("\n"));
  }
  ...
  return {
    ...
    DLA_OPERAND_TRANSFORM_SUFFICIENCY_COMMISSION: DLA_OPERAND_TRANSFORM_SUFFICIENCY_COMMISSION,
