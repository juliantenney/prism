 ...
 * Depth authority: pack GAM-PRES-08/09 word floors are canonical for generation.
 * GAM-FMT char heuristics are capture-time advisory sanity checks only (not word-equivalent).
 */
(function (root, factory) {
  ...
  var GAM_DEPTH_AUTHORITY = "GAM-PRES-08/09";
  /** Capture heuristic only — subordinate to GAM-PRES word floors; not equivalent to 80 words. */
  var MIN_CHECKLIST_BODY = 80;
  /** Capture heuristic only — subordinate to GAM-PRES depth floors. */
  ...
  function countExpectedMaterialsFromLearningActivities(learningActivities) {
    var acts = normalizeLearningActivitiesList(learningActivities);
    var total = 0;
    acts.forEach(function (act) {
      var mats = act && act.required_materials;
      if (Array.isArray(mats)) total += mats.length;
    });
    return total;
  }

  /**
   * Tiered capture gate for workflow run-mode (Tier 1–2 blocking, Tier 3 warning only).
   * @param {string} text
   * @param {{ learningActivities?: *, upstreamAvailable?: boolean }} [options]
   */
  function validateGamPackTextCaptureGate(text, options) {
    var opts = options || {};
    var blockingErrors = [];
    var warnings = [];
    var skippedCoverage = false;
    var upstreamAvailable = opts.upstreamAvailable === true;
    var learningActivities = opts.learningActivities;
    var expectedMaterialCount = upstreamAvailable
      ? countExpectedMaterialsFromLearningActivities(learningActivities)
      : 0;
    var coverageSkipMsg =
    ...
    if (detectGamJsonStubOutput(text)) {
      blockingErrors.push(
        "GAM-FMT-01: JSON specification stub detected — pack text Material/Content required"
      );
    }
    if (!isPackTextGamFormat(text)) {
      blockingErrors.push("GAM-FMT-02: missing Material:/Content: pack text structure");
    }

    var materials = parseGamMaterialsFromText(text);
    var activityBlocks = (String(text || "").match(/Activity ID:\s*\S+/gi) || []).length;
    var thin = [];
    var tier1Pass = !detectGamJsonStubOutput(text) && isPackTextGamFormat(text);

    if (tier1Pass) {
      if (!upstreamAvailable || expectedMaterialCount === 0) {
        skippedCoverage = true;
        warnings.push(coverageSkipMsg);
      } else {
        ...
        var coverage = validateGamActivityCoverage(materials, learningActivities);
        if (!coverage.ok) {
          blockingErrors = blockingErrors.concat(coverage.errors);
        }
      }
    }
    ...
    if (thin.length) {
      warnings.push(
        "GAM-FMT-04: thin Content bodies (capture char heuristic; " +
          GAM_DEPTH_AUTHORITY +
          " word floors authoritative) — " +
          thin.slice(0, 6).join(", ")
      );
    }
  ...
  function buildGamOutputContractSystemPrompt() {
    return [
      ...
      "If a required_material includes evidence_requirement, generate inspectable learner evidence aligned to its learner_action and observable_features; avoid pre-disclosing the target conclusion.",
      "Keep evidence-provider materials observational and keep response scaffolds learner-blank unless they use a different non-focal example.",
      "When evidence_decision.required is true, pre-task teaching/worked_example/modelling_note must not analyse the focal evidence, answer the focal task, or state the preferred judgement; use a distinct analogous case or procedure-only modelling instead.",
      "When evidence_requirement provenance indicates system_generated_simulation (or simulated/synthetic), label it explicitly in Content: Simulated results created for this learning activity.",
      "For combined_evidence_workspace with system_generated_simulation, put Simulated in the material title (e.g. Simulated Entry Evidence Table).",
      "When evidence_requirement provenance is conversation_attachment, reproduce accurate attributed excerpts from the authoritative Copilot conversation material; preserve wording and punctuation; mark partial-line/sentence omissions with ellipsis; do not replace evidence with thematic summaries or pre-task interpretations; do not mix quotation rows with summary-only rows; for combined_evidence_workspace include a fixed quotation/extract/value/observation field (not poem/category alone); do not add a Simulated label; do not invent quotations from memory; if unavailable, do not fabricate or reconstruct—state that the source-bound requirement could not be fulfilled (SOURCE_BOUND_UNFULFILLED) and do not silently substitute simulated evidence.",
      "Depth floors: obey pack " + GAM_DEPTH_AUTHORITY + " word minima on generation.",
      "Capture heuristics (advisory): checklist bodies >= " + MIN_CHECKLIST_BODY + " chars; teaching bodies >= " + MIN_TEACHING_BODY + " chars.",
      "When checklist bodies use guided-review JSON (review_mode guided_criteria), normally include 2–3 independently observable expected/repair pairs per criterion (max 4); forbid generic-only repairs and complete model-answer repairs.",
      "A4 Evaluate (M12 / A4-scenario): scenario Purpose must include 'strategy menu'; Content must include Strategy A: through Strategy E: with Maya household context (>= " + MIN_A4_COMPOSE_BODY + " chars).",
      "A4 Evaluate (M14): worked judgement must contrast Weak Evaluation Example and Strong Evaluation Example (>= " + MIN_A4_COMPOSE_BODY + " chars).",
  ...
  return {
    ...
    countExpectedMaterialsFromLearningActivities: countExpectedMaterialsFromLearningActivities,
    validateGamPackTextCaptureGate: validateGamPackTextCaptureGate,
