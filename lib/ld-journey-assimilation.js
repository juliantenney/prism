...
(function (root, factory) {
  ...
  var CORE_LINES = [
    ...
    "- Module: " +
      MODULE_ID +
      " | Design Page compose only — assimilate existing upstream journey signals into wrapper prose (overview, learning_purpose, knowledge_summary, study_tips, activity_preamble, intellectual_coherence_bridge).",
    "- Goal: express Question → Investigation → Evidence → Judgement — not Activity → Activity → Activity. Improve composition around preserved GAM materials; never rewrite, compress, or mutate activity.materials.* bodies.",
    "- Scope: wrapper sections and learner-facing framing fields only. Obey LD-MATERIALS-COPY PREC-02 and LD-AUTHORIAL-EXPOSITION role separation — journey assimilation does not override materials fidelity or verbatim field preservation."
  ];
  ...
  var UPSTREAM_SIGNAL_LINES = [
    ...
    "  • learning_activities — study_orientation, intellectual_frame (especially first activity), intellectual_coherence_bridge chain, reasoning_orientation, transfer_or_application_task.",
    ...
    "  • learning_sequence — timeline order/duration for activity flow; assimilate transition_to_next and phase_type into overview, bridges, and study_tips synthesis (not into materials bodies)."
  ];
  ...
  var TRANSITION_LINES = [
    "- ACTIVITY WRAPPERS AND TRANSITIONS:",
    "  • Make intellectual movement visible — why this stage follows the previous one and what reasoning carries forward.",
    "  • When learning_sequence.timeline[] is bound, use transition_to_next and phase_type to inform preamble/bridge phrasing without copying facilitator choreography verbatim.",
    "  • Prefer cumulative progression: each stage should feel like evidence or capability accumulated for the next judgement.",
    "  • Preserve upstream intellectual_coherence_bridge and activity_preamble verbatim when present; assimilation improves overview/study_tips and compositional read — do not replace preserved field text with thinner stubs."
  ];
  ...
  var CLOSURE_LINES = [
    "- CLOSURE / STUDY TIPS:",
    "  • Draw on upstream consolidation, transfer, debrief, and judgement structures from GAM and DLA transfer_or_application_task when present.",
