...
(function (root, factory) {
  ...
  /** Design Page LLM shorthand — activity_id + material type token (e.g. A1_TEXT, A1_WORKED). */
  var MATERIAL_REFERENCE_TOKEN_RES = [
    /^[A-Z]\d+_(?:TEXT|WORKED|SAMPLE|CHECK|TABLE|SCENARIO|TEMPLATE|PROMPT)(?:_[A-Z0-9_]+)?$/i,
    /^M\d+(?:_[A-Z][A-Z0-9_]*)?$/i
  ];
  ...
  function looksLikeMaterialDescriptionSynopsis(text) {
    ...
    if (
      ...
      if (/^(definitions? of|overview of|classification of|comparison of)\b/i.test(trimmed)) {
        return true;
      }
      if (/\busing evidence and reasoning\.?$/i.test(trimmed)) {
        return true;
      }
      if (/^inflation refers to\b/i.test(trimmed)) {
        return true;
      }
    }
    return false;
  ...
  function looksLikeMaterialReferenceToken(text) {
    var trimmed = String(text || "").trim();
    if (!trimmed) return false;
    return MATERIAL_REFERENCE_TOKEN_RES.some(function (re) {
      return re.test(trimmed);
    });
  }
  ...
  function stringLooksPlaceholderOnly(text) {
    var trimmed = String(text || "").trim();
    if (!trimmed) return false;
    if (looksLikeMaterialReferenceToken(trimmed)) return true;
  ...
  return {
    ...
    looksLikeMaterialReferenceToken: looksLikeMaterialReferenceToken,
