...
(function (root, factory) {
  ...
  function extractMaterialHeadings(html) {
    var re = /<(?:h4|p)([^>]*)>([\s\S]*?)<\/(?:h4|p)>/gi;
    var headings = [];
    var m;
    while ((m = re.exec(String(html || ""))) !== null) {
      var attrs = String(m[1] || "");
      if (!/\b(?:util-material-heading|util-supporting-label)\b/i.test(attrs)) continue;
      headings.push({
        pos: m.index,
        text: String(m[2] || "")
          .replace(/<[^>]+>/g, "")
    ...
    var tableRe = /<div[^>]*class="[^"]*\butil-material-table\b[^"]*"[^>]*>/gi;
    while ((m = tableRe.exec(String(html || ""))) !== null) {
      headings.push({ pos: m.index, text: "Worksheet" });
    }
  ...
  function findActivityHtmlBlock(html, page, rowIndex) {
    ...
    var journeyMarker = 'id="activity-' + String(rowIndex + 1) + '"';
    var journeyStart = text.indexOf(journeyMarker);
    if (journeyStart >= 0) {
      var nextJourneyMarker = 'id="activity-' + String(rowIndex + 2) + '"';
      var nextJourneyStart = text.indexOf(nextJourneyMarker, journeyStart + journeyMarker.length);
      return text.substring(
        journeyStart,
        nextJourneyStart >= 0 ? nextJourneyStart : text.length
      );
    }
    function activityHeadingPosition(activityTitle, fromIndex) {
      var escaped = String(activityTitle).replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      var re = new RegExp(
        '<h2[^>]*class="[^"]*\\butil-activity-title\\b[^"]*"[^>]*>[\\s\\S]*?' +
          escaped,
        "gi"
      );
    ...
    var start = activityHeadingPosition(title, 0);
    if (start < 0) return "";
    ...
    for (i = 0; i < rows.length; i += 1) {
      ...
      var pos = activityHeadingPosition(otherTitle, start + title.length);
  ...
  function assertStableRoleIdentity(row, activityLabel, activityHtml, gate) {
    ...
    Object.keys(indexMap).forEach(function (key) {
      ...
      if (!found) {
        var expected = expectedRegistryHeading(entry.role_family, row);
        var plainActivity = String(activityHtml || "")
          .replace(/<[^>]+>/g, " ")
          .replace(/\s+/g, " ")
          .toLowerCase();
        found = !!expected && plainActivity.indexOf(String(expected).toLowerCase()) !== -1;
      }
      if (!found) {
        gateFail(
          gate,
