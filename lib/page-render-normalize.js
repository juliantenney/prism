...
(function (root, factory) {
  ...
  function materialTypeToRenderKey(type) {
    ...
    if (t === "analysis_table" || t === "comparison_table" || t === "classification_table") {
      return "impact_table";
    }
    if (t === "tables" || t === "table") return "impact_table";
  ...
  function convertMaterialsArrayToObjectMap(materials) {
    ...
    materials.forEach(function (entry, index) {
      ...
      if (typeof entry === "string") {
        appendMaterialBucket(out, "text", entry);
        return;
      }
      ...
      var payload =
        entry.body != null
          ...
          ? entry.text
          : entry;
      var renderKey = typeKey;
      if (usedKeys[renderKey]) {
        renderKey = materialId ? slugFromMaterialId(materialId) : typeKey + "_" + (index + 1);
      }
      if (usedKeys[renderKey] && materialId) {
        renderKey = slugFromMaterialId(materialId);
      }
      ...
      appendMaterialBucket(out, renderKey, payload);
    });
    return out;
  }
