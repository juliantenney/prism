...
var mathEntrySpikeRuntime = require("./math-entry-spike-runtime");
...
function renderPage(model, renderOptions) {
  ...
  var hasMathEntrySpike = body.indexOf('data-input-modality="math"') >= 0;
  var mathEntrySpikeMarkup = hasMathEntrySpike
    ? mathEntrySpikeRuntime.getMathEntrySpikeHeadMarkup() +
      "<style>" +
      mathEntrySpikeRuntime.getMathEntrySpikePresentationCss() +
      "</style>" +
      "<script>" +
      mathEntrySpikeRuntime.getMathEntrySpikeRuntimeScript() +
      "</script>"
    : "";
