 ...
 * Does not duplicate: LD-MATERIALS-COPY, activity/material regeneration (owned upstream + assemble).
 */
(function (root, factory) {
  ...
  var CONTRACT_VERSION = "78-DP-PARTIAL-T047";
  ...
  function buildDesignPagePartialContractBlock() {
    return [
      ...
      "- page_synthesis.study_tips — TRANSPORT ONLY (S78-D04): when Copilot conversation / GAM materials contain a Markdown section headed \"### Page learner-resource closure\", copy that section's body verbatim into study_tips.body (do not paraphrase, expand, repair, or invent). Omit study_tips when that designated section is absent. Do not synthesise study_tips from activity ### Closure/### Debrief alone, knowledge_summary, or general inference. Do not invent final_synthesis or next_steps fields.",
      "",
      ...
      "",
      "S78-DP disciplinary warrant (Design Page):",
      "- page_synthesis (especially knowledge_summary) must not upgrade activity procedures into unrestricted disciplinary slogans.",
      "- For visual_affordances generate rows, allowed_claims, disallowed_claims, and canonical_discipline_note must encode taught model-class and claim-strength bounds (not filler).",
      "- Visuals, captions, alt text, detailed descriptions, and on-image labels must not introduce stronger conclusions or broader notation/representation classes than upstream taught examples and claims warrant.",
      "- Omit advanced theory freely; do not upgrade the strength of what remains.",
      "",
      "S78-VA synthesis commissioning (page-scoped / purpose:synthesis generate rows):",
      "- must_show and allowed_claims must name the taught entities, category membership, and relationships the figure is authorised to visualise — not vague phrases such as \"inputs and outputs around the system\" or \"important concepts\".",
      "- Preserve qualification, conditionality, and claim strength from upstream taught prose (especially page_synthesis.knowledge_summary).",
      "- When plausible disciplinary extensions would materially alter the taught model, list them explicitly in must_not_show and/or disallowed_claims.",
      "- Synthesis visuals integrate multiple AUTHORISED relationships from must_show / allowed_claims and evidence_anchors; they must not introduce peer categories, fluxes, boundary crossings, or causal links absent from that commission.",
      "- evidence_anchors for synthesis should include page_synthesis.knowledge_summary when that field is the authoritative consolidation prose.",
      "",
      ...
      "- FAIL-CLOSED: every visual_affordances[] row MUST include a non-empty rationale explaining why that visual is pedagogically/usefully warranted (not title/topic restatement, not a copy of alt_text/detailed_description/caption_intent). Capture rejects rows that omit rationale.",
      "- Every row required children: affordance_id; scope (activity|page); activity_id (activity scope) or region knowledge_summary (page scope); visual_decision (generate|defer|skip); rationale; subject; context; evidence_anchors[] (canonical activity_id.path or page_synthesis.field).",
      "- generate also requires: visual_slot, tier (essential|valuable), purpose, preferred_representation, reasoning_supported, learner_stage (pre_classification|post_reasoning), anti_spoiler (boolean), spoiler_boundary when anti_spoiler is true, representation_avoid[], canonical_discipline_note, requires_exact_data_match (boolean), must_show[], must_not_show[], allowed_claims[], disallowed_claims[], source_basis, caption_intent, discipline_risk_level (low|medium|high); author alt_text and detailed_description with caption_intent.",
      "- optional material_anchor: when present MUST be an upstream material_id for the material this visual REPRESENTS (not merely relevant context).",
      "- Material-role commissioning (Gate 1): do NOT claim to use or ground in the activity's scenario/capacity setting unless material_anchor identifies that material. Conceptual visuals omit material_anchor; worked visuals anchor the model/worked_example material; grounded visuals anchor the learner source/scenario material.",
      "- defer also requires defer_reason and MUST omit purpose, preferred_representation, visual_slot, and other generate-only fields. skip also requires skip_reason and the same omissions.",
      "- Enums, examples, and evidence_anchor forms: follow the appended Sprint 38 visual affordance authoring contract — do not invent a second schema.",
      "",
