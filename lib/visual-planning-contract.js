...
(function (root, factory) {
  ...
  function validateEvidenceAnchorSyntax(
    ...
    if (normalizeKey(activityId) === "materials") {
      target.push(
        makeIssue(
          "VPC_EVIDENCE_ANCHOR_MALFORMED",
          anchorPath +
            ": materials.* path fragments must be prefixed with activity_id (e.g. A1.materials.scenarios)",
          { path: anchorPath, index: rowIndex, affordance_id: affordanceId }
        )
      );
      return;
    }
    if (activityIdSet && !activityIdSet[normalizeKey(activityId)]) {
      target.push(
        makeIssue(
  ...
  function isCanonicalEvidenceAnchorSyntax(anchor) {
    ...
    var activityId = trimmed.slice(0, dot);
    var remainder = trimmed.slice(dot + 1);
    if (!isNonEmptyString(activityId) || !isNonEmptyString(remainder)) return false;
    if (/\s/.test(trimmed)) return false;
    // Path fragments like materials.scenarios are not canonical without an activity root.
    if (normalizeKey(activityId) === "materials") return false;
    return true;
  }
  ...
  function validateVisualPlanningCaptureShape(page) {
    ...
    if (affordances) {
      ...
      affordances.forEach(function (record, index) {
        ...
        var decision = resolvedDecision(record);
        if (decision === "generate") {
          validateGeneratePlanningFields(record, index, null, errors);
        } else {
          validateOptionalEvidenceAnchors(record, index, null, errors);
        }
      });
    }
    return {
