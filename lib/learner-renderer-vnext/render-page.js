...
var getRevisionCriterionRuntimeScript =
  require("./revision-criterion-runtime").getRevisionCriterionRuntimeScript;
var getRevisionReminderPresentationCss =
  require("./revision-criterion-accompaniment").getRevisionReminderPresentationCss;
var getLearnerDraftRuntimeScript =
  require("./learner-draft-runtime").getLearnerDraftRuntimeScript;
var mathEntryRuntime = require("./math-entry-runtime");
...
function renderOrientationRegion(model, visualOptions) {
  ...
  var outcomesHtml = renderLearningOutcomes(model.learningOutcomes, model.sessionFraming);
  if (outcomesHtml) innerParts.push(outcomesHtml);

  var progressionHtml = renderProgressionGuidance(
    model.progressionGuidance,
    model.sessionFraming
  );
...
/**
 * Render one Expository exposition section (title + transported explanation + materials + VA).
 * Does not fabricate activities/workspaces/evidence UI.
 *
 * Ordering:
 * - Ordinary / earlier sections: title → exposition → materials → section VA
 * - Final Expository section (EQ8 terminal close): title → materials → section VA → exposition
 *   so supporting artefacts participate before the single XD-owned consolidating close.
 *
 * @param {import("./types").ExpositionSection} section
 * @param {Object} visualOptions
 * @param {{ terminalClose?: boolean }=} sectionOptions
 * @returns {string}
 */
function renderExpositionSection(section, visualOptions, sectionOptions) {
  if (!section || typeof section !== "object") return "";
  var sectionId = String(section.id || "").trim();
  if (!sectionId) return "";
  var terminalClose = !!(sectionOptions && sectionOptions.terminalClose);
  var title = String(section.title || "").trim();
  var explanation = String(section.explanation || "").trim();
  var materialsHtml = html
    .arrayOrEmpty(section.materials)
    .map(function (material) {
      if (!material || typeof material !== "object") return "";
      var structuredHtml = expositoryStructured.renderExpositionStructuredMaterial(material);
      if (structuredHtml) return structuredHtml;
      var rendered = renderMaterial(material, {
        activityId: "",
        interactiveChecklist: false
      });
      if (!rendered) return "";
      // Stamp section ownership for placement assertions without altering material body.
      if (rendered.indexOf("data-section-id=") === -1) {
        rendered = rendered.replace(
          /^<article\b/,
          '<article data-section-id="' + html.escapeAttribute(sectionId) + '"'
        );
      }
      if (material.kind && rendered.indexOf("data-material-kind=") === -1) {
        rendered = rendered.replace(
          /^<article\b/,
          '<article data-material-kind="' +
            html.escapeAttribute(String(material.kind || "")) +
            '"'
        );
      }
      return rendered;
  ...
  var afterHook = section.visualAffordanceAfterContent
    ? renderVisualAffordance(section.visualAffordanceAfterContent, visualOptions)
    : "";

  var titleHtml = title ? learnerIcons.renderSectionHeading(title, "section.overview") : "";
  var explanationHtml = explanation
    ? '<div class="util-exposition-explanation util-prose-measure" data-exposition-field="explanation"' +
      (terminalClose ? ' data-expository-closure="terminal"' : "") +
      ">" +
      // wrapTables: host for Expository table breakout (+ narrow overflow fallback)
      (html.renderMarkdownBlock(explanation, { wrapTables: true }) ||
        html.renderPlainText(explanation)) +
      "</div>"
    : "";
  var materialsBlock = materialsHtml
    ? '<div class="util-exposition-materials" data-region="exposition-materials">' +
      materialsHtml +
      "</div>"
    : "";

  // EQ8: final substantive section owns the close — artefacts must not displace it.
  var bodyHtml = terminalClose
    ? titleHtml + materialsBlock + afterHook + explanationHtml
    : titleHtml + explanationHtml + materialsBlock + afterHook;

  return (
    '<section class="util-exposition-section" data-region-item="exposition-section" data-section-id="' +
    html.escapeAttribute(sectionId) +
    '"' +
    (terminalClose ? ' data-expository-section-role="terminal-close"' : "") +
    ">" +
    bodyHtml +
    "</section>"
  );
...
/**
 * Ordered exposition region — after orientation, before Interactive activities.
 *
 * @param {import("./types").LearnerPageModel} model
 * @param {Object} visualOptions
 * @returns {string}
 */
function renderExpositionRegion(model, visualOptions) {
  var sections = html.arrayOrEmpty(model.expositionSections);
  if (!sections.length) return "";
  var isExpositoryPage = String((model && model.pageKind) || "") === "expository";
  var terminalIndex = -1;
  if (isExpositoryPage) {
    for (var i = sections.length - 1; i >= 0; i--) {
      if (sections[i] && String(sections[i].id || "").trim()) {
        terminalIndex = i;
        break;
      }
    }
  ...
  var inner = sections
    .map(function (section, index) {
      return renderExpositionSection(section, visualOptions, {
        terminalClose: isExpositoryPage && index === terminalIndex
      });
    })
    .filter(Boolean)
    .join("");
  if (!inner) return "";
  return (
    '<section class="util-exposition-sections" data-region="exposition">' +
    inner +
    "</section>"
  );
}

function renderLearningOutcomes(outcomes, sessionFramingModel) {
  ...
  var framing =
    sessionFramingModel && sessionFramingModel.outcomesIntro
      ? sessionFramingModel
      : sessionFraming.SELF_STUDY_FRAMING;
  ...
  return (
    ...
    '<p>' + html.escapeHtml(framing.outcomesIntro) + "</p>" +
    "<ol>" +
    items +
    "</ol></div></section>"
  );
...
function renderProgressionGuidance(text, sessionFramingModel) {
  var guidance = String(text || "").trim();
  if (!guidance) return "";
  var framing =
    sessionFramingModel && sessionFramingModel.progressionHeading
      ? sessionFramingModel
      : sessionFraming.SELF_STUDY_FRAMING;
  return (
    '<section class="util-orientation-section util-progression-guidance" data-orientation-type="progression_guidance">' +
    learnerIcons.renderSectionHeading(framing.progressionHeading, "section.progression") +
...
/**
 * Expository resource closing — brief carry-away prose after exposition.
 * Distinct from Interactive study_tips; omit when empty.
 */
function renderClosingParagraph(closingParagraph) {
  if (!String(closingParagraph || "").trim()) return "";
  return (
    '<section class="util-page-closing" data-region="page-closing" aria-label="Closing">' +
    '<h2 class="util-section-heading">Closing</h2>' +
    '<div class="util-page-closing__content util-prose-measure">' +
    renderMarkdownRegion(closingParagraph) +
    "</div></section>"
  );
}
...
function renderPage(model, renderOptions) {
  ...
  // reading order (orientation → exposition → closing → activities → assessment).
  var headerHtml = renderHeader(model);
  var orientationHtml = renderOrientationRegion(model, visualAssetOptions);
  var expositionHtml = renderExpositionRegion(model, visualAssetOptions);
  var closingHtml = renderClosingParagraph(model.closingParagraph);
  ...
  var revisionCriterionScript =
    body.indexOf("data-revise-with-criterion") >= 0
      ? "<script>" + getRevisionCriterionRuntimeScript() + "</script>"
      : "";
  var revisionReminderStyle =
    body.indexOf("data-revision-reminder") >= 0
      ? "<style id=\"prism-revision-reminder-css\">" +
        getRevisionReminderPresentationCss() +
        "</style>"
      : "";
  ...
  var hasMathEntry = body.indexOf('data-input-modality="math"') >= 0;
  var mathEntryMarkup = hasMathEntry
    ? mathEntryRuntime.getMathEntryHeadMarkup() +
      "<style>" +
      mathEntryRuntime.getMathEntryPresentationCss() +
      "</style>" +
      "<script>" +
      mathEntryRuntime.getMathEntryRuntimeScript() +
      "</script>"
    : "";
  ...
  var pageKind = String((model && model.pageKind) || "").trim();
  var isExpositoryPage = pageKind === "expository";
  var pageKindAttrs = isExpositoryPage
    ? ' data-page-kind="expository"'
    : pageKind
      ? ' data-page-kind="' + html.escapeAttribute(pageKind) + '"'
      : "";
  var pageKindClass = isExpositoryPage ? " util-page--expository" : "";

  return (
    '<main class="util-learner-page util-page util-learner-renderer-vnext' +
    pageKindClass +
    '" data-renderer="vnext"' +
    pageKindAttrs +
    persistenceAttrs +
    ...
    assessmentScript +
    guidedReviewScript +
    revisionReminderStyle +
    revisionCriterionScript +
    mathEntryMarkup +
    draftScript +
    "</main>"
  );
