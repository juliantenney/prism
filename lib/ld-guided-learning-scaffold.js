 ...
 * DLA generation + Design Page assembly preservation. No new fields or page structures.
 */
(function (root, factory) {
  ...
  function () {
    ...
    var FIELD_WORD_RANGES = {
      ...
      // Soft substantive floor only — hard DLA validation requires non-empty, not a 30-word floor.
      intellectual_coherence_bridge: { min: 8, max: 60 },
    ...
    var BRIDGE_ACTIVITY_NUMBER_RE =
      /\bactivity\s+\d+\s+(follows|comes after|after)\s+activity\s+\d+\b/i;
    var BRIDGE_GENERIC_LO_RE =
      /connect this activity'?s? conclusion to the broader learning goal/i;
    ...
    var CORE_LINES = [
      ...
      "- Design Page assembly: copy these fields verbatim — never shorten, label-ify, or paraphrase into metadata."
    ];

    var COMPACT_DLA_FIELD_LINES = [
      "- AUTHORITATIVE WORD RANGES (no other prompt block may restate):",
      "  activity_preamble 50–120; reasoning_orientation, self_explanation_prompt, conceptual_contrast_prompt, argument_structure_hint, transfer_or_application_task 35–80; expected_output 30–70; intellectual_coherence_bridge prefer 1–2 concise sentences (substantive, not scheduling-only); support_note 20–70 when used.",
      "- SCAFFOLD GENRE: scaffold strings = learner-facing page copy (not metadata). Cognition = three substantive sentences — peer learns the reasoning move; not stems or cues. expected_output = quality threshold. intellectual_coherence_bridge (mandatory every activity including A1) = cumulative cognitive sequence — distinct from activity_preamble.",
      "- FORBIDDEN on scaffold fields: procedural openers (Begin by…, In this activity you will…); arrow-only chains; one-line stems (Explain your answer); deliverable labels without quality criteria; scheduling-only or activity-number-only bridges; bridges that duplicate the preamble."
    ];
    ...
    var FIELD_LINES = [
      ...
      "- intellectual_coherence_bridge (mandatory on every activity including A1; prefer 1–2 concise sentences):",
      "  • A1: connect page orientation, enquiry, learning purpose, prior knowledge or knowledge summary to the first activity — do not refer to a nonexistent preceding activity.",
      "  • A2+: identify carried learning, reasoning, evidence, learner production or capability from earlier work and how this activity develops or uses it.",
      "  • Distinct from activity_preamble. FORBIDDEN: Activity 2 follows Activity 1; scheduling-only text; duplicating the preamble."
    ];
    ...
    var MANDATORY_LINES = [
      ...
      "  • intellectual_coherence_bridge (mandatory every activity including A1): concise cumulative-sequence prose — A1 from orientation/purpose; A2+ from prior carried learning — not connective metadata and not a preamble duplicate.",
      "  • Do not emit procedural-only rows (title, learner_task, expected_output, required_materials)."
    ];

    var COMPOSE_LINES = [
      "- ASSEMBLY PRESERVATION (Design Page):",
    ...
    var TRANSITION_LINES = [
      ...
      "  • Concise conceptual continuity on every activity row: what was established (or page orientation for A1), why this stage follows, what reasoning move the learner makes.",
    ...
    var DLA_PRE_EMIT_LINES = [
      ...
      "  2. Mandatory coverage: every activity has activity_preamble, expected_output, at least one cognition scaffold field, and intellectual_coherence_bridge (including A1).",
    ...
    function buildLdGuidedLearningScaffoldPromptBlock(options) {
      ...
      if (composeOnly && includeCompose && !includeDlaPreEmit) {
        var composeOnlyLines = [
          ...
          "- Module: " +
            MODULE_ID +
            " | Design Page assembly preservation — copy upstream scaffold fields verbatim; do not generate, rewrite, or compress to metadata labels.",
          "- Scope: activity-row scaffold fields only. Obey LD-DESIGN-PAGE-PARTIAL-CONTRACT field preservation boundaries."
        ];
        if (!includeMarker) {
      ...
      if (includeDlaPreEmit && !includeCompose) {
        coreLines = coreLines.filter(function (line) {
          return line.indexOf("Design Page assembly:") !== 0;
        });
      }
    ...
    function activityTopicContext(row) {
      ...
      var purpose = normalizeFieldText(row && (row.purpose || row.activity_purpose));
      var reasoning = normalizeFieldText(row && row.reasoning_orientation);
      var blob = (
        title +
        " " +
        task +
    ...
    function buildRepairContext(activities, options) {
      ...
      var sequence =
        opts.learningSequence ||
        opts.learning_sequence ||
        (opts.page && opts.page.learning_sequence) ||
        null;
      var timeline =
        sequence && Array.isArray(sequence.timeline) ? sequence.timeline : [];
      timeline.forEach(function (entry) {
        if (!entry || typeof entry !== "object") return;
        var aid = String(entry.activity_id || entry.id || "").trim();
        var purpose = normalizeFieldText(entry.purpose);
        if (aid && purpose) ctx.timelinePurposeByActivityId[aid] = purpose;
      });
      var page = opts.page && typeof opts.page === "object" ? opts.page : null;
      var synthesis =
        (page && page.page_synthesis) ||
        opts.page_synthesis ||
        opts.pageSynthesis ||
        null;
      function slotText(slot) {
        if (!slot) return "";
        if (typeof slot === "string") return normalizeFieldText(slot);
        if (typeof slot === "object") return normalizeFieldText(slot.body || slot.text || "");
        return "";
      }
      if (synthesis && typeof synthesis === "object") {
        ctx.pageOrientation = slotText(synthesis.overview);
        ctx.learningPurpose = slotText(synthesis.learning_purpose);
        ctx.knowledgeSummary = slotText(synthesis.knowledge_summary);
      }
      if (!ctx.pageOrientation) {
        ctx.pageOrientation = normalizeFieldText(
          opts.pageOrientation || opts.overview || ""
        );
      }
      if (!ctx.learningPurpose) {
        ctx.learningPurpose = normalizeFieldText(
          opts.learningPurpose || opts.learning_purpose || ""
        );
      }
      if (!ctx.knowledgeSummary) {
        ctx.knowledgeSummary = normalizeFieldText(
          opts.knowledgeSummary || opts.knowledge_summary || ""
        );
      }
      if (Array.isArray(activities)) {
        activities.forEach(function (row) {
          ...
          if (aid && !normalizeFieldText(row.purpose) && ctx.timelinePurposeByActivityId[aid]) {
            row.purpose = ctx.timelinePurposeByActivityId[aid];
          }
        });
      }
      return ctx;
    ...
    function extractTopicPhrase(row, ctx) {
      ...
      var aid = String((row && row.activity_id) || (row && row.id) || "").trim();
      if (aid && ctx.timelinePurposeByActivityId && ctx.timelinePurposeByActivityId[aid]) {
        return ctx.timelinePurposeByActivityId[aid].split(/[.!?]/)[0].slice(0, 120);
      }
      if (topicCtx.purpose) return topicCtx.purpose.split(/[.!?]/)[0].slice(0, 120);
      if (topicCtx.preamble && wordCount(topicCtx.preamble) >= 8) {
        return topicCtx.preamble.split(/[.!?]/)[0].slice(0, 120);
      }
      if (topicCtx.reasoning && wordCount(topicCtx.reasoning) >= 8) {
        return topicCtx.reasoning.split(/[.!?]/)[0].slice(0, 120);
      }
    ...
    function extractReasoningMove(row, ctx) {
      ...
      if (topicCtx.purpose) return topicCtx.purpose.split(/[.!?]/)[0].slice(0, 120);
      if (topicCtx.preamble && wordCount(topicCtx.preamble) >= 12) {
        return topicCtx.preamble.split(/[.!?]/)[0].slice(0, 120);
      }
      if (topicCtx.reasoning && wordCount(topicCtx.reasoning) >= 12) {
        return topicCtx.reasoning.split(/[.!?]/)[0].slice(0, 120);
      }
      if (topicCtx.task) return topicCtx.task.split(/\n/)[0].slice(0, 100);
      var lo = resolveLoStatement(row, ctx);
    ...
    function bridgeLooksSchedulingOnly(text) {
      ...
      if (BRIDGE_ACTIVITY_NUMBER_RE.test(raw)) return true;
      return false;
    }

    function bridgeDuplicatesPreamble(bridge, preamble) {
      var a = normalizeFieldText(bridge).toLowerCase().replace(/\s+/g, " ");
      var b = normalizeFieldText(preamble).toLowerCase().replace(/\s+/g, " ");
      return !!(a && b && a === b);
    }

    function bridgeLooksGenericWithoutCarriedLearning(text) {
      var raw = normalizeFieldText(text);
      if (!raw) return false;
      if (BRIDGE_GENERIC_LO_RE.test(raw)) return true;
      if (
        /^building on the prior activity\b/i.test(raw) &&
        wordCount(raw) < 12
      ) {
        return true;
      }
    ...
    function bridgeRefersToPrecedingActivity(text) {
      var raw = normalizeFieldText(text);
      if (!raw) return false;
      return /\b(previous activity|prior activity|preceding activity|activity\s+\d+|earlier activity)\b/i.test(
        raw
      );
    }

    function bridgeFailsSoftQuality(text, row, index) {
      var raw = normalizeFieldText(text);
      if (!raw) return true;
      if (wordCount(raw) < FIELD_WORD_RANGES.intellectual_coherence_bridge.min) return true;
      if (bridgeLooksSchedulingOnly(raw)) return true;
      if (bridgeLooksGenericWithoutCarriedLearning(raw)) return true;
      if (row && bridgeDuplicatesPreamble(raw, row.activity_preamble)) return true;
      if (index === 0 && bridgeRefersToPrecedingActivity(raw)) return true;
      if (scaffoldLooksLikeTerseLabel(raw) && wordCount(raw) < 12) return true;
      return false;
    }

    function fieldFailsScaffoldQuality(fieldId, text) {
      if (!fieldHasValue(text)) return false;
      if (fieldId === "intellectual_coherence_bridge") {
        return bridgeFailsSoftQuality(text, null, -1);
      }
    ...
    function collectRowScaffoldIssues(row, index, total, ctx) {
      ...
      if (bridgeFailsSoftQuality(row.intellectual_coherence_bridge, row, index)) {
    ...
    function extractPageOrientationCue(row, ctx) {
      var repairCtx = ctx || buildRepairContext([], {});
      var fromRow =
        normalizeFieldText(row && row.intellectual_frame) ||
        normalizeFieldText(row && row.study_orientation) ||
        normalizeFieldText(row && row.prior_knowledge_activation);
      if (fromRow) return fromRow.split(/[.!?]/)[0].slice(0, 120);
      if (repairCtx.learningPurpose) {
        return repairCtx.learningPurpose.split(/[.!?]/)[0].slice(0, 120);
      }
      if (repairCtx.pageOrientation) {
        return repairCtx.pageOrientation.split(/[.!?]/)[0].slice(0, 120);
      }
      if (repairCtx.knowledgeSummary) {
        return repairCtx.knowledgeSummary.split(/[.!?]/)[0].slice(0, 120);
      }
      if (repairCtx.workflowGoal) {
        return repairCtx.workflowGoal.split(/[.!?]/)[0].slice(0, 120);
      }
      var lo = resolveLoStatement(row, repairCtx);
      if (lo) return lo.split(/[.!?]/)[0].slice(0, 100);
      return "the page learning purpose";
    }
    ...
    function expandIntellectualCoherenceBridge(row, priorRow, ctx) {
      ...
      var index = priorRow ? 1 : 0;

      if (existing && !bridgeFailsSoftQuality(existing, row, index)) {
        return trimToWordMax(existing, range.max);
      }
      ...
      if (
        ...
        !bridgeDuplicatesPreamble(existing, row && row.activity_preamble) &&
        !(index === 0 && bridgeRefersToPrecedingActivity(existing)) &&
        !bridgeLooksGenericWithoutCarriedLearning(existing) &&
        wordCount(existing) > 0 &&
        wordCount(existing) < range.min
      ) {
        return expandInPlace(existing, range.min, range.max, [
          priorRow
            ? "Carry that reasoning into " + extractTopicPhrase(row, repairCtx) + "."
            : "Use that foundation as you begin " + extractTopicPhrase(row, repairCtx) + "."
        ]);
      }
      ...
      if (!priorRow) {
        var orientation = extractPageOrientationCue(row, repairCtx);
        synthesized =
          "The overview introduced " +
          orientation.charAt(0).toLowerCase() +
          orientation.slice(1) +
          ". This first activity begins by using that foundation to " +
          currentTopic.charAt(0).toLowerCase() +
          currentTopic.slice(1) +
          (materialCue ? " through " + materialCue.toLowerCase() : "") +
          ".";
      } else {
        var priorMove = extractReasoningMove(priorRow, repairCtx);
        synthesized = priorMove
          ? "You established " +
            priorMove.charAt(0).toLowerCase() +
            priorMove.slice(1) +
            ". This activity develops that capability by focusing on " +
            currentTopic.charAt(0).toLowerCase() +
            currentTopic.slice(1) +
            (materialCue ? " through " + materialCue.toLowerCase() : "") +
            "."
          : "Carry forward the reasoning and evidence from earlier work as you focus on " +
            currentTopic.charAt(0).toLowerCase() +
            currentTopic.slice(1) +
      ...
      if (bridgeDuplicatesPreamble(synthesized, row && row.activity_preamble)) {
        synthesized =
          (priorRow
            ? "Build on the capability developed earlier rather than restarting from scratch. "
            : "Connect the page orientation to this opening move. ") +
          "Focus now on " +
          currentTopic.charAt(0).toLowerCase() +
          currentTopic.slice(1) +
          ".";
    ...
    return {
      ...
      bridgeDuplicatesPreamble: bridgeDuplicatesPreamble,
      bridgeFailsSoftQuality: bridgeFailsSoftQuality,
