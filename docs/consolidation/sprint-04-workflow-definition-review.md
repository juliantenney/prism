...
- dual prompt-source fields remain (`prompt_source` and `prompt_source_type`) for compatibility
...
- `validateWorkflow(wf)` is a pure warning evaluator over a workflow-definition snapshot.
- It checks only:
...
- Lifecycle currently observed:
  - normalize first, validate second on import/load-oriented flows (`normalizeWorkflowForV1` before `validateWorkflow`)
  - save/edit/render flows validate already-canonicalized snapshot shapes produced by gather/save/load paths
  - warning surfacing ownership is caller-specific (`renderWorkflowValidationWarnings`, toasts, import summary text)
...
- **UI completeness**
  - missing prompt/output conditions are surfaced for editor guidance, but enforced nowhere.
