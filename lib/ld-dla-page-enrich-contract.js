...
(function (root, factory) {
  ...
  var LEGACY_CONTRACT_VERSION = "76-DLA-PARTIAL-9";
  var CONTRACT_VERSION = "77-DLA-CANONICAL-3";
  ...
  /**
   * Canonical assembler (Sprint 77). Live when workflowOutputSpec.dlaCanonicalAssembler
   * is true or missing. Legacy builders remain for rollback (flag false).
   */
  ...
  function buildDlaSectionOutput(ctx) {
    ...
    var parts = [
      ...
      "- when evidence_requirement is present it MUST include: kind (literal \"learner_evidence\"), purpose (non-empty string), learner_action (non-empty string), observable_features (non-empty string array)",
  ...
  function buildDlaWorkbookOverlayBlock() {
    return [
      "Domain / workbook overlay (subordinate). Apply when delivery_context is self_directed AND the brief implies a ~60-minute learner workbook or learner page. Do not replace learner production, task-input, commissioning, or evidence decisions in §§4–8. Types/purposes only — GAM authors bodies.",
      "",
      ...
      "- G1 Verification (DLA-WB-26): every activity MUST list type checklist with purpose verification/self-check/rubric; depth_floor L3: ≥4 criteria-linked check items + repair/revise if any fail; table completion alone does NOT satisfy verification.",
      "- G2 Transfer → transfer_prompt row when Transfer Required.",
      ...
      "- DLA-WB-01: resource_intent self_study_workbook, session_duration_target_minutes (~60), consolidation_requirement, workbook_contract_applied: true (optional generation_notes only if already valid on the page schema; do not invent new top-level output keys).",
