 ...
 * Operationalises Pattern Library SP-02, SP-03, SP-04, and SP-06 for GAM material authoring.
 */
(function (root, factory) {
  ...
  function buildInstructionalPatternPromptBlock(options) {
    var opts = options && typeof options === "object" ? options : {};
    var parts = [];
    if (!opts.sp03Only && !opts.sp06Only && !opts.sp04Only) {
      parts.push(buildSp02PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp06Only && !opts.sp04Only) {
      parts.push(buildSp03PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp03Only && !opts.sp04Only) {
      parts.push(buildSp06PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp03Only && !opts.sp06Only) {
