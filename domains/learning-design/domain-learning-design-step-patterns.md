...
```json
{
  ...
  "defaultPromptNotes": "Generate complete, usable activity materials directly from required_material specifications; when delivery_context is self_directed, ensure all materials are self-contained and do not depend on live tutor clarification. When specification requests worked_example or sample_output, show reasoning moves (why each step), not answer-only tables. When specification requests a faded template, pre-fill only the cells/rows named in the specification and leave learner response cells empty. When specification requests prompt_set self-check or misconception-interrupt bullets, realise concise If X revisit Y cues — not answer keys or tutoring narration. When specification requests step → meaning or concept/procedure integration, realise labelled bullets or column hints (procedure → conceptual purpose; Use this when… where learners choose a method). When specification requests closure, debrief, or evaluative judgement, realise ### Closure or ### Debrief with 2–3 concise bullets (not diary prompts). For display-oriented learner-facing material text, use only a limited Markdown subset: ##/### headings, - bullet lists, 1. numbered lists, **bold**, pipe tables, and --- horizontal rules. When maths is needed in learner-facing material text, use only \\(...\\) inline and \\[...\\] block delimiters; never $...$/$$...$$, code wrappers, or HTML-escaped delimiters. When learner_task asks learners to arrange/order/sequence events chronologically, realise event-list text materials in deliberately mixed non-chronological order; template/table row order is not the answer key. Chronological event lists are allowed only when the task is interpretation/significance, not re-ordering.",
  "runnerInstructions": {
...
```json
{
  ...
  "defaultPromptStrategy": "default_template",
  "preferredOutputFormat": "json",
  "defaultPromptNotes": "Assemble one profile-aware readable and self-contained page from upstream artefacts with strict fidelity to explicit components, quantities, and exclusions; enforce strict JSON contract: sections must always be an array of section objects, each section object must include these required keys (section_id, heading, content), and do not emit alternate wrapper shapes; canonical section_id order (when applicable): overview, learning_purpose, knowledge_summary, learning_activities, assessment_check, support_notes; for page_profile learner, sections overview and learning_purpose must establish session intellectual journey (topic, stakes, one why-this-is-hard tension, what the learner will do intellectually, how activities build) — not topic-coverage summaries or component inventories only; when upstream learning_activities or learning_content include orientation prose, compose overview/learning_purpose from that substance rather than thinning to coverage blurbs; name one primary difficulty type in overview or learning_purpose when the page involves interpretation, comparison, misconception repair, or evidence judgement (conceptual difficulty, procedural difficulty, interpretive ambiguity, disciplinary uncertainty, competing explanations, or plausible misconception) using discipline-specific discriminating moves—not generic struggle or reflective uncertainty prompts; for multi-activity learner pages include intellectual_coherence_bridge on each activity after the first naming the carried reasoning move and escalation (less scaffolding, harder judgement, or new context), and state the session arc in overview or learning_purpose; for single-activity pages with multiple material blocks, name within-activity progression in overview and align learner_task order to that arc; include study_tips (or epistemic closure bullets in support_notes when study_tips is omitted) that synthesise the session: at least one bullet stating what should now be clearer or what distinction can be sustained, tied to the page tension and arc—not procedural completion, generic reflection, or detached revision-only tips; within study_tips include at most one durable-transfer bullet naming the session move and one adjacent context and optional at most one limit-of-transfer bullet where reasoning fails (mechanism vs policy, procedure vs interpretation, assumption breaks elsewhere — domain §6k); preserve upstream transfer_or_application_task on final activities; if knowledge_model or learning_content exists, include knowledge_summary; learning_activities.content must always be an array of activity objects with predictable keys (activity_id, title, timing/duration_minutes, purpose, activity_preamble, prior_knowledge_activation, reasoning_orientation, self_explanation_prompt, transfer_or_application_task, scaffold_hint_sequence, uncertainty_tension_prompt, learner_task, learner_instructions, materials, expected_output, support_note); each activity.materials must be a structured object containing usable learner-facing content (not a bullet list string or markdown blob), and markdown content must remain in named material fields; do not flatten materials into label+markdown strings and do not place raw markdown tables inside bullet-list strings; examples of valid material fields include task_cards (array), scenarios (array), checklist (bullet list), worksheet (markdown table or worksheet lines), prompts, and examples; do not output materials as labels-only arrays such as [\"Task Cards\", \"Checklist\"]; prefer object form such as {\"task_cards\":[...],\"checklist\":[...]}; assessment_check.content must be an object with items array; when assessment_items are provided, include all provided items by default and do not sample/omit/compress unless an explicit page constraint requires reduction; preserve item_id, stem, options, and any available answer/explanation fields; if feedback_display is answer_grid_end or answers_explanations and answer-bearing data is available, preserve those answer-bearing fields in page JSON for renderer use; source_artefacts must be object booleans; internal enum values (for example selected_response_only) must not appear in learner-facing content; for learning_activities.content[].materials, copy learner-facing material content verbatim or near-verbatim from upstream activity_materials and preserve full delivery structure (including scenarios, observation sets, background information, checklist sections, prompt labels, response options, templates, and sequenced records); treat scenario/checklist/template/prompt_set content as delivery materials, not source notes; design-page assembly may reorganise and relabel, but must not rewrite into abstracts or summary-only prose; if content is long, preserve structure and shorten only clearly non-essential prose while retaining all learner-actionable blocks; preserve supported math delimiters when present (\\(...\\) inline, \\[...\\] block); do not convert maths to $...$/$$...$$, code wrappers, or HTML-escaped delimiters; VISUAL AFFORDANCES (Sprint 38 — required at page root): every row needs affordance_id and activity_id; generate rows need visual_slot and full 38-4 field set with strict JSON types (anti_spoiler boolean not prose; tier essential|valuable never core; representation_avoid allow-listed tokens only; source_basis string not array); defer/reject rows omit purpose, preferred_representation, visual_slot; always visual_affordance_schema_version 38.4 plus activities_visual_review[] and visual_affordances[] (empty arrays valid); visual_decision must be generate, defer, or reject; for generate include purpose (distinction|comparison|classification|mechanism|evidence_structure|data_pattern_reading), preferred_representation (comparison_framework|classification_matrix|causal_model|evidence_t_chart|number_line_segments|ordered_bar_strip|labelled_contrast_panel), reasoning_supported, representation_avoid (array, at least one token), anti_spoiler (boolean), spoiler_boundary when anti_spoiler is true, must_show, must_not_show, allowed_claims, disallowed_claims, source_basis, caption_intent, requires_exact_data_match, discipline_risk_level (low|medium|high), tier (essential|valuable), learner_stage (pre_classification|post_reasoning), visual_slot when a figure is intended (activity-after-header|materials-entry|materials-card-grid-after|materials-table-pair-between|assessment-before-checkpoint); number_line_segments requires requires_exact_data_match true; defer rows require defer_reason (worked_example_sufficient_first|model_row_sufficient_first) and must omit purpose/preferred_representation/visual_slot; reject rows require rejection_reason (low_pedagogical_value|debrief_without_new_reasoning|duplicate_existing_structure|decorative_only|spoiler_risk|assessment_text_sufficient|insufficient_source_basis) and must omit purpose/preferred_representation/visual_slot; never emit topic-only affordances, never use a topic field, never justify generate with this topic can be illustrated or decorative topic posters; debrief-only or low-value activities should use reject with activity_visual_value none where appropriate; ACTIVITY MEMBERSHIP (hard): when upstream learning_activities are provided, every upstream activity_id must appear in page.sections[].learning_activities.content unless explicitly omitted; learning_sequence controls order/timing only (not membership); activity_materials enrich only (do not define membership); representative subsets are forbidden unless the user explicitly requests a subset; each intentional omission must be recorded in generation_notes.activities_omitted[] with activity_id, title, reason, authority (user_constraint | duration_constraint | explicit_exclusion) and summarized in generation_notes.limitations; before return validate (U \\ X) ⊆ C where U = upstream activity ids, C = composed ids in sections[], X = traced exclusions; omit unsupported sections and record unmet hard constraints in generation_notes.limitations.",
  "runnerInstructions": {
