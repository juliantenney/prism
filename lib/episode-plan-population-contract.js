...
(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    root.PRISM_EPISODE_PLAN_POPULATION = factory(root.PRISM_EPISODE_PLAN_V1_VOCABULARY);
  }
})(typeof globalThis !== "undefined" ? globalThis : this, function (vocabularyMod) {
  var CONTRACT_VERSION = "38S-2";

  if (!vocabularyMod || !vocabularyMod.FUNCTION_ENUM) {
  ...
  var FUNCTION_SPECS = {
    ...
    verification: {
      classes: ["material"],
      pedagogical_spec:
        "Diagnostic guided review: 3–4 quality dimensions (max 5) with observable response features and criterion-specific repair path.",
  ...
  function normalizeFunction(fn) {
    return vocabularyMod.normalizeFunctionKey
      ? vocabularyMod.normalizeFunctionKey(fn)
      : String(fn || "")
          .trim()
          .toLowerCase();
  }

  function isApprovedBeatFunction(fn) {
    if (vocabularyMod && typeof vocabularyMod.isApprovedFunction === "function") {
      return vocabularyMod.isApprovedFunction(fn);
    }
    return !!FUNCTION_SPECS[normalizeFunction(fn)];
  }
  ...
  // Pedagogical FUNCTION_SPECS must stay aligned with shared FunctionEnum membership.
  (function assertFunctionSpecsAlignWithVocabulary() {
    var missing = [];
    var extra = [];
    vocabularyMod.FUNCTION_ENUM.forEach(function (id) {
      if (!FUNCTION_SPECS[id]) missing.push(id);
    });
    Object.keys(FUNCTION_SPECS).forEach(function (id) {
      if (!vocabularyMod.FUNCTION_ENUM_SET[id]) extra.push(id);
    });
    if (missing.length || extra.length) {
      throw new Error(
        "FUNCTION_SPECS/FunctionEnum drift: missing=[" +
          missing.join(",") +
          "] extra=[" +
          extra.join(",") +
          "]"
  ...
  function assertPlanBeforePopulateGate(episodePlan) {
    ...
    beats.forEach(function (b) {
      ...
      else if (!isApprovedBeatFunction(b.function)) {
  ...
  function detectAcViolations(episodePlan, activity) {
    ...
    mats.forEach(function (m) {
      if (m.instructional_function === "verification" && m.type === "checklist") {
        if (!/rubric|repair|dimension|diagnostic|feature|criterion|criteria/i.test(m.specification)) {
          violations.push({
            rule: "AC-02",
            message:
              "verification checklist without diagnostic dimension/feature/repair spec (material " +
              m.material_id +
              ")"
          });
        }
  ...
  return {
    ...
    /** @deprecated Prefer episode-plan-v1-vocabulary; re-exported for callers. */
    FUNCTION_ENUM: vocabularyMod.FUNCTION_ENUM,
    isApprovedFunction: isApprovedBeatFunction,
