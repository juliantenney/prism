...
(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    root.PRISM_IMAGE_BRIEF_COMPILER = factory(
      planner,
      contract,
      houseVisual,
      precisionFidelity,
  ...
  if (
    ...
    throw new Error("prism-image-brief-compiler requires prism-house-visual-language");
  }
  if (
    !precisionFidelityMod ||
    typeof precisionFidelityMod.collectPrecisionCriticalClaims !== "function"
  ) {
    throw new Error("prism-image-brief-compiler requires prism-image-precision-fidelity");
  }
  ...
  function compileSourceEvidence(resolvedSources) {
    ...
    return list.map(function (src) {
      ...
      if (isNonEmptyString(src.material_id)) entry.material_id = String(src.material_id).trim();
      if (isNonEmptyString(src.material_type)) entry.material_type = String(src.material_type).trim();
      if (isNonEmptyString(src.evidence_role)) entry.evidence_role = String(src.evidence_role).trim();
      return entry;
    });
  }
        ...
        var text =
          typeof src.content_text === "string"
            ...
            : typeof src.content === "string"
              ? src.content
              : "";
        var clipped = precisionFidelityMod.clipEvidenceText(text, 240);
  ...
  function buildNodeLabelMap(nodes) {
    ...
    (Array.isArray(nodes) ? nodes : []).forEach(function (node, index) {
      if (typeof node === "string" || typeof node === "number") {
        var scalar = edgeText(node);
        if (scalar) map[scalar] = scalar;
        return;
      }
      ...
      var label =
        ...
        edgeText(node.title) ||
        edgeText(node.name) ||
        edgeText(node.heading) ||
        "Concept " + (index + 1);
      var id = edgeText(node.id);
      if (id) map[id] = label;
      map[label] = label;
    });
    return map;
  ...
  function resolveEdgeEndpoint(value, labelMap) {
    ...
    if (isPlainObject(value)) {
      return (
        edgeText(value.label) ||
        edgeText(value.title) ||
        edgeText(value.name) ||
        (labelMap && labelMap[edgeText(value.id)]) ||
        edgeText(value.id) ||
        ""
      );
    }
    var raw = edgeText(value);
    if (!raw) return "";
  ...
  /**
   * Extract authorised from→to edges from a structured diagram/graph body.
   * Resolves ids to labels; never invents edges by parallel-list index order.
   */
  function extractEdgesFromStructuredBody(body) {
    if (!isPlainObject(body)) return [];
    var nodes =
      (Array.isArray(body.elements) && body.elements) ||
      (Array.isArray(body.nodes) && body.nodes) ||
      (Array.isArray(body.vertices) && body.vertices) ||
      [];
    var edges =
      (Array.isArray(body.relationships) && body.relationships) ||
      (Array.isArray(body.edges) && body.edges) ||
      (Array.isArray(body.links) && body.links) ||
      (Array.isArray(body.arcs) && body.arcs) ||
      [];
    if (!edges.length) return [];
    var labelMap = buildNodeLabelMap(nodes);
    var out = [];
    edges.forEach(function (edge) {
      if (!isPlainObject(edge)) return;
      var from = resolveEdgeEndpoint(
        edge.from != null ? edge.from : edge.source,
        labelMap
      );
      var to = resolveEdgeEndpoint(edge.to != null ? edge.to : edge.target, labelMap);
      if (!from || !to) return;
      var meaning =
        edgeText(edge.label) ||
        edgeText(edge.meaning) ||
        edgeText(edge.relation) ||
        edgeText(edge.description) ||
        "";
      out.push({
        from: from,
        to: to,
        meaning: meaning,
        line: meaning ? from + " → " + to + " (" + meaning + ")" : from + " → " + to
      });
    });
    return out;
  }

  function extractEdgesFromArrowPhrases(items) {
    var out = [];
    (Array.isArray(items) ? items : []).forEach(function (item) {
      var raw = edgeText(item);
      if (!raw) return;
      var match = raw.match(/^(.+?)\s*(?:→|->|⇒)\s*(.+)$/);
      if (!match) return;
      var from = edgeText(match[1]);
      var to = edgeText(match[2]).replace(/\s*[\(:].*$/, "").trim();
      if (!from || !to) return;
      out.push({ from: from, to: to, meaning: "", line: from + " → " + to });
    });
    return out;
  }

  /**
   * Collect authorised semantic connector edges for image briefs.
   * Prefer structured relationships/edges; also honour explicit A → B phrases in must_show.
   */
  function extractAuthorisedSemanticEdges(sourceEvidence, mustShowItems) {
    var edges = [];
    var seen = Object.create(null);
    function pushAll(list) {
      (list || []).forEach(function (edge) {
        if (!edge || !edge.line || seen[edge.line]) return;
        seen[edge.line] = true;
        edges.push(edge);
      });
    }
    (Array.isArray(sourceEvidence) ? sourceEvidence : []).forEach(function (src) {
      var structured = src && src.content_structured;
      if (!structured && src && typeof src.content_text === "string") {
        try {
          var parsed = JSON.parse(src.content_text);
          if (isPlainObject(parsed) || Array.isArray(parsed)) structured = parsed;
        } catch (_err) {
          structured = null;
        }
      }
      if (isPlainObject(structured)) {
        pushAll(extractEdgesFromStructuredBody(structured));
        if (isPlainObject(structured.body)) {
          pushAll(extractEdgesFromStructuredBody(structured.body));
        }
      }
    });
    pushAll(extractEdgesFromArrowPhrases(mustShowItems));
    return edges;
  }

  function formatAuthorisedSemanticEdgeLines(edges) {
    return (Array.isArray(edges) ? edges : [])
      .map(function (edge) {
        return edge && edge.line ? String(edge.line) : "";
      })
      .filter(Boolean);
  }

  var AUTHORISED_CONNECTOR_RULE =
    "Connect ONLY the authorised from→to pairs listed below. Do not invent connectors from spatial adjacency, column/row order, parallel entity lists, or index pairing.";

  function formatAuthorisedConnectorsBlock(sourceEvidence, mustShowItems) {
    var edges = extractAuthorisedSemanticEdges(sourceEvidence, mustShowItems);
    var lines = formatAuthorisedSemanticEdgeLines(edges);
    if (!lines.length) return "";
    return (
      "- " +
      AUTHORISED_CONNECTOR_RULE +
      "\n" +
      lines
        .map(function (line) {
          return "- " + line;
  ...
  function collectJobPrecisionClaims(job, sourceEvidence) {
    var additional = [];
    asStringArray(job && job.allowed_claims).forEach(function (item) {
      additional.push(String(item));
    });
    if (job && job.subject) additional.push(String(job.subject));
    if (job && job.context) additional.push(String(job.context));
    return precisionFidelityMod.collectPrecisionCriticalClaims({
      sourceEvidence: sourceEvidence,
      mustShowItems: asStringArray(job && job.must_show),
      additionalTexts: additional
    });
  }

  var CLAIM_STRENGTH_PRESERVATION_LINE =
    "Do not strengthen qualified or conditional claims.";

  var COMMISSIONING_CUE_SUBORDINATION_LINE =
    "Subject and context are commissioning cues only — not authoritative on-image claim labels.";

  var COMMISSIONING_CUE_LABEL_BOUNDARY_LINE =
    "Learner-facing on-image labels and captions must stay within allowed_claims, disallowed_claims, and canonical_discipline_note.";

  function readCanonicalDisciplineNote(jobOrBrief) {
    if (!jobOrBrief || typeof jobOrBrief !== "object") return "";
    if (isNonEmptyString(jobOrBrief.canonical_discipline_note)) {
      return String(jobOrBrief.canonical_discipline_note).trim();
    }
    if (
      jobOrBrief.discipline_guidance &&
      isNonEmptyString(jobOrBrief.discipline_guidance.canonical_note)
    ) {
      return String(jobOrBrief.discipline_guidance.canonical_note).trim();
    }
    return "";
  }

  function readAllowedClaims(jobOrBrief) {
    if (!jobOrBrief || typeof jobOrBrief !== "object") return [];
    if (jobOrBrief.claim_constraints && Array.isArray(jobOrBrief.claim_constraints.allowed)) {
      return asStringArray(jobOrBrief.claim_constraints.allowed);
    }
    return asStringArray(jobOrBrief.allowed_claims);
  }

  function readDisallowedClaims(jobOrBrief) {
    if (!jobOrBrief || typeof jobOrBrief !== "object") return [];
    if (jobOrBrief.claim_constraints && Array.isArray(jobOrBrief.claim_constraints.disallowed)) {
      return asStringArray(jobOrBrief.claim_constraints.disallowed);
    }
    return asStringArray(jobOrBrief.disallowed_claims);
  }

  function buildClaimBoundariesInstructionBlock(job) {
    var allowed = readAllowedClaims(job);
    var disallowed = readDisallowedClaims(job);
    var canonicalNote = readCanonicalDisciplineNote(job);
    var lines = [
      "- " + CLAIM_STRENGTH_PRESERVATION_LINE,
      "- allowed_claims:",
      formatList(allowed.map(String), "  - "),
      "- disallowed_claims:",
      formatList(disallowed.map(String), "  - ")
    ];
    if (canonicalNote) {
      lines.push("- canonical_discipline_note: " + canonicalNote);
    }
    lines.push("- " + COMMISSIONING_CUE_SUBORDINATION_LINE);
    lines.push("- " + COMMISSIONING_CUE_LABEL_BOUNDARY_LINE);
    lines.push(
      "- If a relationship is authorised only qualitatively and no exact formal relationship is supplied, do not invent an equation — use arrows, grouping or short labels."
    );
    return lines.join("\n");
  }

  function buildCommissioningCuesInstructionBlock(job) {
    return (
      "- " +
      COMMISSIONING_CUE_SUBORDINATION_LINE +
      "\n- " +
      COMMISSIONING_CUE_LABEL_BOUNDARY_LINE +
      "\n- subject: " +
      String(job.subject || "(none)") +
      "\n- context: " +
      String(job.context || "(none)")
    );
  }

  /**
   * Shared claim-discipline lines for human/operator prompts (matches canonical §3).
   */
  function buildHumanClaimDisciplineLines(jobOrBrief) {
    var allowed = readAllowedClaims(jobOrBrief);
    var disallowed = readDisallowedClaims(jobOrBrief);
    var canonicalNote = readCanonicalDisciplineNote(jobOrBrief);
    var lines = ["- " + CLAIM_STRENGTH_PRESERVATION_LINE];
    disallowed.forEach(function (claim) {
      lines.push("- Do not claim: " + claim);
    });
    allowed.forEach(function (claim) {
      lines.push("- Supported claim boundary: " + claim);
    });
    if (canonicalNote) {
      lines.push("- Model-class scope (canonical_discipline_note): " + canonicalNote);
    }
    lines.push("- " + COMMISSIONING_CUE_SUBORDINATION_LINE);
    lines.push("- " + COMMISSIONING_CUE_LABEL_BOUNDARY_LINE);
    lines.push(
      "Use only claims supported by the supplied brief. Do not invent unsupported factual detail."
    );
    lines.push(
      "Do not introduce mathematical forms, constraint classes, or optimality/evidence claims stronger or broader than the allowed claims, disallowed claims, and exclusions in this brief."
    );
    lines.push(
      "If a relationship is authorised only qualitatively and no exact formal relationship is supplied, do not invent an equation — use arrows, grouping or short labels."
    );
  ...
  function buildCommissioningCueHumanLines(jobOrBrief) {
    var subject = String((jobOrBrief && jobOrBrief.subject) || "").trim();
    var context = String((jobOrBrief && jobOrBrief.context) || "").trim();
    if (!subject && !context) return [];
    var lines = [
      "- " + COMMISSIONING_CUE_SUBORDINATION_LINE,
      "- " + COMMISSIONING_CUE_LABEL_BOUNDARY_LINE
    ];
    if (subject) lines.push("- Subject cue: " + subject);
    if (context && context !== subject) lines.push("- Context cue: " + context);
    return lines;
  }
  ...
  function buildGenerationInstruction(briefCore, job, template, sourceEvidence) {
    var precisionClaims = collectJobPrecisionClaims(job, sourceEvidence);
    var sections = [];
    sections.push(
    ...
    sections.push("3. Claim boundaries\n" + buildClaimBoundariesInstructionBlock(job));
    sections.push("4. Commissioning cues (subordinate)\n" + buildCommissioningCuesInstructionBlock(job));
    sections.push("5. Evidence basis\n" + formatEvidenceBasis(sourceEvidence));
    var precisionBlock = precisionFidelityMod.formatCanonicalPrecisionBlock(
      precisionClaims,
      job
    );
    if (precisionBlock) {
      sections.push("5b. Precision-critical fidelity\n" + precisionBlock);
    }
    var authorisedConnectors = formatAuthorisedConnectorsBlock(
      sourceEvidence,
      asStringArray(job && job.must_show)
    );
    if (authorisedConnectors) {
      sections.push("5c. Authorised connectors (semantic)\n" + authorisedConnectors);
    }
    sections.push(
      "6. Required content\n" +
    ...
    sections.push("8. Spoiler boundary\n" + formatSpoilerSection(job));
    sections.push(
    ...
    sections.push(
      "11. Resource visual language\n" +
        formatList(houseVisualMod.getHouseVisualLanguageLines())
    );
    sections.push(
  ...
  function compileOneBrief(job, index, meta, errors, warnings, diagnostics) {
    ...
    if (job.requires_exact_data_match === true) {
      brief.requires_exact_data_match = true;
    }
    if (job.material_role && typeof job.material_role === "object") {
      brief.material_role = deepClone(job.material_role);
      if (job.material_role.requires_exact_data_match === true) {
        brief.requires_exact_data_match = true;
      }
    }
  ...
  return {
    ...
    CLAIM_STRENGTH_PRESERVATION_LINE: CLAIM_STRENGTH_PRESERVATION_LINE,
    COMMISSIONING_CUE_SUBORDINATION_LINE: COMMISSIONING_CUE_SUBORDINATION_LINE,
    COMMISSIONING_CUE_LABEL_BOUNDARY_LINE: COMMISSIONING_CUE_LABEL_BOUNDARY_LINE,
    buildClaimBoundariesInstructionBlock: buildClaimBoundariesInstructionBlock,
    buildCommissioningCuesInstructionBlock: buildCommissioningCuesInstructionBlock,
    buildHumanClaimDisciplineLines: buildHumanClaimDisciplineLines,
    buildCommissioningCueHumanLines: buildCommissioningCueHumanLines,
    extractAuthorisedSemanticEdges: extractAuthorisedSemanticEdges,
    formatAuthorisedSemanticEdgeLines: formatAuthorisedSemanticEdgeLines,
    formatAuthorisedConnectorsBlock: formatAuthorisedConnectorsBlock,
    AUTHORISED_CONNECTOR_RULE: AUTHORISED_CONNECTOR_RULE,
