 ...
 * Sprint 57: depth/completeness floors defer to pack GAM-PRES-08/09 (authoritative).
 */
(function (root, factory) {
  ...
  var MARKER_PREFIX = "INSTRUCTIONAL-PATTERN-PROMPT (GAM depth reference)";
  ...
  var SP_GAM_PRES_PREFIX_LINES = [
    "",
    MARKER_PREFIX + ":",
    "- Depth, completeness, DEPTH-COLLAPSE, and type minima: pack GAM-PRES-08/09 authoritative.",
    "- Blocks below add pattern-specific shape, FM fail codes, and GOOD examples only.",
  ...
  var SP01_LINES = [
    ...
    "- MUST link at least two distinct ideas with explicit relational language; teach and explain for the learner — clearly distinct from assigning the learner deliverable or expected_output task.",
  ...
  var SP02_LINES = [
    ...
    "- When emitting Material: ... (decision_table): every decision_table body MUST include exactly one partial exemplar row modelling evidence-gathering before learner judgement.",
    "- On the exemplar row only: populate evidence/reasoning cells; remaining rows leave learner-completion cells empty.",
  ...
  var SP03_LINES = [
    ...
    "- When emitting Material: ... (transfer_prompt): every transfer_prompt body MUST provide substantive learner-owned transfer scaffolding — not a thin slot (MP-1).",
    "- MUST include learner-context selection — learner chooses application context.",
    "- MUST include an operational completion criterion — word band and/or required elements per GAM-PRES-08 (T1) when transfer_prompt.",
    "- MUST include at least three distinct application cues or analytical moves in bullet form.",
    "- MUST explicitly name or link to session criteria or key ideas from upstream materials when available.",
    ...
    "  Choose a [context you know] (e.g. …). Write [word band per GAM-PRES-08 (T1)] addressing:",
  ...
  var SP04_LINES = [
    ...
    "- MUST require learner-produced synthesis — imperative learner-write instruction.",
    "- MUST include an operational completion criterion — word band per GAM-PRES-08 when consolidation/closure specified.",
    "- MUST include at least three synthesis angles or prompts in bullet form.",
    "- Synthesis angles MUST focus on reflection, distinction, judgement, uncertainty, and/or future questions.",
    "- Do not supply a completed consolidation essay or pre-written synthesis; scaffolding only (MP-1).",
  ...
  var SP05_FALLBACK_LINES = [
    "",
    MARKER_SP05 + ":",
    "- Module: " + MODULE_ID + " | Pattern: SP-05 / CL-SP-01 guided diagnostic verification checklist | Contract: Sprint 71 guided-review quality",
    '- When emitting Material: ... (checklist) for verification: prefer structured guided-review JSON, not Markdown bullet lists.',
    '- Use body_format: "json" and body object: { "review_mode": "guided_criteria", "criteria": [ ... ] }.',
    "- Emit 3–4 criteria (hard maximum 5).",
    "- Normally emit 2–3 independently observable {expected, repair} pairs per criterion (maximum 4).",
    "- Every feature requires a paired, discipline-specific repair; generic-only repairs are forbidden.",
    "- Repairs must help the learner revise their own response — never supply a complete replacement answer (MP-1).",
    "- Simple Markdown bullet checklists remain valid only for backward-compatible non-guided cases."
  ];

  function buildSp05Lines() {
    var contract = resolveGuidedReviewGenerationContract();
    var lines = ["", MARKER_SP05 + ":", "- Module: " + MODULE_ID + " | Pattern: SP-05 / CL-SP-01 guided diagnostic verification checklist | Contract: Sprint 71 guided-review quality", "- When emitting Material: ... (checklist) for verification: prefer structured guided-review JSON, not Markdown bullet lists."];
    if (contract && typeof contract.buildGuidedReviewGenerationGuidanceLines === "function") {
      return lines.concat(contract.buildGuidedReviewGenerationGuidanceLines());
    }
    return SP05_FALLBACK_LINES.slice();
  }
  ...
  var SP06_LINES = [
    ...
    "- When emitting Material: ... (worked_example): every worked_example body MUST include an explicit parallel-task bridge from modelled method to learner's independent task.",
    ...
    "- After labelled steps and before **Bridge:**, MUST include `## What experts notice` (≥2 bullets on disciplinary reasoning moves — not generic praise).",
    "- worked_example with steps but no `## What experts notice` is an instructional FAIL (FM-12) — do not emit.",
    "- End with **Bridge:** (word band per GAM-PRES-08 when worked_example bridge specified) directing learner to apply the same method on their scenario — transfer the method, not the answer (MP-3).",
    "- FORBIDDEN bridge stubs: Apply this reasoning to all stages; bridges below GAM-PRES-08 minima.",
    ...
    "  **Bridge:** Now use the same method on your assigned scenario — identify the mechanism, ask what problem it solves, then explain the functional consequence. Do not copy the model outcome.",
    ...
    "- worked_example containing checklist or verification items is an instructional FAIL (FM-14) — do not emit.",
    "- FORBIDDEN: step chain + model conclusion only, with no parallel-task bridge — FM-05.",
    "- When S78-WS-2 MODEL-PRACTICE-INDEPENDENCE (auto-applied) lists a model material_id, its per-binding operand rules are authoritative for that row: distinct model operand; bound attempt material_id(s) are learner-owned — Bridge transfers method only, not attempt answer or load-bearing reasoning."
  ];
  ...
  var SP07_LINES = [
    ...
    "- Bullets MUST name reasoning quality, conceptual connections, analytical moves, evaluative moves, or disciplinary judgement.",
    "- End with: Use this as a quality guide, not as text to copy.",
    "- Exemplar MUST be parallel to but not identical with the learner's assigned deliverable when independent production is required (MP-1).",
    ...
    "- When a model row carries practice_independence in the authoritative DLA commission, sample_output must not pre-answer bound attempt operand material_id(s) — parallel quality guide only."
  ];
  ...
  function prefixAlreadyPresent(text) {
    return new RegExp(MARKER_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i").test(
      String(text || "")
    );
  }
  ...
  function buildSpGamPresPrefixBlock() {
    return SP_GAM_PRES_PREFIX_LINES.join("\n");
  }
  ...
  function buildSp05PromptBlock(options) {
    var opts = options && typeof options === "object" ? options : {};
    var includeMarker = opts.includeMarker !== false;
    var sp05Lines = buildSp05Lines();
    var lines = includeMarker ? sp05Lines.slice() : sp05Lines.slice(2);
    return lines.join("\n");
  }
  ...
  function buildInstructionalPatternPromptBlock(options) {
    ...
    if (!prefixAlreadyPresent("")) {
      parts.push(buildSpGamPresPrefixBlock());
    }
    if (
      !opts.sp03Only &&
      !opts.sp05Only &&
  ...
  function appendSpBlock(out, block) {
    if (!block) return out;
    if (!prefixAlreadyPresent(out)) {
      out = (out + buildSpGamPresPrefixBlock() + block).trim();
    } else {
      out = (out + block).trim();
    }
    return out;
  ...
  function applyInstructionalPatternPromptBlockToDraft(draft, context) {
    ...
    if (!sp02AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp02PromptBlock());
    }
    if (!sp03AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp03PromptBlock());
    }
    if (!sp06AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp06PromptBlock());
    }
    if (!sp07AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp07PromptBlock());
    }
    if (!sp04AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp04PromptBlock());
    }
    if (!sp05AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp05PromptBlock());
    }
    if (!sp01AlreadyPresent(out)) {
      out = appendSpBlock(out, buildSp01PromptBlock());
    }
    return out;
  }
  ...
  return {
    ...
    buildSpGamPresPrefixBlock: buildSpGamPresPrefixBlock,
