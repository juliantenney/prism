...
(function (root, factory) {
  ...
  function () {
     ...
     */
    var SCRIPT_VERSION = "20260715-e01w";
    ...
    var RULES = {
      ...
      evaluation_judgement:
        "Address the supplied evaluation question with actual domain reasoning (not meta-advice about what good judgements look like). Apply every supplied criterion to the supplied evidence rather than listing criteria as a checklist: for each criterion, show how relevant evidence counts for or against it. Explicitly weigh competing criteria or interpretations against each other. Surface the supplied trade-offs, limitations, or competing interpretations as material tensions that constrain the conclusion. Reach a justified judgement that directly addresses judgement_focus and follows from that comparative weighing. Do not make unsupported claims beyond the supplied evidence. Do not collapse the task into a generic pros-and-cons list, a recall answer, a definition, or abstract statements such as \"a convincing judgement compares evidence across criteria\" or \"consider strengths and weaknesses\" as a substitute for performing the evaluation. Write connected learner-facing prose — do not emit Question:/Criteria:/Evidence:/Trade-offs:/Judgement: rubric headings or raw planning-schema fields as learner-facing labels. When material_type is worked_example: the body must model a complete worked judgement sequence — decision/question → criteria in use → evidence assessed against each criterion → at least one limitation, trade-off, or competing interpretation → comparative weighing → final justified conclusion tied to judgement_focus — with inferential links shown in domain terms; never end after stating the qualities of a good answer. When material_type is template: prefer a clear reasoning scaffold such as Initial position → Evidence assessed against the criteria → Competing interpretation or limitation → Final justified judgement; avoid duplicative Conclusion and Final judgement headings that ask for the same content twice."
    };
    ...
    function validateMaterialArchetypePlan(material, options) {
      ...
      if (!isAllowedArchetype(archetype)) {
        errors.push(
          pathLabel(activityId, materialId, "instructional_archetype") +
            ...
            "); no archetype rule will activate"
        );
        return { ok: false, errors: errors, warnings: warnings, active: false };
      }
      ...
      } else if (archetype === "evaluation_judgement") {
        if (!nonEmptyString(plan.question)) {
          errors.push(
            pathLabel(activityId, materialId, "archetype_plan.question") +
              " required (non-empty)"
          );
        }
        if (nonEmptyStringList(plan.criteria).length < 2) {
          errors.push(
            pathLabel(activityId, materialId, "archetype_plan.criteria") +
              " requires at least two non-empty entries"
          );
        }
        if (nonEmptyStringList(plan.evidence).length < 1) {
          errors.push(
            pathLabel(activityId, materialId, "archetype_plan.evidence") +
              " requires at least one non-empty entry"
          );
        }
        if (nonEmptyStringList(plan.tradeoffs).length < 1) {
          errors.push(
            pathLabel(activityId, materialId, "archetype_plan.tradeoffs") +
              " requires at least one non-empty entry"
          );
        }
        if (!nonEmptyString(plan.judgement_focus)) {
          errors.push(
            pathLabel(activityId, materialId, "archetype_plan.judgement_focus") +
              " required (non-empty)"
          );
        }
      }
    ...
    function planPayloadForPrompt(archetype, plan) {
      ...
      if (archetype === "evaluation_judgement") {
        return {
          question: nonEmptyString(plan.question),
          criteria: nonEmptyStringList(plan.criteria),
          evidence: nonEmptyStringList(plan.evidence),
          tradeoffs: nonEmptyStringList(plan.tradeoffs),
          judgement_focus: nonEmptyString(plan.judgement_focus)
        };
      }
      return {};
    }
    ...
    function analyzePromptArchetypeRuleFlags(promptText) {
      ...
      return {
        ...
        contains_evaluation_judgement_rule:
          text.indexOf(RULES.evaluation_judgement) >= 0 ||
          /Apply every supplied criterion to the supplied evidence/i.test(text) ||
          /model a complete worked judgement sequence/i.test(text)
      };
    }
    ...
    function buildFinalGamPromptObservabilitySnapshot(promptText, meta, page) {
      ...
      return {
        ...
        contains_evaluation_judgement_rule: ruleFlags.contains_evaluation_judgement_rule,
