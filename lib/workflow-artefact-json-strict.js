...
(function (root, factory) {
  ...
  var LC_REQUIRED_TOP_KEYS = ["title", "sections", "key_concepts", "examples"];
  ...
  function normalizeLearningContentShape(obj) {
    if (!obj || typeof obj !== "object" || Array.isArray(obj)) return null;
    var out = Object.assign({}, obj);
    if (typeof out.title !== "string") out.title = String(out.title || "");
    var i;
    for (i = 0; i < LC_REQUIRED_TOP_KEYS.length; i += 1) {
      var key = LC_REQUIRED_TOP_KEYS[i];
  ...
  function validateLearningContentShape(obj) {
    var normalized = normalizeLearningContentShape(obj);
    if (!normalized) {
      return { ok: false, errors: ["learning_content:not_object"] };
    }
    var errors = [];
    if (typeof normalized.title !== "string") {
      errors.push("learning_content:missing_title_string");
    }
    if (!Array.isArray(normalized.sections)) {
      errors.push("learning_content:missing_sections_array");
    }
    if (!Array.isArray(normalized.key_concepts)) {
      errors.push("learning_content:missing_key_concepts_array");
    }
    if (!Array.isArray(normalized.examples)) {
      errors.push("learning_content:missing_examples_array");
    }
    return errors.length
      ? { ok: false, errors: errors }
      : { ok: true, value: normalized, errors: [] };
  ...
  function parseLearningContentCaptureStrict(raw, sanitizeFn) {
    var capture = parseFencedJsonObjectCapture(raw, sanitizeFn, {
      label: "Generate Learning Content",
      rootObject: "learning_content"
    });
    if (!capture.ok) {
      throw new Error(
        "Generate Learning Content capture is not fenced JSON output (" +
          capture.errors.join(", ") +
          ")"
      );
    }
    var shape = validateLearningContentShape(capture.parsed);
    if (!shape.ok) {
      throw new Error(
        "Generate Learning Content JSON structure invalid: " + shape.errors.join("; ")
      );
    }
    return shape.value;
  }
  ...
  function validateWorkflowStepStrictJsonCapture(raw, stepKind, sanitizeFn) {
    ...
    var captureOptions =
      kind === "knowledge_model"
        ...
        : kind === "learning_content"
        ? { label: "Generate Learning Content", rootObject: "learning_content" }
        : { label: "Design Episode Plan", rootObject: "episode_plans" };
    ...
    var prettyViolations = detectPrettyPrintJsonViolations(capture.json);
    if (prettyViolations.length) {
      return {
        ok: false,
        errors: prettyViolations,
        message:
          captureOptions.label +
          " JSON must be pretty-printed with line breaks (not minified single-line JSON)"
      };
    }
    var shape =
      kind === "knowledge_model"
        ...
        : kind === "learning_content"
        ? validateLearningContentShape(capture.parsed)
        : validateEpisodePlansShape(capture.parsed);
    if (!shape.ok) {
      return {
        ok: false,
        errors: shape.errors,
  ...
  function buildStrictLearningContentOutputContractBlock() {
    return [
      "",
      "Output contract (strict — fenced JSON block only):",
      "- Return exactly one markdown fenced JSON block opened with ```json and closed with ```.",
      ...
      "- Do NOT return raw JSON without the ```json fence (unfenced JSON text is invalid).",
      "- Do NOT include JSON comments (no // or block comments).",
      "- JSON top-level keys: title (string), sections (array of {title, content}), key_concepts (array of strings), examples (array of {title, description}).",
      "- Return the fenced block first, then the single STEP N OUTPUT footer line."
    ].join("\n");
  }
  ...
  function buildStrictLearningSequenceOutputContractBlock() {
    return [
      ...
      "- Do NOT include any prose, headings, commentary, or explanations before the fenced block.",
      "- After the closing ``` fence, emit exactly one plain-text runner footer line: STEP N OUTPUT: learning_sequence",
      "- Do NOT return raw JSON without the ```json fence (unfenced JSON text is invalid).",
      "- Do NOT include JSON comments (no // or block comments).",
  ...
  function buildStrictEpisodePlansOutputContractBlock() {
    return [
      "",
      "Output contract (strict — fenced JSON block only):",
      "- Return exactly one markdown fenced JSON block opened with ```json and closed with ```.",
      "- The fenced block body must be the episode_plans root object as valid, complete JSON.",
      "- Pretty-print JSON with 2-space indentation and line breaks after `{`, `[`, and each property.",
      "- Do NOT emit minified single-line JSON (unbroken `{...}` on one line is invalid).",
      "- Do NOT include any prose, headings, commentary, or explanations before the fenced block.",
      ...
      "- Do NOT return raw JSON without the ```json fence (unfenced JSON text is invalid).",
      "- Do NOT include JSON comments (no // or block comments).",
      "- JSON top-level key: episode_plans (required array).",
      "- Each episode_plans entry: activity_id (string), episode_plan { archetype, beats[] { function } }.",
      "- Archetypes (frozen V1): understand | apply | analyse | evaluate only.",
      "- Beat functions: approved FunctionEnum only (e.g. explanation, worked_thinking, guided_practice, verification, transfer).",
      "- Example shape (pretty-printed; adapt to upstream learning_outcomes):",
      '  {',
      '    "episode_plans": [',
      '      {',
      '        "activity_id": "LO1",',
  ...
  /**
   * S87-T-007 live repair — Expository Journey Plan must not echo the LO capture shape.
   * Appended on Copy for EJP steps (parity with learning_outcomes strict contract).
   */
  function buildStrictExpositoryJourneyPlanOutputContractBlock() {
    return [
      "",
      "Output contract (strict — fenced JSON block only):",
      "- Return exactly one markdown fenced JSON block opened with ```json and closed with ```.",
      "- The fenced block body must be an expository_journey_plan root object as valid JSON.",
      ...
      "- Do NOT return raw JSON without the ```json fence (unfenced JSON text is invalid).",
      "- Do NOT return a learning_outcomes root object (Define Learning Outcomes shape). Upstream LO is an input only.",
      "- Required top-level keys: artifact_type \"expository_journey_plan\", title, audience, journey_intent, commissioned_purpose (non-empty free text), epistemic_form (non-empty free text — not an enum), sections (non-empty array).",
      "- Optional top-level: extent, learning_outcomes (references only), generation_notes, schema_version.",
      "- Each sections[] entry: section_id, title, purpose, knowledge_focus, conceptual_move, dependencies[], lo_refs[], elaboration_intentions[], representation_needs[], connections[], synthesis, order.",
      "- Minimal shape (adapt content; keep these keys):",
      '  {',
  ...
  function validateEpisodePlansShape(obj) {
    if (!obj || typeof obj !== "object" || Array.isArray(obj)) {
      return { ok: false, errors: ["episode_plans:not_object"] };
    }
    if (!Array.isArray(obj.episode_plans)) {
      return { ok: false, errors: ["episode_plans:missing_episode_plans_array"] };
    }
    if (!obj.episode_plans.length) {
      return { ok: false, errors: ["episode_plans:empty_episode_plans_array"] };
    }
    var errors = [];
    obj.episode_plans.forEach(function (row, index) {
      if (!row || typeof row !== "object" || Array.isArray(row)) {
        errors.push("episode_plans[" + index + "]: not an object");
        return;
      }
      if (!String(row.activity_id || "").trim()) {
        errors.push("episode_plans[" + index + "]: missing activity_id");
      }
      if (!row.episode_plan || typeof row.episode_plan !== "object" || Array.isArray(row.episode_plan)) {
        errors.push("episode_plans[" + index + "]: missing episode_plan object");
      }
    });
    return errors.length
      ? { ok: false, errors: errors }
      : { ok: true, value: obj, errors: [] };
  }

  function detectPrettyPrintJsonViolations(jsonText) {
    var t = String(jsonText || "").trim();
    if (!t) return ["empty_json"];
    if (t.indexOf("\n") === -1) return ["minified_single_line_json"];
    if (/^\{[^\n]+\}$/.test(t)) return ["minified_single_line_json"];
    return [];
  }
  ...
  return {
    ...
    LC_REQUIRED_TOP_KEYS: LC_REQUIRED_TOP_KEYS,
    ...
    normalizeLearningContentShape: normalizeLearningContentShape,
    validateLearningContentShape: validateLearningContentShape,
    ...
    parseLearningContentCaptureStrict: parseLearningContentCaptureStrict,
    ...
    buildStrictLearningContentOutputContractBlock: buildStrictLearningContentOutputContractBlock,
    ...
    buildStrictEpisodePlansOutputContractBlock: buildStrictEpisodePlansOutputContractBlock,
    buildStrictExpositoryJourneyPlanOutputContractBlock: buildStrictExpositoryJourneyPlanOutputContractBlock,
    validateEpisodePlansShape: validateEpisodePlansShape,
    detectPrettyPrintJsonViolations: detectPrettyPrintJsonViolations
  };
});
