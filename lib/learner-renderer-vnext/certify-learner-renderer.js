...
function certifyPersistence(workflow, html, checks) {
  ...
  var tableInputs = root.querySelectorAll(".util-learner-table-workspace__input");
...
function certifyWorkflow(entry, repoRoot, options) {
  ...
  var primaryArchetypeFailure = (modelResult.errors || [])
    .concat(modelResult.warnings || [])
    .some(function (row) {
      return (
        row &&
        (row.code === "UNKNOWN_ARCHETYPE" ||
          row.code === "ARCHETYPE_GRAMMAR_VALIDATION_FAILED" ||
          row.code === "UNKNOWN_EPISODE_PLAN_BEAT" ||
          row.code === "MIXED_EPISODE_PLAN_VOCABULARY" ||
          row.code === "MALFORMED_EPISODE_PLAN_SEQUENCE" ||
          row.code === "UNKNOWN_ARCHETYPE_VARIANT")
      );
    });
  checks.push(
    checkResult({
      checkId: "archetype.unknown",
      status: primaryArchetypeFailure ? STATUS.FAIL : STATUS.PASS,
      workflowId: entry.id,
      message: primaryArchetypeFailure
        ? "Primary archetype validation failure present."
        : "No primary archetype validation failures."
    })
  );
