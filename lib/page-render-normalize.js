...
(function (root, factory) {
  ...
  function materialTypeToRenderKey(type) {
    ...
    if (t === "strategy_options" || t === "strategy" || t === "strategies") return "task_cards";
  ...
  function appendMaterialBucket(target, key, value) {
    ...
    var listKeys = { scenarios: true, task_cards: true, cards: true, checklist: true, prompt_set: true, strategy_options: true, strategies: true };
    if (listKeys[key]) {
  ...
  function mergeConvertedMaterialMap(target, source, usedKeys, sequence) {
    if (!source || typeof source !== "object") return;
    var sourceTypes =
      source._material_types && typeof source._material_types === "object"
        ? source._material_types
        : {};
    var sourceIds =
      source._material_ids && typeof source._material_ids === "object"
        ? source._material_ids
        : {};
    var sourceSequence = Array.isArray(source._render_sequence) ? source._render_sequence : [];
    sourceSequence.forEach(function (entry, index) {
      var key = String((entry && entry.key) || "").trim();
      if (!key || !Object.prototype.hasOwnProperty.call(source, key)) return;
      var typeKey = materialTypeToRenderKey(sourceTypes[key] || entry.material_type || key);
      var materialId = String(sourceIds[key] || entry.material_id || "").trim();
      var renderKey = uniqueMaterialStorageKey(typeKey, materialId, sequence.length + index, usedKeys);
      usedKeys[renderKey] = true;
      target[renderKey] = source[key];
      target._material_types = target._material_types || {};
      target._material_types[renderKey] = typeKey;
      if (materialId) {
        target._material_ids = target._material_ids || {};
        target._material_ids[renderKey] = materialId;
      }
      sequence.push({
        key: renderKey,
  ...
  function convertMaterialsArrayToObjectMap(materials) {
    ...
    materials.forEach(function (entry, index) {
      ...
      if (
        ...
        var childEntries = Array.isArray(payload.materials)
          ? payload.materials.slice()
          : Array.isArray(payload.items)
          ? payload.items.slice()
          : [];
        if (!childEntries.length && Array.isArray(payload.sections)) {
          payload.sections.forEach(function (section) {
            if (!section || typeof section !== "object" || Array.isArray(section)) return;
            if (Array.isArray(section.materials)) {
              childEntries = childEntries.concat(section.materials);
              return;
            }
            if (
              section.type ||
              ...
              childEntries.push(section);
            }
          });
        }
        var hasTypedChildren =
          childEntries.length &&
          childEntries.every(function (child) {
            return (
              child &&
              typeof child === "object" &&
              !Array.isArray(child) &&
              !!(child.type || child.material_type || child.materialType || child.kind)
            );
          });
        if (hasTypedChildren) {
          mergeConvertedMaterialMap(
            out,
            convertMaterialsArrayToObjectMap(childEntries),
            usedKeys,
            sequence
          );
          return;
