/**
 * Sprint 38-E — Inflation AFTER pipeline capture (DLA → GAM → Design Page).
 * Run: node docs/development/sprints/2026-06-04-sprint-38e-workbook-contract-implementation/artefacts/ev-38e5-inflation-pipeline-capture-once.mjs
 *
 * Writes ONLY to sprint 38-E artefacts/ (does not overwrite 38-B EV-01 fixtures).
 */
import fs from "fs";
...
const ctxHeader =
  "Workflow: Inflation self-directed learner workbook (Sprint 38-E AFTER — same topic/brief as EV-01).\n" +
  "page_profile=learner; apply pack DLA-WB and GAM-WB contracts.\n";
...
const dlaText = await callOpenAI(
  ...
  ctxHeader +
    "\n\nMandatory: This is a 60-minute self_study_workbook. delivery_notes MUST include resource_intent: self_study_workbook, session_duration_target_minutes: 60, consolidation_requirement, workbook_contract_applied: true.\n\nApply ALL Self-study workbook contract (DLA-WB) rules in the augmented prompt below.\n\nUpstream learning_outcomes:\n" +
...
const gamTextRaw = await callOpenAI(
  ...
  ctxHeader +
    "\n\nMandatory: Apply Self-study workbook genre contract (GAM-WB) in augmented prompt. Realise every DLA required_materials entry with full Content bodies.\n\nUpstream learning_activities:\n" +
...
fs.writeFileSync(path.join(outDir, "EV-38E5-AFTER-gam.txt"), gamText, "utf8");
fs.writeFileSync(path.join(outDir, "EV-38E5-AFTER-design-page.json"), JSON.stringify(page, null, 2), "utf8");
fs.writeFileSync(
  path.join(outDir, "EV-38E5-AFTER-dla-learning-activities.json"),
  JSON.stringify(learning_activities, null, 2),
  "utf8"
);
fs.writeFileSync(path.join(outDir, "EV-38E5-AFTER-run-log.json"), JSON.stringify(report, null, 2), "utf8");
if (renderSnippet) {
  fs.writeFileSync(path.join(outDir, "EV-38E5-AFTER-render-excerpt.html"), renderSnippet, "utf8");
}
