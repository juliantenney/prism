...
test("page fixture: metadata keys only inside Production Metadata fold", () => {
  const { api } = loadPrismTestApi();
  const html = renderPageFixture(api, "research-multiline-bullets.json");
  const bodyHtml = html.split('<details class="util-meta"')[0];
  assert.equal(bodyHtml.includes("Briefing Note"), false);
  assert.match(html, /Production Metadata/);
  assert.match(html, /Briefing Note/);
});

test("page fixture: Research string array section.content uses markdown lists", () => {
  const { api } = loadPrismTestApi();
  const html = renderPageFixture(api, "research-array-string-bullets.json");
  assert.match(html, /<ul>/);
  assert.equal((html.match(/<li>/g) || []).length, 3);
  assert.match(html, /First finding from array element/);
});
