...
- **Sprint 09 governance:** **contract surface** (`briefLines`, `extractWorkflowBriefExplicitFactors`, step-context builders, `workflowGenerationContext.js`, domain-pack factor ids/labels/values and prompt-coupled config) is **out of scope** for Sprint 09 **including Pass 2**; Pass 2 may target **only** non-prompt-facing UI copy, explanatory text, ARIA/help/placeholder, and organisational/grouping clarification. Contract-level work → future dedicated sprint **“Workflow Brief Contract Rationalisation”** (see canonical Sprint 09 doc **Sprint 09 governance** / **Sprint 09 Pass 2** sections).
