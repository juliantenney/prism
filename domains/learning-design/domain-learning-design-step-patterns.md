...
```json
{
  "workflowPolicy": {
    ...
    "precedenceRules": [
      ["Normalize Content", "Generate Learning Content"],
...
```json
{
  ...
  "defaultPromptNotes": "Use a canonical normalize-content prompt that cleans and reorganises source content into coherent sections while preserving important detail and examples for downstream modelling. Return exactly one markdown fenced block (triple-backtick markdown fence) containing normalized_content with no prose before the fence.",
  "runnerInstructions": {
...
```json
{
  ...
  "defaultPromptNotes": "Return exactly one \u0060\u0060\u0060json fenced block containing the learning_content object. No prose before the block. Generate or extend pedagogically structured learning_content JSON (title, sections, key_concepts, examples) ready for downstream modelling, journey assimilation, and Design Page spine composition. When source_material is normalized_content, preserve source meaning, do not invent unsupported ideas, and convert into teaching-ready intellectual progression — not source-cleaning headings.",
  "runnerInstructions": {
    "what_this_step_does": "This step creates baseline learning_content from topic and/or normalized source inputs for downstream design steps."
  },
  "defaultOutputStructure": {
    "keys": ["title", "sections", "key_concepts", "examples"]
  },
  "promptTemplate": "Context:\nYou may be provided with topic, audience, level, and optional source_material (including normalized_content from upstream Normalize Content).\n\nTask:\nGenerate or extend teaching-ready learning_content that is pedagogically structured and suitable for downstream learning design.\n\nInstructions:\n- If source_material is provided (including normalized_content), treat it as the factual base: preserve source meaning, do not discard or contradict valid material, and do not invent ideas unsupported by the source\n- When source_material is normalized_content, convert the cleaned source into teaching-ready sectioned learning_content — clarify explanations, add suitable examples only when grounded in the source, and reorganise into intellectual progression (not Normalize-style headings such as Source overview, Cleaned content, Key terms, or Retained examples)\n- State a governing question or central inquiry in title or the first section\n- If source_material is not provided, generate content from topic, audience, and level\n- Use clear explanations, logical sectioning, key concepts, and illustrative examples\n- Organise progression from simpler ideas to more complex ideas\n- Adapt depth, terminology, and explanation style to the specified audience and level\n- Avoid unnecessary technicality unless required by audience/level\n- Avoid generic summaries; produce purposeful teaching content\n- Keep content grounded in provided topic and/or source material\n- Do not hallucinate unnecessary detail\n- Ensure output can be consumed by downstream steps such as knowledge modelling, learning-outcome generation, activity design, and Design Page journey assimilation\n- Apply step notes when provided: {{stepNotes}}\n\nOutput requirements (strict — fenced JSON block only):\n- Return exactly one markdown fenced JSON block opened with \u0060\u0060\u0060json and closed with \u0060\u0060\u0060\n- The fenced block body must be the learning_content root object as valid JSON\n- Pretty-print JSON with 2-space indentation and line breaks after `{`, `[`, and each property\n- Do NOT emit minified single-line JSON (unbroken `{...}` on one line is invalid)\n- Do NOT include any prose, headings, commentary, or explanations before the fenced block\n- After the closing \u0060\u0060\u0060 fence, emit exactly one plain-text runner footer line: STEP N OUTPUT: learning_content\n- Do NOT return raw JSON without the \u0060\u0060\u0060json fence (unfenced JSON text is invalid)\n- Do NOT include JSON comments (no // or block comments)\n- JSON top-level keys: title (string), sections (array of {title, content} objects), key_concepts (array of strings), examples (array of {title, description} objects)\n- title: include governing question or inquiry when possible\n- sections: ordered teaching progression (not Normalize-style source-cleaning headings)\n- Return the fenced block first, then the single STEP N OUTPUT footer line."
}
```
...
```json
{
  ...
  "preferredOutputFormat": "json",
  "defaultPromptNotes": "Return exactly one \u0060\u0060\u0060json fenced block containing the learning_outcomes object. No prose before the block; emit STEP N OUTPUT: learning_outcomes after the closing fence. Generate observable and assessable outcomes aligned with the knowledge model, suitable for downstream assessment and activity design.",
  "runnerInstructions": {
...
- Emit final learner-facing `activities[].title` (replaces Episode Plan provisional shell titles; immutable downstream; no internal activity IDs in titles)
...
- Activity titles must name the designed experience (not LO copy / truncated LO / `Activity A#` / ID suffixes such as `… A1`)

### Active title contract (runtime SSOT)
Authoritative prompt + validation for titles: `lib/ld-activity-title-contract.js` (`DLA-ACTIVITY-TITLE-2`), applied via `lib/ld-dla-page-enrich-contract.js`, `lib/page-dla-enrich.js`, and Copilot draft overlays in `app.js`. This Prompt Factory template must stay aligned with that contract.
...
```json
{
  ...
  "defaultPromptStrategy": "default_template",
  "preferredOutputFormat": "json",
  "defaultPromptNotes": "Return exactly one \u0060\u0060\u0060json fenced block containing the learning_sequence object. No prose before the block; emit STEP N OUTPUT: learning_sequence after the closing fence. When delivery_context is self_directed, frame timeline as independent learner progression rather than live facilitation choreography.",
  "runnerInstructions": {
...
### Input
learning_outcomes, learning_activities, activity_materials, episode_plans (when Design Episode Plan is in the workflow — authoritative archetype and beat order), optional learning_sequence, optional learning_content, optional knowledge_model, optional assessment_items, optional feedback_pack, optional marking_rubric, optional assessment_blueprint
...
- PRISM assembles the final learner page deterministically from upstream partial captures (Episode Plan, DLA, GAM, Learning Sequence)
- Do not regenerate `activities[]`, re-copy `activities[].materials[]`, or replay the full page shell
...

{
  ...
  "defaultPromptStrategy": "default_template",
  "preferredOutputFormat": "json",
  "defaultPromptNotes": "vNext partial-page step (Sprint 58 + Sprint 70 Slice 1): canonical output is a concise learner-facing title, page_synthesis, plus authoritative visual planning metadata on a partial page artefact — not a full-page replay. Required: title (publication-quality, not the request/brief), page_synthesis.knowledge_summary (mandatory), page_synthesis.overview, page_synthesis.learning_purpose, page_synthesis.study_tips, visual_affordance_schema_version 38.4, activities_visual_review[], visual_affordances[] with explicit generate|defer|skip decisions (legacy reject accepted), assembly_state.current_stage design_page, assembly_state.enriched_by includes design_page. activities[] and activities[].materials[] already exist upstream from DLA/GAM partial captures — do not regenerate or re-copy from chat. sections[] is optional legacy dual-read mirror only — not canonical transport. LD-DESIGN-PAGE-PARTIAL-CONTRACT is authoritative at runtime when partialPageOutputs mode is enabled. LD-DESIGN-PAGE-COMPOSE-CONTRACT applies only to rollback/legacy modes (partialPageOutputs off). LD-THIN-ASSEMBLY-COHERENCE-CONTRACT: wrapper-gap fallback for overview/learning_purpose when upstream body absent — minimal, capped. Maths: LD-MATH-RENDER when appended. Removed authorial/journey/rhetoric modules are not current DP authorities.",
  "runnerInstructions": {
    "what_this_step_does": "Emit Design Page owned wrapper prose as page_synthesis on a partial v2 page artefact. activities[] and activities[].materials[] are already hydrated upstream — PRISM assembles the final page deterministically.",
    "what_to_check": "page_synthesis object with knowledge_summary mandatory; overview, learning_purpose, study_tips when upstream/conversation provides substance; visual_affordance_schema_version 38.4 present; activities_visual_review[] present; visual_affordances[] present with authored generate|defer|skip semantics and evidence_anchors; no downstream semantic invention delegated to later prompts; assembly_state.current_stage design_page; assembly_state.enriched_by includes design_page; sections[] optional legacy dual-read mirror only when emitted; no activities[] regeneration; no activity.materials.* recopy; concise learner-facing title present (not request/brief copy); no non-owned shell fields (audience, page_profile, learning_outcomes, episode_plans); orientation bodies omit duplicate renderer headings; LD-DESIGN-PAGE-PARTIAL-CONTRACT governs partialPageOutputs mode; LD-DESIGN-PAGE-COMPOSE-CONTRACT governs rollback/legacy only; learner-facing text excludes internal enum values."
  },
  "defaultOutputStructure": {
    "keys": [
      "artifact_type",
      ...
      "choices": [
        {
          "value": "learner",
          "label": "Learner",
          "promptInstruction": "Emit page_synthesis wrapper prose only. Transport overview/learning_purpose from upstream conversation when bodies exist; LD-THIN-ASSEMBLY-COHERENCE-CONTRACT wrapper-gap fallback only when absent — minimal, capped. knowledge_summary is mandatory substantive synthesis. Do not emit activities[] or materials[] — already hydrated upstream."
        },
        {
          "value": "facilitator",
          "label": "Facilitator",
          "promptInstruction": "Set page_profile to facilitator and prioritise run guidance and facilitation/logistics support."
        },
        {
          "value": "assessment",
          "label": "Assessment",
          "promptInstruction": "Set page_profile to assessment and preserve structured item schema integrity."
        }
      ]
    },
    {
  ...
  "promptTemplate": "PRIMARY IDENTITY — PARTIAL PAGE SYNTHESIS + AUTHORITATIVE VISUAL PLANNING (NOT FULL-PAGE COMPOSE)\n\nDesign Page owns the final learner-facing page title, wrapper/page-level learner prose, and visual-planning semantics. PRISM assembles the final v2 page deterministically from upstream partial captures (Episode Plan shell, DLA activities, GAM materials, Learning Sequence).\n\nUpstream state (already captured — do not regenerate in this step):\n- activities[] instructional fields exist from Design Learning Activities partial\n- activities[].materials[] bodies are already hydrated from Generate Activity Materials partial\n- learning_sequence, shell fields, episode_plans, learning_outcomes exist from earlier stages\n\nYour output is a partial page artefact containing Design Page owned fields only.\n\nRequired envelope:\n- artifact_type: \"page\"\n- schema_version: \"2.0.0\"\n- assembly_state.current_stage: \"design_page\"\n- assembly_state.enriched_by must include \"design_page\"\n\nRequired learner-facing title:\n- title — concise publication-quality resource title naming the subject\n- Do not copy the original request/brief as the title\n- Forbidden title scaffolding: \"Create a…\", duration requirements, audience instructions, workflow language\n- Preserve disciplinary framing; do not invent a subtitle unless the schema supports one\n\nRequired payload (canonical — page_synthesis):\n- page_synthesis.knowledge_summary — mandatory substantive concept synthesis (body + format, e.g. markdown)\n- page_synthesis.overview — transport upstream body from conversation when present\n- page_synthesis.learning_purpose — transport upstream body from conversation when present\n- page_synthesis.study_tips — transport upstream closure/debrief bodies only; do not synthesize from scratch\n\nOrientation body hygiene (renderer owns structural labels Overview / Learning purpose / Knowledge summary / Study tips):\n- page_synthesis.overview.body, learning_purpose.body, knowledge_summary.body, and study_tips.body must contain content only\n- Do not begin those bodies with ## Welcome, ## Overview, ## Learning Purpose, ## Knowledge Summary, or ## Study Tips\n- Preferred: \"This self-study resource explores …\" — not \"## Welcome\\n\\nThis self-study resource explores …\"\n\nRequired visual planning metadata (authoritative):\n- visual_affordance_schema_version: \"38.4\"\n- activities_visual_review[]: one object per upstream activity_id with mandatory activity_visual_value.decision (high|medium|low|none) AND activity_visual_value.rationale (non-empty string). Example: {\"activity_id\":\"A1\",\"activity_visual_value\":{\"decision\":\"high\",\"rationale\":\"Classification matrix supports mechanism typing.\"}}\n- visual_affordances[] with explicit visual_decision per row: generate | defer | skip (legacy reject accepted)\n- Every visual_affordances[] row must author semantic planning: scope, subject, context, evidence_anchors, rationale (mandatory non-empty; explain instructional value, not title/topic restatement)\n- Generate rows must also author caption_intent, alt_text, and detailed_description from the same semantic specification; do not omit rationale when adding those fields; do not copy detailed_description into rationale\n- subject should be a concise noun phrase naming what the visual is about (not just the activity title)\n- context must be a visual brief: relationships/comparisons to communicate, concepts to include, learner perspective, educational purpose, and conceptual boundaries\n- context must exclude artistic style, rendering instructions, filenames, and runtime metadata\n- must_show / must_not_show should use concrete educational constraints (e.g., threshold boundaries, widening residual spread, bidirectional causal links), not vague placeholders\n- evidence_anchors should cite all relevant supporting locations when multiple sources ground the row using canonical forms only: activity_id.path (e.g. A3.learner_task, A3.materials.scenarios) or page_synthesis.field (overview|knowledge_summary|learning_purpose|study_tips). Forbidden: free-text labels, topic names, incomplete IDs, or object shapes\n- Activity rows must include activity_id and slot-compatible placement intent\n- Use existing activity visual slots where possible; introduce new slots only when genuinely required\n- Page-level Knowledge Summary synthesis row must be authored with scope \"page\", region \"knowledge_summary\", and slot \"knowledge-summary-after-content\"\n- Prefer controlled vocabulary for preferred_representation while preserving semantic intent\n- learner_stage (generate only) must be exactly one of: \"pre_classification\" | \"post_reasoning\" — no other values are permitted\n- Use \"pre_classification\" when the visual supports noticing, sorting, comparison, classification, or reasoning before the learner reaches a conclusion\n- Use \"post_reasoning\" when the visual consolidates, summarises, or represents reasoning after the learner has completed the relevant analysis\n- Do not use learner_stage values such as introduction, introductory, foundation, foundational, pre_reasoning, during_reasoning, post_classification, synthesis, consolidation, overview, beginner, intermediate, or advanced\n- Activity generate example: {\"scope\":\"activity\",\"activity_id\":\"A1\",\"visual_slot\":\"materials-entry\",\"learner_stage\":\"pre_classification\",\"rationale\":\"Makes discriminating category cues inspectable before classification.\"}\n- Page Knowledge Summary example: {\"scope\":\"page\",\"visual_slot\":\"knowledge-summary-after-content\",\"learner_stage\":\"post_reasoning\",\"rationale\":\"Externalises concept relationships so learners can revisit synthesis links.\"}\n\nDownstream authority boundary:\n- No downstream prompt may invent visual purpose, representation, subject, context, or evidence anchors\n- Repair/retry passes may repair schema shape only\n\nsections[] is optional legacy dual-read output only:\n- Do not emit sections[] unless a downstream consumer explicitly requires mirrored section rows\n- When page_synthesis fields are populated, sections[] is not required\n- If sections[] is emitted, mirror page_synthesis bodies only — do not duplicate activity content\n\nExplicitly forbidden:\n- full-page replay or non-owned shell fields (audience, page_profile, learning_outcomes, episode_plans)\n- using the original request/brief phrasing as title\n- activities[] regeneration or activities[].materials[] bodies\n- Do not re-resolve materials from conversation or recopy upstream GAM material bodies\n- learning_sequence or assessment_check regeneration\n\nRuntime authorities:\n- partialPageOutputs mode: LD-DESIGN-PAGE-PARTIAL-CONTRACT (authoritative — appended at runtime)\n- rollback/legacy modes (partialPageOutputs off): LD-DESIGN-PAGE-COMPOSE-CONTRACT (compose path only)\n- wrapper-gap fallback: LD-THIN-ASSEMBLY-COHERENCE-CONTRACT when upstream wrapper bodies absent — minimal, capped\n\nContext:\nUse Copilot conversation context for upstream instructional substance (DLA, GAM, LS, LC, KM, LO). PRISM does not embed stored prior step outputs in partial mode.\n\nTask:\nEmit a partial v2 page artefact with a concise learner-facing title, page_synthesis wrapper prose, and authoritative visual planning.\n\nInstructions:\n- Treat workflow goal, desired outputs, and step notes as constraints on title and wrapper prose scope only — derive a publication title from the subject, not from request scaffolding\n- Ground page_synthesis in conversation context from upstream steps\n- Visual planning must be authored directly here; do not defer planning semantics to later prompts\n- Apply step notes: {{stepNotes}}\n\nOutput:\n- Return {{preferredOutputFormat}} only — partial page JSON: artifact_type page, schema_version 2.0.0, title, page_synthesis{}, visual_affordance_schema_version, activities_visual_review[], visual_affordances[], assembly_state{}\n- Optionally sections[] as legacy dual-read mirror\n- Do not emit audience, page_profile, activities, episode_plans, or other non-owned shell fields"
}
