...
function renderDoMoment(moment, activityId) {
  ...
  var body = items
    .map(function (item) {
      ...
      if (item.kind === "material" && item.material) {
        if (item.tableWorkspace) {
          deferredTableHtml.push(
            renderTableWorkspace(item.material, activityId, {
  ...
  var tableWorkspacesHtml = deferredTableHtml.filter(Boolean).join("");
  ...
  // Guidance before response: EO → structure hint → table workspaces → text workspaces.
  return (
  ...
  var body = items
    .map(function (item) {
      ...
      if (item.kind === "material") {
        ...
        return renderCheckMaterialItem(item, activityId);
      }
      if (item.kind === "prompt") {
        if (workspaceCoversPromptField(workspaceList, item.prompt && item.prompt.sourceField)) {
          return "";
