...
(function (root, factory) {
  ...
  /**
   * When material_role_index and materials_order are absent, order type-bucket keys
   * by 38-P role-family sequence weights (Inflation nested material_id captures).
   */
  function buildTypeBucketRoleFallbackRenderPlan(activityRow, materials) {
    if (isRolePrecedenceActive(activityRow)) return [];
    var registry = resolveRoleRegistry();
    if (!registry || typeof registry.getRoleFamilyForPageKey !== "function") return [];

    var mats = materials && typeof materials === "object" ? materials : {};
    var archetype =
      activityRow && activityRow.activity_archetype ? activityRow.activity_archetype : "default";
    var entries = [];

    Object.keys(mats).forEach(function (key) {
      if (!materialHasBody(mats[key])) return;
      var roleFamily = registry.getRoleFamilyForPageKey(key);
      if (!roleFamily) return;
      var regEntry = registry.getRegistryEntry(roleFamily);
      if (regEntry && regEntry.storage === "activity_row") return;
      var weight =
        typeof registry.resolveRoleSequence === "function"
          ? registry.resolveRoleSequence(roleFamily, archetype)
          : null;
      ...
      entries.push({
        ...
        key: key,
        heading:
          typeof registry.resolveRoleHeading === "function"
            ? registry.resolveRoleHeading(roleFamily)
            : null,
    ...
    entries.sort(function (a, b) {
      if (a.sequence_weight !== b.sequence_weight) {
    ...
    entries.forEach(function (entry) {
      if (familiesDone[entry.role_family]) return;
      plan.push({
        role_family: entry.role_family,
        key: entry.key,
        heading: entry.heading
      });
      familiesDone[entry.role_family] = true;
    });
    return plan;
  }

  function isTypeBucketRoleFallbackActive(activityRow, materials) {
    if (isRolePrecedenceActive(activityRow)) return false;
    if (
      activityRow &&
    ...
    return buildTypeBucketRoleFallbackRenderPlan(activityRow, materials).length > 0;
  }
  ...
  return {
    ...
    buildTypeBucketRoleFallbackRenderPlan: buildTypeBucketRoleFallbackRenderPlan,
    isTypeBucketRoleFallbackActive: isTypeBucketRoleFallbackActive,
