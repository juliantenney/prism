...
(function (root, factory) {
  ...
  var SKIP_REASONS = REJECTION_REASONS.slice();

  var SCOPES = ["activity", "page", "section"];

  var PAGE_REGIONS = ["knowledge_summary"];
  ...
  function collectGenerateFigureDescriptionWarnings(record, index) {
    var warnings = [];
    var prefix = "visual_affordances[" + index + "]: ";
    if (!record || typeof record !== "object") return warnings;
    if (String(record.visual_decision || "").trim() !== "generate") return warnings;
    var contract =
      typeof root !== "undefined" && root.PRISM_LEARNER_FIGURE_DESCRIPTION_CONTRACT
        ? root.PRISM_LEARNER_FIGURE_DESCRIPTION_CONTRACT
        : typeof require === "function"
          ...
          : null;
    if (contract && typeof contract.diagnoseFigureDescriptionQuality === "function") {
      var diagnosed = contract.diagnoseFigureDescriptionQuality(record, { substantive: true });
      (diagnosed.diagnostics || []).forEach(function (d) {
        warnings.push({
          code: d.code,
          severity: d.severity || "warn",
          message: prefix + d.message,
          affordance_id: String(record.affordance_id || "").trim()
        });
      });
      return warnings;
    }
    if (!isNonEmptyString(record.detailed_description)) {
      warnings.push({
    ...
    if (!isNonEmptyString(record.alt_text)) {
      warnings.push({
        code: "FIGURE_ALT_TEXT_MISSING",
        severity: "warn",
        message: prefix + "generate row should include concise alt_text (≤140 characters).",
        affordance_id: String(record.affordance_id || "").trim()
      });
    }
    return warnings;
  ...
  function validateRejectAffordance(record, index) {
    var errors = [];
    var prefix = "visual_affordances[" + index + "]: ";
    var rejectReason = isNonEmptyString(record.skip_reason)
      ? String(record.skip_reason).trim()
      : String(record.rejection_reason || "").trim();
    if (!inList(rejectReason, SKIP_REASONS)) {
      errors.push(prefix + "skip_reason (or legacy rejection_reason) must be one of: " + SKIP_REASONS.join(", "));
    }
    if (isNonEmptyString(record.purpose)) {
  ...
  function resolveAffordanceScope(record) {
  ...
  function validateAffordanceEnvelope(record, index) {
    ...
    if (scope && !inList(scope, SCOPES)) {
      errors.push(prefix + "scope must be activity | page | section");
    }
    if (!scope) {
      scope = resolveAffordanceScope(record);
    }
    if (scope === "activity" && !isNonEmptyString(record.activity_id)) {
      errors.push(prefix + "activity_id is required for activity scope");
    }
    if (scope === "section") {
      if (!isNonEmptyString(record.section_id)) {
        errors.push(prefix + "section_id is required for section scope");
      }
      if (isNonEmptyString(record.activity_id)) {
        errors.push(prefix + "activity_id must not be set when scope is section");
      }
    }
    if (scope === "page") {
      if (!isNonEmptyString(record.region)) {
        errors.push(prefix + "region is required for page scope");
      } else if (!inList(record.region, PAGE_REGIONS)) {
        errors.push(prefix + "region must be one of: " + PAGE_REGIONS.join(", "));
      }
      if (isNonEmptyString(record.activity_id)) {
        errors.push(prefix + "activity_id must not be set when scope is page");
      }
    }
    ...
    if (decision !== "generate" && decision !== "defer" && decision !== "reject") {
      errors.push(prefix + 'visual_decision must be "generate", "defer", "skip", or legacy "reject"');
      return errors;
    }
  ...
  function validatePageVisualAffordances(page) {
    ...
    records.forEach(function (record, index) {
      ...
      if (rowErrors.length) {
        ...
        warnings = warnings.concat(collectGenerateFigureDescriptionWarnings(record, index));
      }
    });
  ...
  function resolveRenderPlanScopeKey(row) {
    var scope = resolveAffordanceScope(row);
    if (scope === "page") return "page";
    if (scope === "section") {
      var sectionKey = normalizeActivityIdKey(row && row.section_id);
      return sectionKey ? "section:" + sectionKey : "";
    }
    if (!scope && row && !isNonEmptyString(row.activity_id) && isNonEmptyString(row.region)) {
      return "page";
    }
  ...
  function buildVisualAffordanceRenderPlan(page) {
    ...
    list.forEach(function (row) {
      ...
      slotGenerate[key] = {
        ...
        scope: resolveAffordanceScope(row),
        visual_slot: slot,
        purpose: row.purpose,
  ...
  return {
    ...
    TIERS: TIERS,
    DISCIPLINE_RISK_LEVELS: DISCIPLINE_RISK_LEVELS,
    resolveAffordanceScope: resolveAffordanceScope,
    ...
    collectGenerateFigureDescriptionWarnings: collectGenerateFigureDescriptionWarnings,
