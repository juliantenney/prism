...
function resolveRenderPlanScopeKey(row) {
  ...
  if (!scope) {
    if (row && String(row.section_id || "").trim()) scope = "section";
    else if (row && String(row.region || "").trim()) scope = "page";
    else scope = "activity";
  }
...
function buildVisualAffordanceRenderPlan(page) {
  ...
  list.forEach(function (row) {
    ...
    slotGenerate[key] = {
      ...
      scope: String(row.scope || "").trim() || (row.section_id ? "section" : row.region ? "page" : "activity"),
      visual_slot: slot,
      purpose: row.purpose,
