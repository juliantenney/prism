...
### Purpose
- Transport upstream artefacts into a profile-aware self-contained page container (wrapper structure and activity ordering)
- Keep wrapper/container output directly usable for HTML/VLE rendering
- Preserve explicit component, quantity, and exclusion constraints without redesigning pedagogy; `activity.materials.*` are archival copy-only payloads
...

{
  ...
  "defaultPromptStrategy": "default_template",
  "preferredOutputFormat": "json",
  "defaultPromptNotes": "Transport-and-container step (not authoring): activity.materials.* are archival copy-only from GAM Content:; readability and coherent ordering apply to wrapper sections and activity ordering only. Strict JSON sections[] (section_id, heading, content). Canonical section_ids when applicable: overview, learning_purpose, knowledge_summary, learning_activities, assessment_check, support_notes. LD-DESIGN-PAGE-COMPOSE-CONTRACT at runtime owns membership, materials fidelity (embedded LD-MATERIALS-COPY / LD-TABLE-FIDELITY preserve roles), field preservation, and portable episode_plans schema. LD-THIN-ASSEMBLY-COHERENCE-CONTRACT at runtime is the sole Layer 3 authority for bounded assembly-coherence (R-40/R-44/R-45/R-47 merged) — transport-first wrapper-gap fallback for overview/learning_purpose only when upstream body absent; minimal and capped. R-83 readable assembly (guardrail): Layer 2 structural delimiter for wrapper/container structure only — clear section boundaries, defensible ordering, verbatim transport; forbids condensation, readability rewriting, brevity optimisation, and payload polishing on archival material fields. Transport-first: knowledge_summary transport-or-omit when LC/KM bound; study_tips transport upstream closure only. Learner page_profile: LD-GUIDED-LEARNING-SCAFFOLD compose preservation. Maths: LD-MATH-RENDER. Removed authorial/journey/rhetoric modules are not current DP authorities.",
  "runnerInstructions": {
    "what_this_step_does": "This step transports upstream artefacts into a profile-aware self-contained page container for HTML/VLE use — wrapper structure and activity ordering only; activity.materials.* are archival copy-only payloads from GAM Content: (not authored page prose).",
      ...
      "choices": [
        {
          "value": "learner",
          "label": "Learner",
          "promptInstruction": "Set page_profile to learner. Transport overview/learning_purpose from upstream when bodies exist; LD-THIN-ASSEMBLY-COHERENCE-CONTRACT wrapper-gap fallback only when absent — minimal, capped navigation/sequencing (not teaching). Transport knowledge_summary when LC/KM bound. Full verbatim activity materials — never summary-only or placeholder-only materials."
        },
        {
          "value": "facilitator",
          "label": "Facilitator",
  ...
  "promptTemplate": "PRIMARY IDENTITY OF THIS STEP — TRANSPORT, NOT AUTHORING\n\nThis step is a transport-and-container step, not an educational authoring step.\n\nThe page artefact is a container for upstream records.\n\n`activity.materials.*` fields are archival payload fields.\n\nARCHIVAL DOES NOT MEAN REFERENCED\n\n`activity.materials.*` fields are archival copy zones, but archived payloads must be physically embedded in the page JSON now.\n\nA valid `activity.materials.<field>` value is only the copied upstream GAM `Content:` body itself.\n\nInvalid values include:\n- \"FULL CONTENT BODY REQUIRED FROM...\"\n- \"see Step 6\"\n- \"from upstream\"\n- \"content body required\"\n- material IDs as placeholders\n- references to prior outputs\n- descriptions of missing content\n- instructions to another system to insert content later\n\nDo not emit the page if any `activity.materials.*` value points to, names, requests, or describes an upstream material instead of containing the copied body.\n\nThe word \"archival\" means \"copy the archived record into this JSON field\", not \"reference the archive\".\n\nThe Design Page output is the final embedded container, not a manifest.\n\nWHOLE-BLOCK MATERIAL EXTRACTION\n\nFor each upstream GAM material:\n- The material body begins immediately after the literal `Content:` marker for that material.\n- The material body ends immediately before the next material record, next activity record, next step/output boundary, or end of upstream GAM output.\n- The destination `activity.materials.<field>` value must contain the entire extracted body range.\n- Copy every heading, paragraph, list, table, worked step, example, expert note, bridge task, checklist item, blank line, and markdown structure inside that range.\n- Do not copy only the first heading.\n- Do not copy only the first paragraph.\n- Do not copy only the table header.\n- Do not copy a representative opening excerpt.\n- Do not stop after the scenario introduction.\n- Do not stop after a checklist lead-in.\n- Do not stop after identifying the correct material.\n- The field value is invalid unless it contains the complete `Content:` body for that material.\n\nMaterial bodies inside `activity.materials.*` are not authorable page prose. They must not be improved, rewritten, condensed, summarised, represented, polished, shortened, restructured, made more readable, converted into descriptions, or adapted for page flow.\n\nFor `activity.materials.*`, the model's only job is to copy each upstream Generate Activity Materials `Content:` body into the matching page field.\n\nOnly wrapper/container fields may be assembled or minimally authored where upstream bodies are absent. Material fields are copy-only archival records.\n\nIf a requirement about page readability, learner-facing style, coherent assembly, section ordering, page optimisation, visual presentation, or wrapper prose conflicts with material preservation, material preservation wins.\n\nContext:\nYou are provided with learning_outcomes, learning_activities, activity_materials, episode_plans (when the workflow includes Design Episode Plan), and may also receive learning_sequence, learning_content, knowledge_model, assessment_items, feedback_pack, marking_rubric, and assessment_blueprint.\n\nUpstream source: in a multi-step Copilot workflow these artefacts are the complete outputs from prior steps in this conversation (each marked STEP N OUTPUT: <artefact_name>). The active chat history is context — artefacts do not need to be re-attached in the current message; search conversation history for activity_materials, Generate Activity Materials, Material:, and Content: before claiming bodies are unavailable. Locate and use those full prior outputs — especially the full Generate Activity Materials pack-text body for activity_materials (Activity ID / Material: / Content: blocks). For activity.materials only: treat each GAM Content: body as opaque payload (transport, not authoring) — obey LD-MATERIALS-COPY AUTHORITATIVE GAM CONTENT BINDING, MULTI-MATERIAL ENUMERATION INVARIANT, FULL CONTENT BODY PRESERVATION, MATERIAL PRESERVATION OVERRIDES PAGE OPTIMISATION, PAGE ARTEFACT IS FINAL LEARNER OUTPUT, and OPAQUE PAYLOAD TRANSPORT at runtime. Do not reconstruct material bodies from learning_activities.required_materials specs alone.\n\nTask:\nAssemble one self-contained page container with artifact_type = page. Readability and coherent ordering apply only to wrapper sections, headings, and activity ordering; they do not apply to `activity.materials.*`, which are archival copy-only payloads.\n\nInstructions:\n- Read-only transport step — assemble wrapper section structure, section headings, activity ordering, and transport slots from upstream artefacts; do not redesign pedagogy; `activity.materials.*` is copy-only archival transport\n- READABLE ASSEMBLY (R-83 guardrail — Layer 2 delimiter only): applies to wrapper/container structure only — clear section boundaries, meaningful wrapper section headings, and defensible activity ordering; does not authorise rewriting `activity.materials.*`; verbatim transport of upstream bodies; forbids condensation, summarising, paraphrasing, readability rewriting, brevity optimisation, learner-facing polish, and payload shortening for page size — never use readable assembly or generation_notes to excuse material-body loss\n- Treat workflow goal, desired outputs, and step notes as hard constraints (component types, quantities, exclusions)\n- Set page_profile using {{option:page_profile}}; include audience; ground wrapper sections in provided upstream artefacts only; exclude unsupported content\n- Page JSON must be self-contained for downstream rendering\n- Runtime canonical authorities (obey all appended blocks): LD-DESIGN-PAGE-COMPOSE-CONTRACT (activity membership, materials fidelity via embedded LD-MATERIALS-COPY and LD-TABLE-FIDELITY preserve roles, field preservation, portable episode_plans schema); LD-THIN-ASSEMBLY-COHERENCE-CONTRACT (sole Layer 3 assembly-coherence authority — R-40/R-44/R-45/R-47 merged; wrapper-gap fallback only); LD-GUIDED-LEARNING-SCAFFOLD compose preservation when appended; LD-MATH-RENDER\n- overview / learning_purpose: transport upstream bodies when present; apply LD-THIN-ASSEMBLY-COHERENCE-CONTRACT only for remaining wrapper gaps — minimal, capped; never synthesize from knowledge_model, GAM signals, or activity-row fields; removed authorial/journey/rhetoric modules are not current DP authorities\n- Use meaningful section headings for wrapper sections only; avoid schema-style labels in wrapper/container content (not inside `activity.materials.*` bodies)\n- Coherent wrapper section ordering: overview, learning purpose/outcomes, knowledge_summary when LC/KM bound, learning activities, assessment check, support notes as applicable — plus activity order in learning_activities.content[] (not rewriting material bodies)\n- learning_activities.content MUST be an array of activity objects with structured materials.<field> objects — not labels-only references\n- learning_activities.content[].materials is not an authored learner explanation. It is an archival copy zone. Each value must be copied from upstream GAM Content: exactly, with JSON escaping only.\n- If assessment_items present: Formative Assessment Check with assessment_check.content.items[]; MCQ stems and options; respect include_answers, include_marking_guidance, include_feedback_guidance options\n- If learning_sequence present: use timeline for activity order/timing only\n- Visual affordance metadata: omit visual_affordance_schema_version, activities_visual_review, and visual_affordances unless an upstream VA artefact explicitly provides them — do not generate, infer, author, or specify VA rows on Design Page\n- Record non-material constraint gaps in generation_notes.limitations — never excuse material-body loss (compose contract governs)\n- Apply step notes: {{stepNotes}}\n\nMATERIAL COMPLETENESS CHECK\n\nBefore emitting final JSON, inspect every `activity.materials.*` value.\n\nFor each value, ask:\n1. Does it include the end of the source GAM `Content:` body, not just the beginning?\n2. If the source body contains multiple sections, steps, tables, checklist bullets, expert notes, or bridge tasks, are all of them present?\n3. If the copied value ends immediately after an opening heading, opening paragraph, table header, scenario intro, or checklist lead-in, it is incomplete and must be replaced with the full source body.\n4. Never emit an incomplete material value.\n\nOutput:\n- Return {{preferredOutputFormat}} only — JSON object: artifact_type page, title, audience, page_profile, sections[], episode_plans (when upstream provided), source_artefacts, constraints_applied, generation_notes\n- Learner profile: transport overview/learning_purpose when upstream bodies exist; LD-THIN-ASSEMBLY-COHERENCE-CONTRACT wrapper-gap fallback when absent; knowledge_summary transport when LC/KM bound; full activity-linked materials (not placeholder-only)\n- Facilitator profile: run guidance and facilitation/logistics notes\n- Assessment profile: structured items section — do not flatten to prose\n- Record generation_notes.activities_omitted[] with activity_id, title, reason, authority for intentional omissions"
}
