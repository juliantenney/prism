...
(function (root, factory) {
  ...
  var guidedReviewGeneration = null;
  try {
    // eslint-disable-next-line global-require
    guidedReviewGeneration = require("./guided-review-generation-contract.js");
  } catch (_guidedRequireErr) {
    guidedReviewGeneration = null;
  }
  if (
    !guidedReviewGeneration &&
    typeof globalThis !== "undefined" &&
    globalThis.PRISM_GUIDED_REVIEW_GENERATION_CONTRACT
  ) {
    guidedReviewGeneration = globalThis.PRISM_GUIDED_REVIEW_GENERATION_CONTRACT;
  }
  if (
    !guidedReviewGeneration &&
    typeof window !== "undefined" &&
    window.PRISM_GUIDED_REVIEW_GENERATION_CONTRACT
  ) {
    guidedReviewGeneration = window.PRISM_GUIDED_REVIEW_GENERATION_CONTRACT;
  }
  ...
  function semanticMarkerSatisfied(markerId, text) {
    ...
    switch (markerId) {
      ...
      case "common_mistakes":
        return /## Common mistakes/i.test(body) || guidedChecklistPayloadSatisfied(body);
      case "revise_if_not_met":
        return /### If any check is not met:/i.test(body) || guidedChecklistPayloadSatisfied(body);
      default:
        return body.indexOf(markerId) !== -1;
    }
  }

  function guidedChecklistPayloadSatisfied(text) {
    var raw = text;
    if (typeof text === "string") {
      try {
        raw = JSON.parse(text);
      } catch (_) {
        return /"review_mode"\s*:\s*"guided_criteria"/i.test(text) &&
          /"features"\s*:\s*\[/i.test(text) &&
          /"repair"\s*:/i.test(text);
      }
    }
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) return false;
    if (String(raw.review_mode || raw.reviewMode || "").toLowerCase().replace(/[\s-]+/g, "_") !== "guided_criteria" &&
        String(raw.review_mode || raw.reviewMode || "").toLowerCase().replace(/[\s-]+/g, "_") !== "guided_review") {
      if (!Array.isArray(raw.criteria)) return false;
    }
    var criteria = Array.isArray(raw.criteria) ? raw.criteria : [];
    var complete = 0;
    criteria.forEach(function (row) {
      if (!row || typeof row !== "object") return;
      var statement = String(row.statement || row.criterion || row.question || "").trim();
      var features = Array.isArray(row.features) ? row.features : [];
      var okFeature = features.some(function (feature) {
        return (
          feature &&
          String(feature.expected || "").trim() &&
          String(feature.repair || feature.if_missing || "").trim()
        );
      });
      if (statement && okFeature) complete += 1;
    });
    return complete >= 2;
  }
  ...
  function collectGuidedReviewSoftQualityWarnings(body, label) {
    var warnings = [];
    if (
      !guidedReviewGeneration ||
      typeof guidedReviewGeneration.diagnoseGuidedReviewGenerationQuality !== "function"
    ) {
      return warnings;
    }
    var report = guidedReviewGeneration.diagnoseGuidedReviewGenerationQuality(body);
    if (!report || !report.guided) return warnings;
    (report.diagnostics || []).forEach(function (row) {
      if (!row || row.severity !== "warn") return;
      warnings.push(
        String(label || "checklist") +
          ":guided_review_quality:" +
          String(row.code || "WARN") +
          ":" +
          String(row.message || "thin guided-review treatment")
      );
    });
    return warnings;
  }
  ...
  function validate38MPageFidelity(page, options) {
    ...
    rows.forEach(function (row, index) {
      ...
      Object.keys(mats).forEach(function (key) {
        if (!/checklist/i.test(key)) return;
        var label = (row.activity_id || "A" + (index + 1)) + ":" + key;
        collectGuidedReviewSoftQualityWarnings(mats[key], label).forEach(function (warn) {
          warnings.push(warn);
        });
      });
    });
  ...
  return {
    ...
    collectGuidedReviewSoftQualityWarnings: collectGuidedReviewSoftQualityWarnings,
