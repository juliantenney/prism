...
(function (root, factory) {
  ...
  function () {
    ...
    function parseExpositoryArtefact(raw, stage) {
      ...
      if (!contracts || typeof contracts.normalizeExpositoryArtefact !== "function") {
        throw assemblyStageError("Expository contracts module unavailable for " + stage);
      }
      var normalized = contracts.normalizeExpositoryArtefact(parsed, stage);
      if (!normalized) {
        throw assemblyStageError(stage + " must be a native Expository artefact");
      }
      if (String(normalized.artifact_type || "") !== stage) {
