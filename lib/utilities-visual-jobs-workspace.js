...
(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    root.PRISM_UTILITIES_VISUAL_JOBS_WORKSPACE = factory(contract, planner, compiler, visualAssets);
  }
})(
  typeof globalThis !== "undefined" ? globalThis : this,
  function (contractMod, plannerMod, compilerMod, assetsMod) {
    "use strict";
    ...
    if (
      ...
      throw new Error(
        "utilities-visual-jobs-workspace requires visual planning pipeline and visual asset modules"
      );
    }
    ...
    function buildEducationalModeLines(brief, mode) {
      if (mode === HUMAN_PROMPT_MODES.knowledge_synthesis) {
        return [
          ...
          "Reveal system organisation and integrate relationships across the lesson.",
          "Improve understanding through clear visual organisation.",
          "The image should explain concepts, not set learner tasks.",
          "Do not invent unsupported concepts beyond the authored Knowledge Summary brief."
        ];
      }
      var lines = [
    ...
    function buildConceptBoundaryLines(mode) {
      if (mode !== HUMAN_PROMPT_MODES.activity_learning_support) return [];
      return [
        "Only depict the concepts supplied in this brief.",
        "Use only the concepts explicitly listed in the supplied brief.",
        "Do not introduce additional concepts, relationships, categories, examples, historical details, or outcomes.",
        "Do not invent additional concepts, subtopics, historical actors, locations, institutions, technologies, or consequences."
      ];
    }
    ...
    function buildTextDensityLines(token) {
      var lines = [
    ...
    function buildVisualJobHumanPrompt(brief) {
      ...
      var allowed = asStringList(brief.claim_constraints && brief.claim_constraints.allowed);
      var disallowed = asStringList(brief.claim_constraints && brief.claim_constraints.disallowed);
      var purposeText = normalizePurposeText(brief);
      var subject = String(brief.subject || "").trim();
      var context = normalizeContextText(brief);
      ...
      if (subject) {
        pushSection(lines, "Subject:", [subject]);
      }
      ...
      if (conceptBoundary.length) {
        pushSection(lines, "Concept boundary:", conceptBoundary);
      }
      ...
      pushSection(
        lines,
        "Text and labels:",
        buildTextDensityLines(token).map(function (item) {
          return "- " + item;
        })
      );
      ...
      disallowed.forEach(function (claim) {
        claimLines.push("- Do not claim: " + claim);
      });
      allowed.forEach(function (claim) {
        claimLines.push("- Supported claim boundary: " + claim);
      });
      claimLines.push(
        "Use only claims supported by the supplied brief. Do not invent unsupported factual detail."
      );
      pushSection(lines, "Claim discipline:", claimLines);
      ...
      if (brief.caption_guidance) {
        pushSection(lines, "Caption guidance:", [
          String(brief.caption_guidance).trim(),
          "Keep any on-image caption labels short."
        ]);
      }
    ...
    function diagnoseHumanPrompt(prompt, brief) {
      ...
      return {
        ...
        concept_boundary_present: isActivity
          ? /Only depict the concepts supplied in this brief/i.test(text) &&
        ...
        relationship_visualisation_present: isActivity
          ...
          : true,
        no_extra_concepts_instruction_present: isActivity
          ? /Do not introduce additional concepts/i.test(text)
          : !/Do not introduce additional concepts/i.test(text),
    ...
    function buildVisualJobsPipelineFromPage(page) {
      ...
      var contractResult = contractMod.validateVisualPlanningContract(page);
      var plannerResult = plannerMod.planPrismVisualJobs(page);
      var compilerResult = compilerMod.compilePrismImageBriefs(plannerResult);
      return {
        assembledPageSnapshot: deepClone(page),
    ...
    function visualSlotRank(slot) {
      var key = String(slot || "").trim();
      var index = VISUAL_SLOT_ORDER.indexOf(key);
    ...
    function buildVisualJobsOrderedList(workspaceState) {
      ...
      return briefs.sort(function (a, b) {
        var ai = typeof a.index === "number" ? a.index : 0;
        var bi = typeof b.index === "number" ? b.index : 0;
        if (ai !== bi) return ai - bi;
        var slotDelta = visualSlotRank(a.visual_slot) - visualSlotRank(b.visual_slot);
        if (slotDelta !== 0) return slotDelta;
        return String(a.brief_id || "").localeCompare(String(b.brief_id || ""));
      });
    }
    ...
    function attachVisualAssetToWorkspace(workspaceState, briefId, imageInput, options) {
      ...
      if (!brief) {
        return { ok: false, code: "unknown_brief_id", message: "Selected visual job was not found." };
      }
    ...
    function buildVisualJobsWorkspaceViewModel(workspaceState) {
      ...
      return {
        ...
        activeView:
          workspaceState && workspaceState.activeView === OUTPUT_VIEWS.visual_jobs
            ? OUTPUT_VIEWS.visual_jobs
            : OUTPUT_VIEWS.learner_page,
    ...
    function renderPipelineDetails(workspaceState, vm) {
      ...
      if (vm.summary.partialCompilation) {
        parts.push(
          '<p class="util-vj-warning">Partial compilation: some visual jobs could not be compiled.</p>'
        );
      }
    ...
    function renderWorkspaceSummary(vm) {
      ...
      if (vm.summary.partialCompilation && vm.status !== "success") {
        parts.push(
          '<p class="util-vj-warning"><strong>Some visual jobs could not be compiled.</strong></p>'
        );
      }
      parts.push("</div>");
      return parts.join("");
    ...
    function renderVisualJobsListPane(vm) {
      var parts = [
        '<section class="util-vj-pane util-vj-pane-list" aria-label="Images to create">',
        ...
        '<p class="util-vj-progress">' +
          ...
          " images attached</p>",
        '<div class="util-vj-list" role="listbox" aria-label="Visual jobs">'
      ];
    ...
    function renderSelectedJobPane(vm) {
      ...
      if (!card) {
        return '<section class="util-vj-pane util-vj-pane-detail"><p class="util-vj-empty">Select a visual job.</p></section>';
      }
      var promptExpanded =
        !!(
          vm &&
    ...
    function renderVisualJobsWorkspaceHtml(workspaceState) {
      ...
      var parts = ['<div class="util-vj-workspace-inner">', "<h3>Visual Jobs</h3>"];

      if (vm.status === "no_assembled_page") {
        parts.push(
          '<p class="util-vj-empty">Assemble or preview a page to see its visual jobs.</p>'
        );
        parts.push("</div>");
        return parts.join("");
      }
