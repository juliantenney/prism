 ...
 *
 * Outputs:
 * - lib/learner-renderer-vnext-browser.js (full renderer API for Prism shell)
...
function writeSourceModule(outPath, bundleSource) {
  var payload =
    "/**\n" +
...
function main() {
  TARGETS.forEach(function (target) {
    ...
    fs.writeFileSync(target.outPath, bundle, "utf8");
    console.log("Wrote " + path.relative(repoRoot, target.outPath) + " (" + bundle.length + " bytes)");
    if (target.writeSourceModule && target.sourceOutPath) {
      writeSourceModule(target.sourceOutPath, bundle);
      console.log(
        "Wrote " +
          path.relative(repoRoot, target.sourceOutPath) +
          " (" +
          fs.statSync(target.sourceOutPath).size +
          " bytes)"
      );
    }
  });
