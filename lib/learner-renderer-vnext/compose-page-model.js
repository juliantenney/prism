...
function buildComposedPageModel(modelResult, sourcePage, options) {
  ...
  (Array.isArray(model.activities) ? model.activities : []).forEach(function (modelActivity) {
    ...
    var transferMoment = composeActivityMoments.composeTransferMoment(modelActivity);
    orientMoment = composeActivityMoments.absorbUnassignedOrientationBeatContent(
      ...
      modelActivity,
      {
        ...
        transferMoment: transferMoment
      }
    );

    if (!orientMoment && !learnMoment && !doMoment && !checkMoment && !transferMoment) {
    ...
    var checkHints = composeActivityMoments.checkRenderHintsFromItems(
      {
        kind: "check",
        items: (checkMoment && checkMoment.items ? checkMoment.items : []).concat(
          transferMoment && transferMoment.items ? transferMoment.items : []
        )
      },
      doHints,
      beatAnchors.check
    ...
    if (transferMoment) moments.push(transferMoment);
...
function buildActivityCompositionMap(composed) {
  ...
  composed.activities.forEach(function (activity) {
    ...
    (activity.moments || []).forEach(function (moment) {
      ...
      if (moment && moment.kind === "transfer") transferMoment = moment;
    });
