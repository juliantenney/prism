...
function buildHookDescriptor(slot, plan, activityId, subject) {
  ...
  return {
    ...
    sectionId: resolved ? String(resolved.section_id || "").trim() : "",
...
/**
 * Section-scoped hook: resolve only against section:<id> plan keys.
 * Never remaps onto activity slots.
 *
 * @param {string} slot
 * @param {Object} plan
 ...
 * @returns {import("./types").VisualAffordanceHook|null}
 */
function buildSectionHookDescriptor(slot, plan, sectionId, subject) {
  var sid = String(sectionId || "").trim();
  if (!sid) return null;
  var scopeKey = "section:" + sid;
  var resolved = plan && !plan.legacy ? sprint38.resolveSlotGenerate(plan, scopeKey, slot) : null;
  if (!resolved) return null;
  var resolvedSectionId = String(resolved.section_id || "").trim() || sid;
  if (sprint38.normalizeActivityIdKey(resolvedSectionId) !== sprint38.normalizeActivityIdKey(sid)) {
    return null;
  }
  return {
    slot: slot,
    ...
    affordanceId: String(resolved.affordance_id || "").trim(),
    subject: String(subject || "").trim()
  };
}
...
function attachVisualAffordancePlacements(page, model) {
  ...
  arrayOrEmpty(model.expositionSections).forEach(function (section) {
    if (!section || typeof section !== "object") return;
    var sectionId = String(section.id || "").trim();
    if (!sectionId) {
      section.visualAffordanceAfterContent = null;
      return;
    }
    var scopeKey = "section:" + sectionId;
    var subject = String(section.title || "").trim();
    if (
      shouldEmitHook(plan, scopeKey, "section-after-content", function () {
        return false;
      })
    ) {
      section.visualAffordanceAfterContent = buildSectionHookDescriptor(
...
module.exports = {
  ...
  buildSectionHookDescriptor: buildSectionHookDescriptor
};
