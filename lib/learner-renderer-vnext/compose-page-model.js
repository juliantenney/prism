...
var DEFAULT_MOMENTS_ACTIVITY_IDS = Object.freeze(["A1", "A2", "A3", "A4", "A5"]);

function normalizeActivityIds(options) {
  ...
  return DEFAULT_MOMENTS_ACTIVITY_IDS.slice();
}
 ...
 * @returns {{ ok: boolean, composed: import("./types").ComposedLearnerPageModel|null, errors: Object[], warnings: Object[] }}
 */
function buildComposedPageModel(modelResult, sourcePage, options) {
  ...
  var targetActivityIds = normalizeActivityIds(opts);
  ...
  (Array.isArray(model.activities) ? model.activities : []).forEach(function (modelActivity) {
    ...
    if (!orientMoment && !learnMoment && !doMoment && !checkMoment) return;

    var beatAnchors = composeActivityMoments.momentBeatAnchorsForActivity(activityId);
    if (!beatAnchors) return;
    ...
    renderHints.omitBeatFunctions = renderHints.omitBeatFunctions.concat(
      composeActivityMoments.activityOmitBeatFunctions(activityId)
    );
...
module.exports = {
  DEFAULT_MOMENTS_ACTIVITY_IDS: DEFAULT_MOMENTS_ACTIVITY_IDS,
