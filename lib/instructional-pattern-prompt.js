 ...
 * Operationalises Pattern Library SP-01, SP-02, SP-03, SP-04, SP-05, and SP-06 for GAM material authoring.
 */
(function (root, factory) {
  ...
  var SP01_LINES = [
    ...
    "  [Concept A] — brief definitional prose with application meaning.",
    ...
    "  **Example:** Brief applied illustration bridging concepts to a concrete case — do not pre-answer the learner's independent task.",
  ...
  var SP05_LINES = [
    ...
    "- MUST include explicit revise guidance when any check may fail.",
    ...
    "  Revise your [deliverable] by [specific remediation tied to the failed checks].",
  ...
  function buildInstructionalPatternPromptBlock(options) {
    var opts = options && typeof options === "object" ? options : {};
    var parts = [];
    if (!opts.sp03Only && !opts.sp05Only && !opts.sp06Only && !opts.sp04Only && !opts.sp01Only) {
      parts.push(buildSp02PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp05Only && !opts.sp06Only && !opts.sp04Only && !opts.sp01Only) {
      parts.push(buildSp03PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp03Only && !opts.sp05Only && !opts.sp04Only && !opts.sp01Only) {
      parts.push(buildSp06PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp03Only && !opts.sp05Only && !opts.sp06Only && !opts.sp01Only) {
      parts.push(buildSp04PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp03Only && !opts.sp04Only && !opts.sp06Only && !opts.sp01Only) {
      parts.push(buildSp05PromptBlock(opts));
    }
    if (!opts.sp02Only && !opts.sp03Only && !opts.sp04Only && !opts.sp05Only && !opts.sp06Only) {
