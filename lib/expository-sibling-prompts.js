...
(function (root, factory) {
  ...
  var EJP = [
    ...
    "- Do NOT return a learning_outcomes root object (Define Learning Outcomes shape). Upstream LO is an input, not this step's output.",
    "- Do NOT emit only learning_outcomes / learner_level / scope / alignment_notes without the Expository Journey Plan envelope below",
    "",
    "Step notes: {{stepNotes}}",
    "",
    "Required envelope (authoritative — this step's output shape):",
    "- Top-level object MUST be an Expository Journey Plan, not learning_outcomes.",
    "- Required top-level keys:",
    "  - artifact_type: \"expository_journey_plan\"",
    "  - title (string)",
    "  - audience (string)",
    "  - journey_intent (string)",
    "  - commissioned_purpose (required non-empty free text — EQ1)",
    "  - epistemic_form (required non-empty free text — EQ1; not an enum)",
    "  - sections (required non-empty array — the intellectual journey)",
    "- Optional top-level: extent {scope_text, words_equivalent?, reading_minutes?, interpretation, qualitative_hint}, learning_outcomes (references only; do not substitute for sections[]), generation_notes, schema_version",
    ...
    "    \"epistemic_form\": \"...\",",
    "    \"sections\": [ { \"section_id\": \"S1\", \"title\": \"...\", \"purpose\": \"...\", \"knowledge_focus\": \"...\", \"conceptual_move\": \"...\", \"dependencies\": [], \"lo_refs\": [], \"elaboration_intentions\": [], \"representation_needs\": [], \"connections\": [], \"synthesis\": \"...\", \"order\": 1 } ]",
    "  }",
    "",
    FENCE,
    ...
    "- Do NOT return the Define Learning Outcomes capture shape as this step's output",
    "- Return the fenced block first, then the single STEP N OUTPUT footer line."
  ].join("\n");
  ...
  var XD = [
    ...
    "",
    "Expository register (EQ6 — restrained):",
    "- Make importance apparent through explanation, examples, qualifications, and paragraph development rather than repeatedly announcing it.",
    "- Write with the confidence and continuity of an author developing an argument or explanation, not as a facilitator commenting on the act of learning. Let connections, contrasts, consequences, examples, and qualifications carry the reader forward.",
    "- Rhetorical confidence must not become epistemic overconfidence: distinguish what is established, inferred, contested, uncertain, conditional, or interpretive whenever the subject requires it.",
    "- Prefer direct explanatory transitions over recurrent stock metacommentary that merely signals significance (for example repeatedly framing moves as \"the important point\", \"the key message\", or \"this distinction matters because\" when the intellectual relationship is already clear).",
    "- Vary rhetorical movement naturally across the resource; do not rely on a small set of teacherly / interpretive-check phrasings as a default cadence.",
    "- Preserve useful conceptual signposting, genuine misconception warnings, novice-audience transitions, and statements of significance when significance itself needs explanation or disciplinary clarity requires it.",
    "- Do not ban particular phrases mechanically; do not make prose terse or literary for its own sake.",
    "",
    "Richness:",
    ...
    "- A commission should normally have a distinct explanatory or perceptual function — for example organising relationships, comparing cases, mapping concepts, making sequence or causality perceptible, exposing structure, showing correspondence, supporting discrimination, making quantities/patterns easier to perceive, or providing a useful alternative representation.",
    "- Do not commission a material merely to restate an explanation that the section exposition has already completed in another component.",
    "- Accessibility semantic equivalence is not editorial redundancy: essential meaning must remain available in prose/textual form where required. Prefer forms that add warrantable explanatory or perceptual value while keeping prose sufficient.",
  ...
  var DP = [
    ...
    "- Emit a figure only when the form is warranted and adds explanatory or perceptual value beyond restating completed section prose (for example making relationships, sequence, correspondence, comparison, structure, or quantities more perceptible).",
    "- Accessibility semantic equivalence is not editorial redundancy: keep essential meaning available in prose/alt/detailed_description; do not commission a figure merely as a second copy of the same completed explanation.",
    "- When a figure depicts relationships among named entities, authorise each connector explicitly (must_show pairs such as \"Condition absent → False positives\", and/or evidence_anchors that include upstream structured relationships). Do not rely on spatial adjacency or parallel-list order to invent connectors.",
