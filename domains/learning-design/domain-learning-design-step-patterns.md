...

{
  ...
  "defaultPromptStrategy": "default_template",
  "preferredOutputFormat": "json",
  "defaultPromptNotes": "vNext partial-page step (Sprint 58): canonical output is page_synthesis on a partial page artefact — not a full-page replay. Required: page_synthesis.knowledge_summary (mandatory), page_synthesis.overview, page_synthesis.learning_purpose, page_synthesis.study_tips, assembly_state.current_stage design_page, assembly_state.enriched_by includes design_page. activities[] and activities[].materials[] already exist upstream from DLA/GAM partial captures — do not regenerate or re-copy from chat. sections[] is optional legacy dual-read mirror only — not canonical transport. LD-DESIGN-PAGE-PARTIAL-CONTRACT is authoritative at runtime when partialPageOutputs mode is enabled. LD-DESIGN-PAGE-COMPOSE-CONTRACT applies only to rollback/legacy modes (partialPageOutputs off). LD-THIN-ASSEMBLY-COHERENCE-CONTRACT: wrapper-gap fallback for overview/learning_purpose when upstream body absent — minimal, capped. Maths: LD-MATH-RENDER when appended. Removed authorial/journey/rhetoric modules are not current DP authorities.",
  "runnerInstructions": {
    ...
    "what_to_check": "page_synthesis object with knowledge_summary mandatory; overview, learning_purpose, study_tips when upstream/conversation provides substance; assembly_state.current_stage design_page; assembly_state.enriched_by includes design_page; sections[] optional legacy dual-read mirror only when emitted; no activities[] regeneration; no activity.materials.* recopy; no shell fields (title, audience, page_profile, learning_outcomes, episode_plans); LD-DESIGN-PAGE-PARTIAL-CONTRACT governs partialPageOutputs mode; LD-DESIGN-PAGE-COMPOSE-CONTRACT governs rollback/legacy only; learner-facing text excludes internal enum values."
  },
  "defaultOutputStructure": {
    "keys": [
      "artifact_type",
