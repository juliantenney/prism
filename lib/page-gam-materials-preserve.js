...
(function (root, factory) {
  ...
  function resolveFidelityContract(pageKey, mat) {
    ...
    var type = String((mat && mat.type) || "").toLowerCase();
    if (type === "checklist") {
      return { material_id: mat && mat.material_id, tier: "E", minRatio: 0, markers: [] };
    }
  ...
  function markersPresent(body, markers) {
    ...
    return markers.every(function (marker) {
      if (marker === "Depth" || marker === "Adaptability" || marker === "Transferability") {
  ...
  function tableShellCollapse(pageBody, pageKey) {
    var key = String(pageKey || "");
    if (!/guided_judgement_table|decision_table/.test(key)) return false;
    var text = String(pageBody || "");
    if (!/\|[^\n]+\|/.test(text)) return false;
    if (!/Partial example/i.test(text)) return false;
    if (/A:\s*Budget Tightening/i.test(text)) return false;
    if (/Budget Tightening/i.test(text) && /Moderate:|Focuses on spending/i.test(text)) {
      return false;
    }
    return true;
  }
  ...
  function isSynopsisOrShell(pageBody, gamBody, pageKey, spec) {
    return (
      ...
      tableShellCollapse(pageBody, pageKey) ||
    ...
    var minRatio =
      spec && spec.minRatio != null
        ...
        : TIER_DEFAULT_MIN_RATIO[tier] != null
          ...
          : 0.99;
    if (tier === "E") {
      return stringLooksPlaceholderOnly(pageBody);
    }
  ...
  function classifyLossType(pageBody, gamBody, pageKey, spec) {
    ...
    if (tableShellCollapse(page, pageKey)) return "table_shell_collapse";
  ...
  function applyGamMaterialsToComposedPage(page, upstreamGam, options) {
    ...
    rows.forEach(function (row, index) {
      ...
      row.materials = mergeMaterialsFromGamList(row.materials, gamMatList);
      mergedCount += 1;
    });
  ...
  function measurePageGamFidelity(page, options) {
    ...
    rows.forEach(function (row, index) {
      ...
      gamMaterialsForActivity(gamAct).forEach(function (mat) {
        var pageKey = pageFieldKeyForMaterial(mat);
        var gamBody = materialContent(mat);
        var pageBody = String(pageMats[pageKey] || "");
        ...
        var markersFound = [];
        var markersMissing = [];
        (spec.markers || []).forEach(function (marker) {
          if (pageBody.indexOf(marker) !== -1) markersFound.push(marker);
          else markersMissing.push(marker);
        });
  ...
  function validate38MPageFidelity(page, options) {
    ...
    if (m14) {
      ...
      if (m14.markersMissing.indexOf("Weak Evaluation Example") !== -1) {
        gateError("A4", m14.material_id, "G4", "missing marker Weak Evaluation Example");
      }
      if (m14.markersMissing.indexOf("Strong Evaluation Example") !== -1) {
        gateError("A4", m14.material_id, "G4", "missing marker Strong Evaluation Example");
      }
    }
    ...
    if (m15) {
      ...
      if (rows[3] && rows[3].materials) {
        guidedBody = String(
          rows[3].materials.guided_judgement_table || rows[3].materials.decision_table || ""
        );
      }
      if (tableShellCollapse(guidedBody, "guided_judgement_table")) {
        gateError("A4", m15.material_id, "G5", "table shell collapse detected");
      }
      if (m15.markersMissing.indexOf("Budget Tightening") !== -1) {
        gateError("A4", m15.material_id, "G5", "missing Budget Tightening exemplar");
      }
    }

    var m12 = metricFor("M12_Scenario_Maya_Strategy_Menu");
    if (m12) {
      if (m12.markersMissing.indexOf("Strategy A:") !== -1) {
        gateError("A4", m12.material_id, "G3", "missing Strategy A:");
      }
      if (m12.markersMissing.indexOf("Strategy E:") !== -1) {
    ...
    if (m18) {
      ...
      if (m18.markersMissing.indexOf("at least 80 words") !== -1) {
        gateError("A4", m18.material_id, "G6", "missing at least 80 words");
      }
    }
    ...
    ["M12_Scenario_Maya_Strategy_Menu", "M14_Worked_Judgement_Weak_Strong", "M15_Guided_Judgement_Table"].forEach(function (mid) {
      ...
      var pageBody = rows[3].materials ? rows[3].materials[pageKey] : "";
