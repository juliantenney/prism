...
(function (root, factory) {
  ...
  /**
   * Sprint 62 — learner-visible phase labels (presentation only).
   * Planning/diagnostics continue to use EPISODE_FUNCTION_LABELS.
   * Fallback for unknown functions: existing episodeFunctionLabel (structural),
   * else title-cased function name.
   */
  var LEARNER_FACING_EPISODE_FUNCTION_LABELS = {
    orientation: "Reflect",
    framing: "Reflect",
    activation: "Reflect",
    explanation: "Understand",
    observation: "Understand",
    criteria_exposition: "Understand",
    example: "See it modelled",
    worked_example: "See it modelled",
    worked_thinking: "See it modelled",
    worked_judgement: "See it modelled",
    non_example: "See it modelled",
    comparison: "See it modelled",
    guided_practice: "Your turn",
    guided_reasoning: "Your turn",
    guided_inquiry: "Your turn",
    independent_performance: "Your turn",
    application: "Your turn",
    analysis: "Your turn",
    practice: "Your turn",
    evaluation: "Your turn",
    revision: "Your turn",
    guided_judgement: "Your turn",
    verification: "Check your work",
    check_understanding: "Check your work",
    evaluative_judgement: "Check your work",
  ...
  var RENDERER_MATERIAL_META_KEYS = {
    _material_ids: true,
    _material_types: true,
    _render_sequence: true
  };

  function isRendererMaterialMetaKey(key) {
    return !!RENDERER_MATERIAL_META_KEYS[String(key || "")];
  }
  ...
  function materialKeysFromObject(materials) {
    ...
    return Object.keys(materials).filter(function (key) {
      if (isRendererMaterialMetaKey(key)) return false;
      return !utilityIsEmptyish(materials[key]);
    });
  }

  /**
   * Match renderer materialTypeHintForStorageKey: prefer _material_types sidecar.
   */
  function materialTypeForStorageKey(materials, storageKey) {
    var key = String(storageKey || "");
    if (
      materials &&
      materials._material_types &&
      materials._material_types[key] != null &&
      String(materials._material_types[key]).trim()
    ) {
      return normalizeMaterialType(materials._material_types[key]);
    }
    return normalizeMaterialType(key);
  }
  ...
  function learnerFacingEpisodeFunctionLabel(episodeFunction) {
    var fn = normalizeEpisodeFunction(episodeFunction);
    if (!fn) return "";
    if (LEARNER_FACING_EPISODE_FUNCTION_LABELS[fn]) {
      return LEARNER_FACING_EPISODE_FUNCTION_LABELS[fn];
    }
    return episodeFunctionLabel(fn);
  }
  ...
  /**
   * Authored per-beat instruction from episode_plan.beats[] row.
   * Prefer explicit beat fields; do not invent from activity.learner_task here.
   */
  function resolveBeatRowInstruction(beatRow) {
    if (!beatRow || typeof beatRow !== "object" || Array.isArray(beatRow)) return null;
    var keys = ["what_to_do", "whatToDo", "instruction", "instructions"];
    for (var i = 0; i < keys.length; i += 1) {
      var key = keys[i];
      if (!Object.prototype.hasOwnProperty.call(beatRow, key)) continue;
      if (utilityIsEmptyish(beatRow[key])) continue;
      return beatRow[key];
    }
    return null;
  }

  function defaultEpisodeFunctionForRendererBeat(rendererBeat) {
    var beat = normalizeBeat(rendererBeat);
    if (beat === "READ") return "explanation";
    if (beat === "EXAMPLE") return "worked_thinking";
    if (beat === "PRACTICE") return "guided_practice";
    if (beat === "CHECK" || beat === "EVALUATE") return "verification";
    if (beat === "REFLECT") return "reflection";
    if (beat === "DISCUSS") return "guided_inquiry";
    if (beat === "CREATE") return "guided_practice";
    return "guided_practice";
  }

  function buildBeatGroupFromRow(beatRow, fn, matchedKeys) {
    return {
      beat: fn,
      title: String(
        (beatRow && (beatRow.title || beatRow.heading || beatRow.label)) || ""
      ).trim(),
      label: episodeFunctionLabel(fn),
      what_to_do: resolveBeatRowInstruction(beatRow),
      whatToDo:
        beatRow && Object.prototype.hasOwnProperty.call(beatRow, "whatToDo")
          ? beatRow.whatToDo
          : null,
      instruction:
        beatRow && Object.prototype.hasOwnProperty.call(beatRow, "instruction")
          ? beatRow.instruction
          : null,
      instructions:
        beatRow && Object.prototype.hasOwnProperty.call(beatRow, "instructions")
          ? beatRow.instructions
          : null,
  ...
  function buildSyntheticBeatGroup(materialKey) {
    var entry = getRegistryEntry(materialKey);
    var rendererBeat = entry ? normalizeBeat(entry.beat) : "PRACTICE";
    var fn = defaultEpisodeFunctionForRendererBeat(rendererBeat);
    return {
      beat: fn,
      title: "",
      label: episodeFunctionLabel(fn),
      ...
      instruction: null,
      instructions: null,
      renderer_beat: rendererBeat,
      materials: [materialKey],
      synthetic: true
    };
  }

  function rendererBeatStageOrder(rendererBeat) {
    var beat = normalizeBeat(rendererBeat);
    var order = {
      READ: 10,
      NOTE: 15,
    ...
    return Object.prototype.hasOwnProperty.call(order, beat) ? order[beat] : 35;
  }

  function insertBeatGroupByStageOrder(groups, newGroup) {
    var key = rendererBeatStageOrder(newGroup && newGroup.renderer_beat);
    var insertAt = groups.length;
    for (var i = 0; i < groups.length; i += 1) {
      if (rendererBeatStageOrder(groups[i].renderer_beat) > key) {
        insertAt = i;
        break;
      }
    }
  ...
  function findGroupIndexForRendererBeat(groups, rendererBeat, preferEmpty) {
    var beat = normalizeBeat(rendererBeat);
    if (!beat) return -1;
    var emptyMatch = -1;
    var anyMatch = -1;
    for (var i = 0; i < groups.length; i += 1) {
      if (groups[i].renderer_beat !== beat) continue;
      if (anyMatch < 0) anyMatch = i;
      if (preferEmpty && !(groups[i].materials && groups[i].materials.length) && emptyMatch < 0) {
        emptyMatch = i;
      }
    }
    if (preferEmpty && emptyMatch >= 0) return emptyMatch;
    return anyMatch;
  }

  /**
   * Resolve episode-plan beats to owning material keys (registry lookups only).
   * Returns beat groups in episode-plan order, with stage-ordered synthetic
   * groups only for materials whose pedagogic stage is absent from the plan.
   */
  function resolveBeatMaterialPlan(activity) {
    ...
    beatRows.forEach(function (beatRow) {
      ...
      groups.push(buildBeatGroupFromRow(beatRow, fn, matchedKeys));
    });

    // Pass 2: materials without episodeFunctions → matching renderer beat.
    keys.forEach(function (key) {
      if (assigned[key]) return;
    ...
    // Pass 3: remaining materials → only onto existing groups with the same
    // renderer beat (prefer empty). Never attach EXAMPLE/PRACTICE to CHECK here.
    keys.forEach(function (key) {
      if (assigned[key]) return;
      var entry = getRegistryEntry(key);
      if (!entry) return;
      var target = findGroupIndexForRendererBeat(
        groups,
        normalizeBeat(entry.beat),
        true
      );
      if (target < 0) return;
      groups[target].materials.push(key);
      assigned[key] = groups[target].beat;
    });

    // Pass 4: true stage orphans become their own learner-facing beats.
    // Do not collapse worked examples / practice into checking stages.
    keys.forEach(function (key) {
      if (assigned[key]) return;
      var entry = getRegistryEntry(key);
      var materialBeat = entry ? normalizeBeat(entry.beat) : "PRACTICE";
      var existing = findGroupIndexForRendererBeat(groups, materialBeat, false);
      if (existing >= 0) {
        groups[existing].materials.push(key);
        assigned[key] = groups[existing].beat;
        return;
      }
      var synthIdx = -1;
      for (var j = 0; j < groups.length; j += 1) {
        if (groups[j].synthetic && groups[j].renderer_beat === materialBeat) {
          synthIdx = j;
          break;
        }
      }
      if (synthIdx >= 0) {
        groups[synthIdx].materials.push(key);
        assigned[key] = groups[synthIdx].beat;
        return;
      }
      var synthetic = buildSyntheticBeatGroup(key);
      insertBeatGroupByStageOrder(groups, synthetic);
      assigned[key] = synthetic.beat;
    });
  ...
  function diagnoseActivityBeatMaterialCoverage(activity, options) {
    ...
    var materials =
      activity && activity.materials && typeof activity.materials === "object"
        ...
        : {};
    var keys = materialKeysFromObject(materials);
    var renderPlan = resolveBeatMaterialPlan(activity);
    var renderAssignedKeys = {};
    var beatCoverage = {};
    var assignedByBeat = {};
    var unassignedMaterials = [];
    var conflicts = [];

    renderPlan.beats.forEach(function (group) {
      var rendererBeat = group.renderer_beat;
      if (!rendererBeat) return;
      (group.materials || []).forEach(function (key) {
        renderAssignedKeys[key] = group.beat;
        if (!beatCoverage[rendererBeat]) beatCoverage[rendererBeat] = [];
        beatCoverage[rendererBeat].push(key);
        if (!assignedByBeat[rendererBeat]) assignedByBeat[rendererBeat] = [];
        assignedByBeat[rendererBeat].push(key);
      });
    });

    keys.forEach(function (key) {
      var materialType = materialTypeForStorageKey(materials, key);
      var resolution = resolveMaterialBeat(materialType, materialPreviewText(materials[key]), opts);
      if (renderAssignedKeys[key]) {
        return;
      }
    ...
    if (episodePlan && Array.isArray(episodePlan.beats)) {
      episodePlan.beats.forEach(function (beatRow, index) {
        ...
        var planGroup = null;
        for (var gi = 0; gi < renderPlan.beats.length; gi += 1) {
          if (renderPlan.beats[gi].beat === fn) {
            planGroup = renderPlan.beats[gi];
            break;
          }
        }
        var covered = planGroup && planGroup.materials && planGroup.materials.length;
        if (!covered) {
          emptyBeats.push({
            activity_id: activityId,
            beat: rendererBeat,
  ...
  return {
    ...
    learnerFacingEpisodeFunctionLabel: learnerFacingEpisodeFunctionLabel,
    EPISODE_FUNCTION_LABELS: EPISODE_FUNCTION_LABELS,
    LEARNER_FACING_EPISODE_FUNCTION_LABELS: LEARNER_FACING_EPISODE_FUNCTION_LABELS,
    ...
    isRendererMaterialMetaKey: isRendererMaterialMetaKey,
    materialTypeForStorageKey: materialTypeForStorageKey,
    materialKeysFromObject: materialKeysFromObject
  };
});
