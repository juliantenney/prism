...
(function (root, factory) {
  ...
  function (contractMod, plannerMod, compilerMod, assetsMod, houseVisualMod, precisionFidelityMod, groundingMod) {
    ...
    function briefRequiresExactDataMatch(brief) {
      return precisionFidelityMod.briefRequiresExactDataMatch(brief);
    }
    ...
    function buildEducationalModeLines(brief, mode) {
      ...
      var exactMatch = briefRequiresExactDataMatch(brief);
      var lines = [
      ...
      if (exactMatch) {
        // When Show / must_show authorises exact numerics, do not treat those values as
        // forbidden "answer-key" content — that conflict blanks labelled result fields.
        lines.push(
          "Authorised Show / must_show values (including numeric results) are required on-image content — render them; do not leave labelled value fields blank."
        );
        lines.push(
          "Do not add answer-key content beyond Show / must_show and allowed_claims."
        );
      } else {
        lines.push(
    ...
    function collectBriefPrecisionClaims(brief) {
      ...
      return precisionFidelityMod.collectPrecisionCriticalClaims({
        ...
        mustShowItems: asStringList(
          brief && brief.content_requirements && brief.content_requirements.authored
        ),
    ...
    function buildMaterialRolePolicyLines(brief) {
      ...
      if (role.effective_policy === groundingMod.EFFECTIVE_POLICY.CONCEPTUAL) {
        ...
        if (briefRequiresExactDataMatch(brief)) {
          lines.push(
            "Where Show / must_show authorises specific numeric or labelled values, render those values on the image; use qualitative labels only for relationships that are not given specific values in Show / must_show."
          );
        } else {
          lines.push(
    ...
    function buildRelationshipItems(brief, mode) {
      ...
      var authorisedEdges = compilerMod.extractAuthorisedSemanticEdges(
        brief && brief.source_evidence,
        mustShow
      );
      var authorisedLines = compilerMod.formatAuthorisedSemanticEdgeLines(authorisedEdges);

      if (authorisedLines.length) {
        items.push(compilerMod.AUTHORISED_CONNECTOR_RULE);
        authorisedLines.forEach(function (line) {
          items.push(line);
        });
      } else if (isActivity) {
        items.push(
      ...
      if (
        ...
        if (isActivity && !briefRequiresExactDataMatch(brief)) {
      ...
      if (authorisedLines.length) {
        items.push(
          "Do not connect entities merely because they share a column, row, or list position."
        );
      }
    ...
    function buildVisualJobHumanPrompt(brief) {
      ...
      if (mustShow.length) {
        ...
        if (briefRequiresExactDataMatch(brief)) {
          pushSection(lines, "Show fidelity:", [
            "Every Show item above must appear as on-image content.",
            "Do not substitute qualitative stand-ins (for example lower/higher) for authorised numeric values, and do not leave labelled numeric result fields blank."
          ]);
        }
      }
    ...
    function normalizeAdditionalResourceRow(item, idx) {
    ...
    function normalizePageResourceRefsShape(value) {
      ...
      var additional = Array.isArray(src.additional_resources)
        ? src.additional_resources.map(function (row, idx) {
            return normalizeAdditionalResourceRow(row, idx);
          })
        : [];
      additional = additional
        ...
        .map(function (row, idx) {
          return normalizeAdditionalResourceRow(
            {
    ...
    function addAdditionalResourceReference(workspaceState, resourceId, linkText, externalUrl) {
      if (!workspaceState || !workspaceState.assembledPageSnapshot) {
        return { ok: false, code: "workspace_unavailable" };
      }
      var rid = String(resourceId || "").trim();
      var text = String(linkText || "").trim();
      var url = String(externalUrl || "").trim();
      if (!rid || !text) return { ok: false, code: "invalid_reference" };
      var refs = normalizePageResourceRefsShape(workspaceState.pageResourceRefs);
      refs.additional_resources.push(
        normalizeAdditionalResourceRow(
          {
            resource_id: rid,
            link_text: text,
            external_url: url,
          ...
          refs.additional_resources.length
        )
      );
      writePageResourceRefsToPage(workspaceState, refs);
      return { ok: true };
    ...
    function renderVideoAndResourcesAuthoring(vm) {
      ...
      var resourceItems = (refs.additional_resources || [])
        .map(function (row, idx) {
          ...
          var externalUrl = String(row.external_url || meta.external_url || meta.href || "").trim();
          var isExternal = /^https?:\/\//i.test(externalUrl) && !/^data:/i.test(externalUrl);
          var detail = isExternal
      ...
      if (view === OUTPUT_VIEWS.resources) {
        return (
          ...
          '<p class="util-vj-muted">Add an uploaded file or an external http(s) URL. Learner-facing link text is required.</p>' +
          ...
          '<input type="url" class="util-vj-form-control util-vj-resource-url" data-resource-url-input data-draft-field="resourceUrl" placeholder="https://… (or choose a file)" value="' +
          escapeHtml(String(drafts.resourceUrl || "")) +
