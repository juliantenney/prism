...
(function (root, factory) {
  if (typeof module === "object" && module.exports) {
    ...
    var grounding = root.PRISM_VISUAL_MATERIAL_ROLE_GROUNDING;
    root.PRISM_VISUAL_JOBS_PLANNER = factory(contract, grounding);
  }
})(typeof globalThis !== "undefined" ? globalThis : this, function (contractMod, groundingMod) {
  "use strict";
  ...
  if (!groundingMod || typeof groundingMod.buildMaterialRoleContext !== "function") {
    throw new Error("prism-visual-jobs-planner requires prism-visual-material-role-grounding");
  }
  ...
  function resolvedScope(record) {
    if (contractMod.sprint38 && typeof contractMod.sprint38.resolveAffordanceScope === "function") {
      return contractMod.sprint38.resolveAffordanceScope(record);
    }
  ...
  function buildSectionIndex(page) {
    var index = {};
    if (!page || !Array.isArray(page.sections)) return index;
    page.sections.forEach(function (section) {
      if (!section || typeof section !== "object") return;
      var sid = String(section.section_id || "").trim();
      if (!sid) return;
      var key = normalizeKey(sid);
      if (!index[key]) {
        index[key] = {
          section_id: sid,
          section: section,
        ...
        index[key].section = Object.assign({}, index[key].section, section);
      }
    });
    if (page && Array.isArray(page.materials)) {
      page.materials.forEach(function (mat) {
        if (!mat || typeof mat !== "object") return;
        var sid = String(mat.section_id || "").trim();
        if (!sid) return;
        var bucket = index[normalizeKey(sid)];
        if (!bucket) {
          bucket = {
            section_id: sid,
            section: null,
          ...
          index[normalizeKey(sid)] = bucket;
        }
        var mid = String(mat.material_id || mat.kind || "").trim();
        if (mid) bucket.materials_by_key[normalizeKey(mid)] = mat;
      });
    }
    return index;
  }
  ...
  function resolveActivityMaterial(bucket, activityId, materialKey) {
    ...
    return {
      ...
      record: {
        ...
        material_id: isNonEmptyString(mat.material_id) ? String(mat.material_id).trim() : "",
        material_type: isNonEmptyString(mat.material_type || mat.type)
          ? String(mat.material_type || mat.type).trim()
          : "",
        content: payload.content,
        content_structured: payload.content_structured,
        content_text: payload.content_text,
  ...
  function enrichResolvedSourcesWithMaterialRole(
    affordance,
    activityIndex,
    resolvedSources,
    errors,
    ...
    var activityId = String(affordance.activity_id || "").trim();
    var anchorRaw = hasOwn(affordance, "material_anchor")
      ? affordance.material_anchor
      : null;
    var materialAnchor = isNonEmptyString(anchorRaw) ? String(anchorRaw).trim() : "";

    if (
      groundingMod.contextClaimsScenarioGroundingWithoutAnchor(affordance) &&
      resolvedScope(affordance) === "activity"
    ) {
      errors.push(
        makeIssue(
          "VPC_SCENARIO_GROUNDING_WITHOUT_ANCHOR",
          path +
            ": context claims activity scenario grounding but material_anchor is absent — anchor the represented material or remove scenario-grounding language",
          { path: path, index: index, affordance_id: affordanceId, activity_id: activityId }
        )
      );
      return null;
    }

    var representedMaterial = null;
    if (materialAnchor) {
      if (!activityId) {
        errors.push(
          makeIssue(
            "VPC_MATERIAL_ANCHOR_ACTIVITY",
            path + ": material_anchor requires activity_id on activity-scoped affordances",
            { path: path, index: index, affordance_id: affordanceId }
          )
        );
        return null;
      }
      var resolved = groundingMod.resolveMaterialAnchorStrict(
        activityIndex,
        activityId,
        materialAnchor
      );
      ...
      representedMaterial = resolved.material;
    }

    var roleContext = groundingMod.buildMaterialRoleContext({
      representedMaterial: representedMaterial,
      materialAnchor: materialAnchor,
      activityId: activityId,
      authoredExactDataMatch: affordance.requires_exact_data_match === true
    });

    var sources = Array.isArray(resolvedSources) ? resolvedSources.slice() : [];
    var representedEvidence = groundingMod.buildRepresentedSourceEvidence(
      roleContext.represented_material
    );
    if (representedEvidence) {
      sources = sources.filter(function (src) {
        return sourceMaterialIdFromRecord(src) !== representedEvidence.material_id;
      });
      sources.unshift(representedEvidence);
    }

    return { resolved_sources: sources, material_role: roleContext };
  }

  function sourceMaterialIdFromRecord(src) {
    return String((src && src.material_id) || "").trim();
  }

  function resolveSectionField(bucket, sectionId, fieldPath) {
    if (!bucket || !bucket.section) {
      return {
        ok: false,
        code: "VPC_PLANNER_SOURCE_UNRESOLVED",
        message: "unknown section_id '" + sectionId + "' for evidence anchor"
      };
    }
    var remainder = String(fieldPath || "").trim();
    if (remainder.indexOf("materials.") === 0) {
      var materialKey = remainder.slice("materials.".length);
      var mat = bucket.materials_by_key[normalizeKey(materialKey)];
      if (!mat) {
        return {
          ok: false,
          code: "VPC_PLANNER_SOURCE_UNRESOLVED",
          message:
            "unknown materials key '" + materialKey + "' on section '" + sectionId + "'"
        };
      }
      var payload = extractContentPayload(mat.body != null ? mat.body : mat);
      return {
        ok: true,
        record: {
          anchor_kind: "section_material",
          section_id: sectionId,
          material_id: String(mat.material_id || materialKey).trim(),
          material_type: mat.kind || mat.material_type || "",
          content: payload.content,
          content_structured: payload.content_structured,
          content_text: payload.content_text,
          content_type: payload.content_type,
    ...
    var section = bucket.section;
    var value = section[remainder];
    if (value == null && section.content && typeof section.content === "object") {
      value = section.content[remainder];
    }
    if (value == null) {
      // Still allow planning when the section exists but the named field is soft/optional.
      value = "";
    }
    var fieldPayload = extractContentPayload(value);
    return {
      ok: true,
      record: {
        anchor_kind: "section_field",
        section_id: sectionId,
        field: remainder,
        content: fieldPayload.content,
        content_structured: fieldPayload.content_structured,
        content_text: fieldPayload.content_text,
        content_type: fieldPayload.content_type,
        source_kind: "section_substance"
      }
    };
  }

  function resolveEvidenceAnchor(page, activityIndex, anchor, affordance, sectionIndex) {
    ...
    var activityBucket = activityIndex[normalizeKey(rootId)];
    if (activityBucket) {
      var result;
      if (remainder.indexOf("materials.") === 0) {
        var materialKey = remainder.slice("materials.".length);
        result = resolveActivityMaterial(activityBucket, activityBucket.activity_id, materialKey);
      } else {
        result = resolveActivityField(activityBucket, activityBucket.activity_id, remainder);
      }
    ...
    var sectionBucket = sections[normalizeKey(rootId)];
    if (sectionBucket || resolvedScope(affordance) === "section") {
      var sectionResult = resolveSectionField(
        sectionBucket || { section_id: rootId, section: null, materials_by_key: {} },
        rootId,
        remainder
      );
      if (!sectionResult.ok) return sectionResult;
      if (spoilerProhibited(affordance, sectionResult.record.source_kind)) {
        return {
          ok: false,
          code: "VPC_PLANNER_SPOILER_SOURCE_PROHIBITED",
          message:
            ...
            trimmed +
            "' targets source_kind '" +
            sectionResult.record.source_kind +
            "' prohibited by anti_spoiler boundary"
        };
      }
    ...
    return {
      ok: false,
      code: "VPC_PLANNER_SOURCE_UNRESOLVED",
      message: "unknown activity_id '" + rootId + "' for evidence anchor"
    };
  }
  ...
  function buildJobId(schemaVersion, affordance) {
    ...
    var identityPart =
      scope === "page"
        ? "page"
        : scope === "section"
          ? String((affordance && affordance.section_id) || "section")
          : String((affordance && affordance.activity_id) || "activity");
    return [
      "vj",
      slugToken(schemaVersion),
      slugToken(affordance && affordance.affordance_id),
      slugToken(scope),
      slugToken(identityPart),
  ...
  function buildJobFromAffordance(affordance, index, schemaVersion, resolvedSources) {
    ...
    var job = {
      ...
      alt_text: deepClone(affordance.alt_text),
      detailed_description: deepClone(affordance.detailed_description),
    ...
    if (scope === "activity") {
      ...
      job.section_id = String(affordance.section_id || "").trim();
      job.provenance.section_id = job.section_id;
      job.provenance.section_scope = true;
    } else {
  ...
  function planPrismVisualJobs(page) {
    ...
    var contract = contractMod.validateVisualPlanningContract(workingPage);
    var diagnostics = emptyDiagnostics();
    var errors = [];
    var warnings = [];

    (normalized.warnings || []).forEach(function (warn) {
      warnings.push(
        makeIssue(warn.code || "VPC_EVIDENCE_ANCHOR_WARNING", warn.message || String(warn), {
          path: warn.path,
          index: warn.index,
          affordance_id: warn.affordance_id
        })
      );
    });
    if ((normalized.errors || []).length && !(contract.errors || []).length) {
      (normalized.errors || []).forEach(function (err) {
        errors.push(
          makeIssue(err.code || "VPC_EVIDENCE_ANCHOR_MALFORMED", err.message || String(err), {
            path: err.path,
            index: err.index,
    ...
    diagnostics.affordances = Array.isArray(workingPage.visual_affordances)
      ? workingPage.visual_affordances.length
      : 0;

    if (!contract.valid || errors.length) {
    ...
    var activityIndex = buildActivityIndex(workingPage);
    var sectionIndex = buildSectionIndex(workingPage);
    var affordances = Array.isArray(workingPage.visual_affordances)
      ? workingPage.visual_affordances
      : [];
    var pendingJobs = [];
    var seenJobIds = {};
    ...
    affordances.forEach(function (affordance, index) {
      ...
      for (var ai = 0; ai < anchors.length; ai += 1) {
        var resolved = resolveEvidenceAnchor(
          workingPage,
          activityIndex,
          anchors[ai],
          affordance,
      ...
      var roleEnriched = enrichResolvedSourcesWithMaterialRole(
        affordance,
        activityIndex,
        resolvedSources,
        errors,
      ...
      var job = buildJobFromAffordance(
        affordance,
        index,
        schemaVersion,
        roleEnriched.resolved_sources
      );
      job.material_role = deepClone(roleEnriched.material_role);
      if (job.material_role && job.material_role.requires_exact_data_match === true) {
        job.requires_exact_data_match = true;
      }
    ...
    jobs.forEach(function (job) {
      ...
      if (job.scope === "section") diagnostics.section_scoped_jobs += 1;
    });
  ...
  return {
    ...
    buildSectionIndex: buildSectionIndex,
