...
(function (root, factory) {
  ...
  if (!vocabularyMod || typeof vocabularyMod.approvedFunctionSet !== "function") {
  ...
  function approvedFunctionSet() {
    return vocabularyMod.approvedFunctionSet();
  }

  function normalizeKey(value) {
    return vocabularyMod.normalizeFunctionKey
      ? vocabularyMod.normalizeFunctionKey(value)
      : String(value || "")
          .trim()
          .toLowerCase();
  }
  ...
  function validateEpisodePlanV1(episodePlan, prefix) {
    ...
    beats.forEach(function (beat, index) {
      ...
      if (!fn) {
        ...
        vocabularyMod && typeof vocabularyMod.isApprovedFunction === "function"
          ? !vocabularyMod.isApprovedFunction(beat.function)
          : !fnSet[fn]
      ) {
        errors.push(
  ...
  /**
   * Validate episode_plan beat vocabulary on a vNext page shell / assembled page.
   * Returns structured diagnostics so capture enforcement can reject illegal beats
   * (e.g. "consolidation") before the renderer becomes the first failure point.
   *
   * @param {object} page
   * @param {{ canonicalSource?: string, owner?: string }} [options]
   * @returns {{ ok: boolean, errors: string[], diagnostics: object[] }}
   */
  function validatePageEpisodePlanVocabulary(page, options) {
    var opts = options && typeof options === "object" ? options : {};
    var owner = String(opts.owner || "Episode Plan capture").trim();
    var canonicalSource = String(opts.canonicalSource || "Episode Plan FunctionEnum").trim();
    var errors = [];
    var diagnostics = [];
    var fnSet = approvedFunctionSet();

    function inspectPlan(activityId, episodePlan, pathLabel) {
      if (!episodePlan || typeof episodePlan !== "object") {
        errors.push(pathLabel + ": missing episode_plan object");
        return;
      }
      var planCheck = validateEpisodePlanV1(episodePlan, pathLabel);
      if (!planCheck.ok) {
        (planCheck.errors || []).forEach(function (msg) {
          errors.push(msg);
        });
      }
      var beats = Array.isArray(episodePlan.beats) ? episodePlan.beats : [];
      beats.forEach(function (beat, index) {
        var fn = normalizeKey(beat && beat.function);
        var approved =
          vocabularyMod && typeof vocabularyMod.isApprovedFunction === "function"
            ? vocabularyMod.isApprovedFunction(beat && beat.function)
            : !!(fn && fnSet[fn]);
        if (!fn || approved) return;
        diagnostics.push({
          code: "NON_CANONICAL_EPISODE_PLAN_BEAT",
          activityId: activityId || "",
          beat: String((beat && beat.function) || ""),
          beatIndex: index,
          owner: owner,
          canonicalSource: canonicalSource,
          message:
            "NON_CANONICAL_EPISODE_PLAN_BEAT Activity: " +
            (activityId || "(unknown)") +
            " Beat: " +
    ...
    if (!page || typeof page !== "object" || Array.isArray(page)) {
      return {
        ok: false,
        errors: ["page episode_plan vocabulary: invalid page object"],
    ...
    if (Array.isArray(page.activities)) {
      page.activities.forEach(function (activity, index) {
        ...
        var activityId = String(activity.activity_id || "activities[" + index + "]").trim();
        inspectPlan(activityId, activity.episode_plan, "activities[" + index + "].episode_plan");
      });
    }

    if (Array.isArray(page.episode_plans)) {
      page.episode_plans.forEach(function (row, index) {
        if (!row || typeof row !== "object") return;
        var activityId = String(row.activity_id || "episode_plans[" + index + "]").trim();
        var plan = resolveEpisodePlanFromRow(row, index);
        inspectPlan(activityId, plan, "episode_plans[" + index + "].episode_plan");
      });
    }
  ...
  return {
    ...
    FUNCTION_ENUM: vocabularyMod.FUNCTION_ENUM,
    frozenArchetypeSet: frozenArchetypeSet,
    approvedFunctionSet: approvedFunctionSet,
    isApprovedFunction: vocabularyMod.isApprovedFunction,
    ...
    validatePageEpisodePlanVocabulary: validatePageEpisodePlanVocabulary
  };
});
