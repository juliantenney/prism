...
(function (root, factory) {
  ...
  var MATHJAX_PACKAGE_LOADER_PATH = "lib/mathjax/es5/tex-chtml.js";
  var MATHJAX_CDN_REFERENCE_RE = /cdn\.jsdelivr\.net\/npm\/mathjax/i;
  ...
  function getMathJaxPackageAssetsApi(opts) {
    ...
    if (typeof require === "function") {
      try {
        ...
        /* browser bundle — fall through to global */
      }
    }
    var root =
      typeof globalThis !== "undefined"
        ...
        : typeof window !== "undefined"
          ? window
          : null;
    if (root && root.PRISM_MATHJAX_PACKAGE_ASSETS) {
      return root.PRISM_MATHJAX_PACKAGE_ASSETS;
    }
    return null;
  }

  function rewriteMathJaxLoaderInPackageHtml(html, mathJaxApi) {
    var out = String(html == null ? "" : html);
    if (mathJaxApi && typeof mathJaxApi.rewriteMathJaxLoaderToPackagePath === "function") {
      return mathJaxApi.rewriteMathJaxLoaderToPackagePath(out);
    }
    if (out.indexOf(MATHJAX_CDN_LOADER_SRC) >= 0) {
      return out.split(MATHJAX_CDN_LOADER_SRC).join(MATHJAX_PACKAGE_LOADER_PATH);
    }
    return out;
  }

  function packageHtmlStillReferencesMathJaxCdn(html) {
    return MATHJAX_CDN_REFERENCE_RE.test(String(html == null ? "" : html));
  }
  ...
  function appendMathJaxPackageAssets(htmlOut, packageAssets, packagedPaths, opts) {
    ...
    var htmlRewritten = rewriteMathJaxLoaderInPackageHtml(htmlOut, mathJaxApi);
    if (packageHtmlStillReferencesMathJaxCdn(htmlRewritten)) {
      return {
        html: htmlOut,
        error: {
  ...
  function buildLearnerPackage(input) {
    ...
    additionalResources.forEach(function (item, index) {
      ...
      if (/^https?:\/\//i.test(sourceHref)) {
        // External URL resources stay as absolute links; do not package bytes.
        return;
      }
  ...
  return {
    ...
    rewriteMathJaxLoaderInPackageHtml: rewriteMathJaxLoaderInPackageHtml,
    packageHtmlStillReferencesMathJaxCdn: packageHtmlStillReferencesMathJaxCdn,
