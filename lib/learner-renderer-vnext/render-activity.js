...
var groupingDisplayLabel = require("./grouping-display-label");
  ...
  if (String(activity.grouping || "").trim()) {
    badges.push(
      '<span class="util-badge util-badge-group">' +
        html.escapeHtml(groupingDisplayLabel.labelForGroupingDisplay(activity.grouping)) +
        "</span>"
    );
  }
...
function renderActivityBeats(activity, composition, renderOptions) {
  ...
  var transferRendered = false;
  var momentRenderOptions = Object.assign({}, renderOptions || {}, {
    taskCheckNav: taskCheckNav.shouldEnableTaskCheckNav(composition)
  });
  ...
  beats.forEach(function (beat) {
    ...
    if (composition && composition.checkMoment && !checkRendered && shouldInjectMoment(composition, "check", beatFunction)) {
      ...
      checkRendered = true;
      if (composition.transferMoment && !transferRendered) {
        parts.push(
          renderComposedMoment.renderTransferMoment(
            composition.transferMoment,
            activity.id,
            activity,
            momentRenderOptions
          )
  ...
  if (composition && composition.transferMoment && !transferRendered) {
    parts.push(
      renderComposedMoment.renderTransferMoment(
        composition.transferMoment,
        activity.id,
