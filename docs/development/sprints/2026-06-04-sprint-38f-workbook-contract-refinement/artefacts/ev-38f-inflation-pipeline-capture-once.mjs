/**
 * Sprint 38-F — Inflation AFTER re-run (38F-4) post 38F-2 pack refinement.
 * Run: node docs/development/sprints/2026-06-04-sprint-38f-workbook-contract-refinement/artefacts/ev-38f-inflation-pipeline-capture-once.mjs
 *
 * Writes EV-38F-AFTER-* only. Does not overwrite EV-01, EV-38E5-AFTER, EV-38E10-AFTER, or 38-B fixtures.
 */
import fs from "fs";
...
function augment(api, md, wf, sectionHeading, stepId, title, outputName) {
  ...
  const seeded = api.buildSeededStepPromptForWorkflowStep({
    workflowName: "Inflation EV-38F-AFTER",
    step,
...
function analyzeMaterialValue(value, pathLabel) {
  ...
  const isTablePath = /_table|comparison_table|analysis_table|impact_table|classification_table|reference_table|data_table|decision_table|planning_table|template/i.test(
    pathLabel
  );
...
const TABLE_TYPE_RE =
  /classification_table|comparison_table|analysis_table|impact_table|reference_table|data_table|decision_table|planning_table|_table/;
...
function typeFamilies(types) {
  ...
  types.forEach((t) => {
    ...
    if (TABLE_TYPE_RE.test(x)) families.add("table");
...
function scoreDla(learning_activities) {
  ...
  const tableOnly =
    typeList.length > 0 && typeList.every((t) => TABLE_TYPE_RE.test(t));
  return {
    activityCount: acts.length,
    durationSum,
    uniqueTypes: typeList,
    tableOnlySpec: tableOnly,
    hasTableSpec: typeList.some((t) => TABLE_TYPE_RE.test(t)),
    hasScenarioSpec: typeList.some((t) => t === "scenario"),
...
const ctxHeader =
  "Workflow: Inflation self-directed learner workbook (Sprint 38-F-4 — same topic/brief as EV-01 / EV-38E10).\n" +
  "page_profile=learner; apply pack DLA-WB (38E-8 + 38F-2) and GAM-WB (38E-9 + 38F-2).\n";
...
const dlaText = await callOpenAI(
  ...
  ctxHeader +
    "\n\nMandatory: 60-minute self_study_workbook. delivery_notes: resource_intent self_study_workbook, session_duration_target_minutes 60, consolidation_requirement, workbook_contract_applied true.\n\n38E-8 + 38F-2 mandatory JSON rows (all coexist): (1) early activity: worked_example AND sample_output; (2) final activity: consolidation_summary; (3) practice activity: analysis_table or comparison_table (DLA-WB-06a); (4) practice activity with household scenarios: type scenario with ≥2 cases (DLA-WB-18) — NOT task_cards-only for case bodies.\n\nApply ALL DLA-WB rules below.\n\nUpstream learning_outcomes:\n" +
...
const gamTextRaw = await callOpenAI(
  ...
  ctxHeader +
    "\n\nMandatory 38E-9 + 38F-2: Material: worked_example, sample_output, consolidation_summary with full bodies; Material: scenario when DLA lists scenario; pipe-table Material for DLA table types; task_cards cannot replace scenario (F5); missing table when DLA listed = FAIL (F6).\n\nUpstream learning_activities:\n" +
...
const tableOnlyGam =
  gamTypes.length > 0 && gamTypes.every((t) => TABLE_TYPE_RE.test(t));
...
const report = {
  ...
  runId: RUN_PREFIX,
  phase: "38F-4",
  priorRuns: ["NEG-EV-01", "EV-38E5-AFTER", "EV-38E10-AFTER"],
  model,
  brief: BRIEF,
  promptChars: {
  ...
  gam: {
    ...
    hasTable: gamTypes.some((t) => TABLE_TYPE_RE.test(t)),
