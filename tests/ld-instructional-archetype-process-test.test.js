...
test("process stamp selects process_walkthrough rule only", () => {
  ...
  assert.match(routed, /worked walkthrough/i);
  assert.match(routed, /every supplied stage/i);
  assert.match(routed, /explicit intermediate finding/i);
...
test("process_walkthrough rule duties: finding-transfer enactment", () => {
  const rule = archetype.RULES.process_walkthrough;
  assert.match(rule, /worked walkthrough/i);
  assert.match(rule, /every supplied stage/i);
  assert.match(rule, /perform the reasoning for that stage on the investigation/i);
  assert.match(rule, /explicit intermediate finding/i);
  assert.match(rule, /carry that finding forward/i);
  assert.match(rule, /next stage uses it/i);
  assert.match(rule, /Each later stage must consume information generated by earlier stages/i);
  assert.match(rule, /final conclusion must depend on those findings/i);
  assert.match(rule, /Show the reasoning being performed/i);
  assert.match(rule, /do not restate stage labels as procedural Step\/how-to/i);
  assert.match(rule, /do not write advice about what a learner should do/i);
  assert.match(rule, /do not produce a how-to guide/i);
  assert.match(rule, /procedural checklist/i);
  assert.doesNotMatch(rule, /distinct section: state what is being examined/);
  ...
  assert.match(block, /explicit intermediate finding/i);
  assert.match(block, /how-to guide/i);
  assert.doesNotMatch(block, /Realise every entry in required_links/);
  assert.doesNotMatch(block, /Build a coherent account of the named system/);
});

test("assembled process routing rejects procedural Step how-to shape in rule duties", () => {
  const page = pageWithMaterial({
    material_id: "A4-M1",
    material_type: "worked_example"
  });
  archetype.applyEnzymesProcessTestPlanToDlaPage(page);
  const block = archetype.buildArchetypeRoutingBlock(page);
  // Intended duties present (fail if we regress to section-only wording).
  assert.match(block, /produce an explicit intermediate finding/i);
  assert.match(block, /carry that finding forward/i);
  assert.match(block, /final conclusion must depend on those findings/i);
  assert.match(block, /do not restate stage labels as procedural Step\/how-to/i);
  // Old section-only obligations must not be the sole realisation contract.
  assert.doesNotMatch(
    block,
    /For each stage as a distinct section: state what is being examined/
  );
});
